...
Scorer v10 的断兼容 schema、两 logit 模型、argmax decoder、严格 dataset validator 和 random-init trainer plumbing 已完成真实 full training。原 numeric checkpoint 的 corrected-r3 人工 gate 已完成并确认一处真语音截断，因此被否决。首个 internal-background A/B checkpoint 的 29 条严格差集也已人工确认两处真语音误删/截断，因此同样被否决。提高 worst-frame 保护后的保守候选仍有 36 条未被既有人工 background 区间覆盖的新增 drop，当前等待 focused audit；production registry 与 `segment()` 继续 fail-fast 为 `pending_binary_scorer_audit`。
