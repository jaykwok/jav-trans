...
This remains a candidate, not a training authorization. `agents/audits/20260721_081529_scorer-v10-canonical-r4-replacement19/` audits every rendered occurrence using separate exact source-event, rendered occurrence and actual background-to-speech playback, plus a whole-island player for sentence context. A boundary-incomplete, no-longer-target or unsure verdict blocks the repair. Until all 19 pass, feature-label recompilation, a new training manifest, Scorer retraining and registry promotion remain disabled.
