...
Scorer v10 的断兼容 schema、两 logit 模型、argmax decoder、严格 dataset validator 和 random-init trainer plumbing 已完成真实 full training。原 numeric checkpoint、首个 internal-background A/B 和提高 worst-frame 保护后的保守候选都在 corrected-r3 人工 gate 中出现真语音误删/截断，因此全部被否决。production registry 与 `segment()` 继续 fail-fast 为 `pending_binary_scorer_audit`。
...
The first saved verdict set is complete at `36/36`: `canonical_should_be_background=34 / true_speech_clipped=1 / unsure=1`. `000673` is a confirmed true-speech clipping violation, so the conservative checkpoint is rejected independently of the remaining ambiguity. The unsure row `001747` contains two 20–40 ms dropped intervals inside the spoken hesitation `wa… watashi mo`; the red frames may themselves be silence, but the current no-merge workflow turns the original `0–2.32s` unit into three downstream islands. The audit UI and gate therefore add `same_asr_unit_fragmented`: this is a workflow-continuity blocker distinct from zero-clipping and from a safe canonical-background repair. CueQC replay remains diagnostic-only and cannot rescue a Scorer candidate that already deleted true speech.
