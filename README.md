...
- 默认 Boundary Refiner：随源码提供 learned `transformers.Mamba2Model` 小 checkpoint，文件为 `src/boundary/checkpoints/boundary_refiner.pt`，用于边界候选的 merge / split 决策。
- 默认显存目标：单阶段峰值适配 6GB 级 NVIDIA 显卡。更小显存可手动降低 `.env` 中的 ASR / aligner batch size。
- 当前字幕策略：边界层尽量把 ASR chunk 切成接近一句台词；字幕层负责 2-frame gap、显示时长、短 cue 合并和读速控制。
...
Release 版是否内置模型以对应 release notes 为准。当前 main 分支默认会从 Hugging Face 下载模型到 `models/`。
...
| Boundary Refiner | learned `transformers.Mamba2Model` checkpoint | `src/boundary/checkpoints/boundary_refiner.pt` |

常用配置见 [.env.example](.env.example)。通常只需要修改 API key、翻译模型、`HF_ENDPOINT`、ASR backend 和 batch size。

分发 / 推理只需要源码内置的 `src/boundary/checkpoints/boundary_refiner.pt`、默认 0.6B ASR / frozen feature 模型和 forced aligner。Boundary Refiner 训练时生成的 CUDA feature cache、synthetic WAV、sequence JSONL 和 `datasets/train/...` 产物都不是运行依赖，不随源码或 Windows release 打包。
...
- `src/boundary/`：Boundary features、candidate extraction、Boundary Refiner 接口、sequence backbone、constrained planner 和 boundary-cache v1。
