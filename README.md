...
- SpeechBoundary-JA scorer 当前 schema 是 `speech_boundary_ja_mamba2_frame_boundary_scorer_v4`，输出三头：`speech_prob`、`split_boundary_prob`、`drop_gap_prob`。运行时先用 speech hysteresis 找 coarse island，用 `drop_gap_prob` 删除明确非语音 gap，再用 `split_boundary_prob` 做 adaptive top-k peak picking / NMS / snap-to-valley 切开 island；不再支持旧 `cut_prob` gate 或绝对 split threshold，也不在 ASR 后做时长/文本比例硬拆。
...
- 当前字幕策略：切分发生在 ASR 前的 scorer/refiner 阶段。ASR 后的 aligned_segments 只保留按 source chunk 边界聚合的结果，不再做时长/文本比例硬拆；SRT writer 默认只做排序、no-overlap、2-frame gap 和必要 timing polish，不再做 subtitle-level soft split 或 display policy。
- ASR chunk 导出只保留 0.08s 级去毛刺过滤，避免 scorer 已识别的短促台词在写 WAV 阶段被 0.25s 旧阈值吞掉。
...
-> speech island / drop gap / adaptive split peak decoding
...
- `tmp/cache/boundary/`：SpeechBoundary-JA frame score 到 Boundary Planner 输出的 boundary-cache v7。
...
- `src/boundary/`：Boundary Refiner v6 checkpoint loader、edge-sequence Mamba2 adapter、core planner 和 boundary-cache v7。
