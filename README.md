...
视频输入
  ...
  -> 按 ASR repo 进入互不混用的边界链
     - 1.7B：Outer Edge Refiner v2
       -> Acoustic Split v4 binary argmax
       -> provisional sub-islands
       -> Pre-ASR CueQC v13 binary argmax
       -> Inner Edge Refiner v1 paired edges
       -> chunk packing / boundary-cache
     - 0.6B legacy：Outer Edge Refiner v1
       -> Semantic Split v2 calibrated threshold chain
       -> Cut Edge Refiner v1
       -> Pre-ASR CueQC v12 drop-threshold
       -> chunk packing / boundary-cache
     - 两条链都使用 repo-bound learned Boundary Proposal v1
...
- 0.6B 继续使用 Split v2 / CueQC v12 legacy threshold chain；其 checkpoint、runtime 和数据合同不与 1.7B 混用。
...
1.7B 使用 Boundary pipeline v11：Outer v2 → Acoustic Split v4 binary → provisional sub-islands → CueQC v13 binary → Inner v1 → Chunk；0.6B 保持 Split v2 / Cut v1 / CueQC v12 legacy chain，禁止与 1.7B checkpoint 半升级混用。模型缺失、repo 不匹配或 schema 不兼容都会直接报错（无规则 fallback、无静默迁移）。实验指标与版本决策见 [docs/HISTORY.md](docs/HISTORY.md)。
...
# 仅 0.6B CueQC v12 legacy 使用；1.7B CueQC v13 binary 不读取此阈值。
...
Boundary cache 当前版本为 `v20`。cache 签名包含 repo-bound 模型路径和运行配置，不兼容缓存会直接 miss。
