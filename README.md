...
- Qwen3-ASR SFT 云端建议命令：云服务器先设置 `HF_HOME` 到数据盘，海外节点优先直连官方 HF；国内节点可按平台实测加 `HF_ENDPOINT`。云端默认没有 `Qwen/Qwen3-ASR-1.7B`，所以必须先下载模型，再生成 full SFT 数据。新增 `tools/fusionvad_ja/prepare_qwen_asr_cloud_assets.sh`，默认把模型下载到 `models/Qwen-Qwen3-ASR-1.7B`，把数据包生成到 `datasets/train/qwen3-asr-ja-galgame/v1-full/`；可通过 `HF_CACHE_DIR`、`HF_ENDPOINT_VALUE`、`QWEN_MODEL_DIR`、`QWEN_MODEL_REVISION`、`HF_MAX_WORKERS`、`SFT_OUTPUT_ROOT`、`SFT_REVISION` 覆盖。full 模式中 train limit `0` 表示训练集不限量，val/test 默认从 shuffle 后流中各保留 ASR `1000/1000`、SER `500/500`，后续可按云端磁盘和训练预算调整。
