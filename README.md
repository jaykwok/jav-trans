...
“思考强度”三档 `low` / `medium` / `max`，默认 `medium`，**三档都会思考**。曾经还有一档真正关掉思考的 `none`，已于 2026-08-14 退役：同一部片、同一条时间轴只换这一档实测，关掉思考有 171 条 cue（10.1%）直接把日文原文抄了过去，而三个会思考的档都是 0 条——省下的时间不值一部片里十分之一的台词还是日文。

三档的实测差别（sample-b，1,700 条 cue，只重跑翻译阶段）：

| 档位 | 整片翻译耗时 | 完整率 | 整条未翻 | 中文字符 |
| --- | ---: | ---: | ---: | ---: |
| `low` | 238s | 1,700/1,700 | 0 | 16,490 |
| `medium`（默认） | 456s | 1,700/1,700 | 0 | 15,464 |
| `max` | 599s | 1,700/1,700 | 0 | 14,889 |
...
每个翻译请求的输出上限**不是** `TRANSLATION_MAX_TOKENS`（384000，只是给 API 模型的天花板），而是按这一批的源字符数算出来的：`源字符数 × TRANSLATION_OUTPUT_CHAR_RATIO + 结构开销 + 思考额度`。两者取小，所以调天花板不会有任何效果，要放宽只能调 `TRANSLATION_OUTPUT_CHAR_RATIO`（默认 1.5）。

**这个上限还要付思考的钱**：`max_tokens` 在推理模型上先花在思考流上，而前两项只建模了看得见的回复。实测 deepseek-v4-flash，`medium` 下一个 8 条 cue 的小批要思考 2,058 字符、54 条批 20,231 字符，`max` 更高（54 条 53,388 字符），都比按字符算出来的预算高一到两个数量级。所以预算里固定含一份 `TRANSLATION_REASONING_TOKEN_ALLOWANCE`（默认 32000；`max` 再乘 `TRANSLATION_REASONING_MAX_EFFORT_MULTIPLIER`，默认 2.0）。三档全部会思考，所以三档都拿这份额度——`low` 与 `medium` 共用同一份，因为实测两者的思考量互有高低、分不开。思考量主要由强度决定、不与源字符成比例，所以它是固定额度而不是第二个比例系数。
...
这类失败现在会让下一次请求**减半**（批内只降不升），直到降到模型能抄对 id 的规模，日志里对应 `batch_span_narrowed` 事件；剩下的 id 走正常的补发路径。所以一般不需要手动干预。如果整部片反复在同一处失败，可以在 `.env` 里直接调小起始批：

```env
TRANSLATION_BATCH_SIZE=24
```

它是实际生效的上限（自动批大小会取它与按 worker 数算出的值的较小者）。批越小，每条回复要抄的 id 越少，代价是请求数变多。
...
整片耗时的大头是 ASR 解码。151 分钟的片子在 RTX 4060 Ti 上冷跑 920 秒（约 9.9× 实时），其中文本转写 798s、静音切块 23s、字幕时间轴 32s、翻译 47s、音频准备与写出各不到 6s。**但那 47s 是在已退役的“不思考”档下测的**：同一部片按现在的默认 `medium` 重翻整片要 456s、`low` 238s、`max` 599s，所以现在一次冷跑更接近 1,330 秒（选 `low` 约 1,110 秒），翻译占三分之一左右。除解码和翻译之外的所有阶段加起来只有一分钟，调它们省不出时间。
