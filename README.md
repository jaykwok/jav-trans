...
- 本次是断兼容重构：旧 v3 scorer、v5 refiner、v3-Fusion CueQC checkpoint 不会被兼容加载。1.7B 新 checkpoint 训练落盘前，默认 registry 指向目标文件名并会 fail-fast。
...
- `tools.boundary.build_refiner_frame_sequence_dataset`：基于 scorer v4 island edge 导出 `boundary_edge_refiner_dataset_v6` 训练 JSONL。
