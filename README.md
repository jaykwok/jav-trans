...
- 逐句字幕对比报告：`video/匿名样片 K/匿名样片 K.subtitle_compare.html`。
...
- 逐句报告：`video/匿名样片 F/匿名样片 F_匿名样片 J.full_vad_modes_line_compare.html`，同一报告也归档到 `video/匿名样片 J/`。
...
- 匿名样片 K 汇总：`whisperseg-adaptive` 700 SRT / 210 ASR drops；`fusion_lite` 683 SRT / 167 drops；`fusion_lite_boost` 688 SRT / 185 drops；`fusion_lite_sigmoid` 677 SRT / 148 drops。
...
- 匿名样片 F 汇总：`whisperseg_adaptive` 230 SRT / 96 ASR drops；`fusion_lite` 231 SRT / 63 drops；`fusion_lite_boost` 230 SRT / 84 drops；`fusion_lite_sigmoid` 230 SRT / 50 drops。
- cue plan 短尾合并改为 frame-based 规则。
- 匿名样片 F 离线验证：`00:02:56,839 --> 00:02:59,160` 合并为 `アルマリスト 室で イラックス ステイマンを受けて`。
- 验证：字幕定向 50 passed。
