...
`0.6B` 和 `1.7B` 在 6GB 档默认统一使用 `ASR_WORKER_MODE=inproc`。`subprocess` 模式可以在 ASR 卡死或 OOM 时杀掉子进程重启，但会多一个 CUDA context；6GB 上尤其不建议给 `1.7B` 使用。

`inproc` 同进程推理显存占用更低，也少一次进程通信开销；代价是 ASR generate 真卡住时无法只中断 ASR 子进程，可能需要重启整个任务进程。高显存机器如果更看重故障隔离，可手动设 `ASR_WORKER_MODE=subprocess`。
