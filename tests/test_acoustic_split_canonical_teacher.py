...
from tools.audits import (
    evaluate_acoustic_split_canonical_candidate_audit as evaluate_audit,
)
from tools.audits import (
...
def test_canonical_selection_covers_false_cut_and_long_residual(tmp_path: Path) -> None:
    args = _args(tmp_path)

    selected = labeler.prepare_hard_cases(args)

    assert len(selected) == 1
    assert selected[0]["schema"] == labeler.SELECTION_SCHEMA
...
def test_canonical_heldout_is_known_same_sentence_continue(tmp_path: Path, monkeypatch) -> None:
    args = _args(tmp_path)
    ...
    assert heldout["window_id"] == "匿名样片 C-known-same-sentence-23.538"
...
def test_canonical_prompt_does_not_leak_model_scores_or_labels(tmp_path: Path) -> None:
...
def test_canonical_teacher_labels_one_window(tmp_path: Path, monkeypatch) -> None:
    ...
    assert all(row["schema"] == labeler.LABEL_SCHEMA for row in labels)
    assert all(row["prompt_version"] == labeler.PROMPT_VERSION for row in labels)
...
def test_canonical_clip_radius_is_cli_adjustable() -> None:
...
def test_canonical_total_candidate_limit_round_robins_windows() -> None:
    windows = [
...
def test_canonical_audit_prioritizes_disagreements_and_has_manual_controls(
    tmp_path: Path,
    monkeypatch,
) -> None:
    args = _args(tmp_path)
