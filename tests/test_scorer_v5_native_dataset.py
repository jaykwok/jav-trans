...
from boundary.ja.train import scorer_v5_targets_from_record, time_span_to_frame_range
from tools.boundary.ja.build_scorer_v5_native_dataset import (
    DATASET_SCHEMA,
    DEFAULT_MIX,
    build_scorer_v5_native_dataset,
...
def test_build_scorer_v5_native_dataset_labels_speech_and_split_only(tmp_path: Path):
    ...
    records = read_jsonl(tmp_path / "dataset" / "scorer_v5_native_labels.jsonl")
    ...
    pure_speech, pure_split = scorer_v5_targets_from_record(
        pure_negative,
    ...
    mixed_speech, _split = scorer_v5_targets_from_record(
        mixed,
    ...
    _speech, split_points = scorer_v5_targets_from_record(
        split_stress,
    ...
    long_speech, long_split = scorer_v5_targets_from_record(
        long_chain,
    ...
    assert float(np.max(long_split)) == 1.0
    ...
    splits = json.loads((tmp_path / "dataset" / "scorer_v5_native_splits.json").read_text(encoding="utf-8"))
...
def test_audited_low_info_vocalization_manifest_is_speech_negative_only(tmp_path: Path):
    ...
    speech, split_points = scorer_v5_targets_from_record(
        record,
