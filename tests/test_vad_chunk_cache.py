...
def _chunk_config() -> dict:
    return {
        ...
        "pre_asr_drop_gap_split_enabled": False,
        "pre_asr_drop_gap_split_min_parent_frames": 400,
        "pre_asr_drop_gap_split_min_gap_frames": 3,
        "pre_asr_drop_gap_split_min_gap_s": 0.60,
        "pre_asr_drop_gap_split_min_child_frames": 25,
        "pre_asr_drop_gap_split_max_children": 8,
        "pre_asr_drop_gap_split_threshold": 0.80,
        "pre_asr_risk_split_enabled": False,
        "pre_asr_risk_split_min_core_frames": 420,
        "pre_asr_risk_split_target_core_frames": 270,
        "pre_asr_risk_split_safe_core_frames": 360,
        "pre_asr_risk_split_min_gap_frames": 6,
        "pre_asr_risk_split_min_child_frames": 45,
        "pre_asr_risk_split_max_children": 8,
        "pre_asr_risk_split_threshold": 1.0,
        "pre_asr_risk_split_continuous_threshold": 2.0,
        "pre_asr_risk_split_valley_threshold": 0.20,
...
def test_vad_chunk_cache_round_trips_packed_chunks(monkeypatch, tmp_path):
    ...
    chunks = [
        PackedChunk(
            ...
            risk_split_count=1,
            risk_score=2.5,
            risk_reasons=("long_core", "internal_gap"),
    ...
    assert loaded_chunks[0].risk_split_count == 1
    assert loaded_chunks[0].risk_score == 2.5
    assert loaded_chunks[0].risk_reasons == ("long_core", "internal_gap")
...
def test_vad_chunk_cache_key_changes_with_pre_asr_risk_split(monkeypatch, tmp_path):
    ...
    cfg = _chunk_config()
    cfg["pre_asr_risk_split_enabled"] = True
    lookup_b = vad_chunk_cache.build_cache_lookup(
        str(audio),
        vad_signature={"backend": "fusionvad_ja", "threshold": 0.10},
        chunk_config=cfg,
...
class _DropGapScoredVadBackend(_CountingVadBackend):
    def segment(
        self,
        audio_path: str,
        *,
        ...
        del audio_path, target_sr, threshold_override
        self.calls += 1
        segments = [SpeechSegment(0.0, 15.0, 0.9)]
        return SegmentationResult(
            segments=segments,
            groups=[segments],
            method=self.name,
            ...
            parameters={
                ...
                "threshold": 0.10,
                "frame_scores": [0.9] * 15,
                "drop_gap_frame_scores": [0.05] * 5 + [0.90] * 3 + [0.05] * 7,
                "frame_hop_s": 1.0,
            },
            processing_time_sec=0.0,
        )
...
def test_pipeline_uses_drop_gap_scores_for_drop_gap_split_but_does_not_cache_scores(monkeypatch, tmp_path):
    monkeypatch.setenv("VAD_CHUNK_CACHE_DIR", str(tmp_path / "vad-cache"))
    monkeypatch.setenv("ASR_CHUNK_PACKING_ENABLED", "1")
    monkeypatch.setenv("ASR_CHUNK_PACK_FRAME_HOP_S", "1.0")
    monkeypatch.setenv("ASR_CHUNK_PACK_WINDOW_FRAMES", "30")
    monkeypatch.setenv("ASR_CHUNK_PACK_RESERVE_FRAMES", "2")
    monkeypatch.setenv("ASR_CHUNK_PACK_TARGET_PADDING_FRAMES", "1")
    monkeypatch.setenv("ASR_CHUNK_PACK_GAP_MERGE_FRAMES", "10")
    monkeypatch.setenv("ASR_PRE_ASR_DROP_GAP_SPLIT_ENABLED", "1")
    monkeypatch.setenv("ASR_PRE_ASR_DROP_GAP_SPLIT_MIN_PARENT_FRAMES", "8")
    monkeypatch.setenv("ASR_PRE_ASR_DROP_GAP_SPLIT_MIN_GAP_FRAMES", "2")
    monkeypatch.setenv("ASR_PRE_ASR_DROP_GAP_SPLIT_MIN_GAP_S", "0.0")
    ...
    backend = _DropGapScoredVadBackend()
    import vad
    ...
    assert len(spans) == 2
    assert all(isinstance(span, PackedChunk) for span in spans)
    assert {span.split_policy for span in spans} == {"r21_pre_asr_drop_gap_v1"}
    assert {span.drop_gap_score_max for span in spans} == {0.90}
    assert asr._LAST_VAD_SIGNATURE["chunk_packing"]["pre_asr_drop_gap_split"]["enabled"] is True
    cached_files = list((tmp_path / "vad-cache").glob("*.json"))
    assert len(cached_files) == 1
    payload = cached_files[0].read_text(encoding="utf-8")
    assert "frame_scores" not in payload
