...
def test_analyze_cut_splits_simulates_endpoint_cut_and_risk_stats(tmp_path):
    ...
    rows = [
        json.loads(line)
        for line in (output_dir / "cut_split_plan.jsonl").read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    ...
    assert rows[0]["cut_split"] is True
    assert rows[0]["children"][0]["split_policy"] == "r17_pre_asr_cut_v1"
    assert rows[1]["cut_split"] is False
