...
def test_concrete_acoustic_evidence_still_wins(flags: list[str]) -> None:
    """A flag naming an actual sound must keep typing the span non_vocal.

    `non_speech_noise` is the case that makes ordering matter: discarding it
    for containing `non_speech` would throw away the word `noise`.
    """
