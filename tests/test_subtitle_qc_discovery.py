...
def test_review_content_candidate_label_uses_positive_allowlist() -> None:
    assert review_content.candidate_label_from_path("REAL-988", Path("REAL-988.fusion_lite.srt")) == "fusion_lite"
    assert (
        review_content.candidate_label_from_path("REAL-988", Path("REAL-988.whisper_ja_1_5b_fusion_lite.srt"))
        == "fusion_lite"
    )
    assert (
        review_content.candidate_label_from_path("REAL-988", Path("REAL-988.whisperseg-adaptive.bilingual.json"))
        == "whisperseg_adaptive"
    )

    with pytest.raises(SystemExit, match="fusion_lite_sigmoid"):
        review_content.candidate_label_from_path("REAL-988", Path("REAL-988.fusion_lite_sigmoid.srt"))
    with pytest.raises(SystemExit, match="silero"):
        review_content.candidate_label_from_path("REAL-988", Path("REAL-988.silero.srt"))


def test_review_content_auto_discovery_errors_on_unknown_vad_artifact(tmp_path: Path) -> None:
    video = tmp_path / "REAL-988.mp4"
    artifact_dir = tmp_path / "REAL-988"
    artifact_dir.mkdir()
    (artifact_dir / "REAL-988.fusion_lite.srt").write_text("", encoding="utf-8")
    (artifact_dir / "REAL-988.fusion_lite_sigmoid.srt").write_text("", encoding="utf-8")
...
def test_review_content_explicit_candidate_allows_trial_labels(tmp_path: Path) -> None:
    path = tmp_path / "REAL-988.lite_gate08.srt"
    path.write_text("", encoding="utf-8")

    spec = review_content.parse_candidate_spec(f"lite_gate08={path}", "REAL-988", set())
...
def test_generate_discover_modes_normalizes_requested_modes() -> None:
    assert generate.discover_modes("REAL-988", ["fusion-lite", "whisperseg-adaptive", "fusion_lite"]) == [
        "fusion_lite",
        "whisperseg_adaptive",
    ]

    with pytest.raises(SystemExit, match="fusion_lite_sigmoid"):
        generate.discover_modes("REAL-988", ["fusion_lite_sigmoid"])


def test_generate_discover_modes_uses_allowlist_for_prefixed_outputs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    video_dir = tmp_path / "video" / "REAL-988"
    video_dir.mkdir(parents=True)
    (video_dir / "REAL-988.whisper_ja_1_5b_fusion_lite.bilingual.json").write_text("{}", encoding="utf-8")
    (video_dir / "REAL-988.whisper_ja_1_5b_whisperseg_adaptive.bilingual.json").write_text("{}", encoding="utf-8")

    monkeypatch.setattr(generate, "PROJECT_ROOT", tmp_path)

    assert generate.discover_modes("REAL-988", None) == [
...
def test_generate_discover_modes_errors_on_unknown_auto_artifact(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    video_dir = tmp_path / "video" / "REAL-988"
    video_dir.mkdir(parents=True)
    (video_dir / "REAL-988.fusion_lite_sigmoid.bilingual.json").write_text("{}", encoding="utf-8")

    monkeypatch.setattr(generate, "PROJECT_ROOT", tmp_path)

    with pytest.raises(SystemExit, match="fusion_lite_sigmoid"):
        generate.discover_modes("REAL-988", None)
