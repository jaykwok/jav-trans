...
def test_audit_index_lists_latest_without_auto_refresh(tmp_path: Path):
    ...
    latest_html = _write_text(
        audit_root / "fusionvad-ja" / "latest-review" / "index.html",
        "<!doctype html><title>latest</title>",
    )
    old_html = _write_text(
        audit_root / "fusionvad-ja" / "old-review" / "index.html",
    ...
    assert 'href="fusionvad-ja/latest-review/index.html"' in index
    assert 'href="fusionvad-ja/old-review/index.html"' in index
...
def test_latest_audit_entry_is_static_link(tmp_path: Path):
    ...
    assert 'href="fusionvad-ja/latest-review/index.html"' in entry
