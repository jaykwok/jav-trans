...
| T-AR | 新增 `fusion_lite` VAD 实验后端，只保留 `whisperseg` 自适应阈值与 `fusion_lite` 两条路线；字幕默认硬上限改为 7s | 匿名样片 A 前 5 分钟：WhisperSeg 14 字幕/11 drops；fusion_lite 15 字幕/7 drops；匿名样片 F/匿名样片 J 全片 VAD 对比 generation overflow/error 均为 0 |
