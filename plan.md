...
| T-AR | 新增 `fusion_lite` VAD 实验后端，只保留 `whisperseg-adaptive` 与 `fusion_lite` 两条公开路线；ffmpeg 仅保留为内部灾难恢复 fallback；字幕默认软目标/硬上限收紧为 5.5s/6.5s | 匿名样片 A 前 5 分钟：WhisperSeg 14 字幕/11 drops；fusion_lite 15 字幕/7 drops；匿名样片 F/匿名样片 J 全片 VAD 对比 generation overflow/error 均为 0；字幕定向 23 passed，全量 383 passed, 5 skipped |
