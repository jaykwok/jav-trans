...
- **AC-5** ✅（匿名样片 J PoC 完成）：WSL2 上对 匿名样片 J 完成 baseline vs PoC 双跑；顺带发现并修复预存 bug `_run_pyin ParameterError` → commit `6cacbdd7`；结论见下方 PoC 报告。
- **AC-6** ✅（匿名样片 F PoC 完成，建议切默认）：WSL2 上对 匿名样片 F（真实男女密集对话，76 分钟）完成 baseline vs PoC 双跑；结论见下方报告。
...
#### T-AC PoC 报告（匿名样片 J，anime-whisper，WSL2 RTX 4060 Ti）
...
**决策：暂缓切默认。** 结果积极但不决定性：gender split 改善，但 None 比例上升，且混合段反而增加（+75%）；匿名样片 F PoC 见下节。

---

#### T-AC PoC 报告（匿名样片 F，anime-whisper，WSL2 RTX 4060 Ti）
...
**正向**：gender split +69%（16→27）；混合段 **-60%**（5→2）——与 匿名样片 J 混合段上升相反，匿名样片 F 真实男女对话场景下 chunk packing 有效减少了混合段。

**待观察**：None 段比例上升（21%→34%），主要来自呻吟/过渡片段（F0 本身无法可靠分类）；max_dur 16.18s 超 8s 阈值，soft_split 会处理；耗时轻微上升 +7%（无 ASR 块数缩减效果，因 匿名样片 F baseline VAD 分布不同）。

**综合两轮 PoC 决策：建议切默认前先处理 None 段比例上升问题，或直接切默认并观察实际产出字幕质量。** 匿名样片 F 混合段改善信号明确；Windows GPU 生产环境下尚未验证仍是主要风险。
...
**背景**：T-AC 匿名样片 F PoC 结果明确（gender split +69%、混合段 -60%），切默认。同步修 ASR overflow bug：`_build_initial_prompt_for_chunk` 无 token 上限，导致 ~34% 的 chunk 触发 `decoder_input_ids exceeds max_target_positions` 后静默返回 `text=""`。
...
### 匿名样片 K 基准测试 ✅（T-AA 完成后）

运行于 2026-05-15，commit 6500fc8。报告：`agents/temp/sample-k-baseline-report.md`
