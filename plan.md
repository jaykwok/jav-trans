...
| T-AR | 新增 `fusion_lite` VAD 实验后端，只保留 `whisperseg` 自适应阈值与 `fusion_lite` 两条路线；ffmpeg 仅保留为内部灾难恢复 fallback；字幕默认软目标/硬上限收紧为 5.5s/6.5s | NAMH-055 前 5 分钟：WhisperSeg 14 字幕/11 drops；fusion_lite 15 字幕/7 drops；HAME-052/NMSL-036 全片 VAD 对比 generation overflow/error 均为 0；字幕定向 23 passed，全量 383 passed, 5 skipped |
