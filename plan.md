...
- 匿名样片 F / 匿名样片 J 全片三模式历史对比显示 fusion_lite 输出接近 WhisperSeg，但 drops 少于 WhisperSeg；逐句报告：`reports/匿名样片 F_匿名样片 J.full_vad_modes_line_compare.html`。
...
- 证据：`agents/temp/t-ar-fusion-lite-sample-a/summary.json`、`agents/temp/t-ar-fusion-lite-sample-a-asr/summary.json`、`agents/temp/t-as-full-vad-compare/summary.json`、`reports/匿名样片 A.5min.vad_modes_fusion_lite_line_compare.html`。
...
- 匿名样片 K 使用 `whisper-ja-anime-v0.3`、跳过翻译进行 VAD/ASR 对比。CUDA ONNX 在 sandbox 内失败，原因是 GPU 被操作系统/sandbox 阻断；外部执行确认 RTX 4060 Ti、Torch CUDA 和 ONNXRuntime CUDA provider 可用。
- 匿名样片 K 汇总：`whisperseg-adaptive` 700 SRT / 210 ASR drops；`fusion_lite` 683 SRT / 167 drops；`fusion_lite_boost` 688 SRT / 185 drops；`fusion_lite_sigmoid` 677 SRT / 148 drops。逐句对齐以 `whisperseg-adaptive` 为基准，`fusion_lite_boost` 最接近：SAME 564、MISSING 41、平均相似度 0.9560。
- 逐句报告产物：`agents/temp/sample-k-vad-variant-compare/sample-k_sentence_report.html`、`sample-k_sentence_report.md`、`sample-k_sentence_diffs_only.md`、`sample-k_sentence_report.csv`、`sample-k_sentence_report_summary.json`。
...
- 匿名样片 F 四模式完整双语对比已输出到 `video/匿名样片 F.*.srt`，逐句 HTML 报告为 `video/匿名样片 F.vad_bilingual_compare.html`；四模式 ASR generation error / overflow / timeout 均为 0，最终 SRT 未包含可见 `[M]` / `[F]` 性别标签。
- 匿名样片 F 汇总：`whisperseg_adaptive` 230 SRT / 96 ASR drops；`fusion_lite` 231 SRT / 63 drops；`fusion_lite_boost` 230 SRT / 84 drops；`fusion_lite_sigmoid` 230 SRT / 50 drops。逐句报告以 `whisperseg_adaptive` 为基准，`fusion_lite` 平均相似度最高。
...
- 匿名样片 F 离线验证：`00:02:56,839 --> 00:02:59,160` 合并为 `アルマリスト 室で イラックス ステイマンを受けて`，避免 `受け` / `受けて` 被拆成两条造成语义断裂。
...
### 匿名样片 F 四后端 skip-translation 对比
...
默认全量翻译（anime-whisper + bilingual）：`pipeline_total=575.30s`，字幕块数 150，产物 `video/匿名样片 F.srt`。
