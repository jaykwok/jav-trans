...
class IslandCandidateSequenceNetwork:
    ...
    Encodes every candidate with the same per-candidate stack as v1
    (``frame_proj`` -> bidirectional Mamba2 over bins -> mean pool + scalar arm)
    so a v1 checkpoint can warm-start those submodules, then runs a second
        ...
        class _Network(nn.Module):
            def __init__(self) -> None:
                ...
                if self.ptm_input_dim:
                    ...
                    # Identity-slice init: step-0 output equals the incumbent
                    # ptm[:, :ptm_projected_dim] truncation, so v1 warm-started
                    # encoders see unchanged inputs before training moves W.
                    with torch.no_grad():
                ...
                self.frame_proj = nn.Linear(frame_dim, hidden_size)
                # Zero-initialized so a v1 warm-started encoder sees unchanged
                # inputs at step 0; learned later to mark left/gap/right bins
...
class SplitDecision:
    ...
    p_continue: float
    p_unsure: float
    # v2 island verifier extras; v1 leaves the defaults.
    role: str = ""
    p_role: float = 0.0
    offset_s: float = 0.0
...
) -> "SemanticSplitVerifier | SemanticSplitIslandVerifier":
    import torch

    checkpoint_path = Path(path)
    payload = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    schema = payload.get("schema")
    if schema == SEMANTIC_SPLIT_V2_SCHEMA:
