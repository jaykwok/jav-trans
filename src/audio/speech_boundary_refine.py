...
from boundary.base import SpeechBoundaryBackend


def refine_chunks_via_speech_boundary(
    chunks: list[dict],
    *,
    boundary_backend: SpeechBoundaryBackend,
    threshold_override: float | None = None,
    timeout_per_chunk_s: float = 30.0,
) -> list[dict]:
    """Speech-boundary subdivide QC-failed chunks for re-transcription.

    Each returned dict preserves the original keys plus a '_speech_parent_index'
    int so the caller can group sub-chunks back to the original position and
    merge text results.

    On boundary backend returning <=1 segment or timeout/exception the original chunk is
    returned unchanged (with '_speech_parent_index' added).  threshold_override is
    passed through to boundary backends that support per-call threshold overrides.
    """
    result: list[dict] = []
    ...
    try:
        for chunk in chunks:
            ...
            try:
                ...
                executor = ThreadPoolExecutor(max_workers=1)
                result.append({**chunk, "_speech_parent_index": original_index})
                continue
            except Exception:
                result.append({**chunk, "_speech_parent_index": original_index})
                continue
...
def _slice_wav_at_segments(chunk: dict, segments) -> list[dict]:
    """Slice a chunk WAV into sub-WAVs at speech segment boundaries."""
        ...
        sub_chunks.append(
            {
                ...
                "_speech_parent_index": original_index,
            }
        )
