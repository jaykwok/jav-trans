#!/usr/bin/env python3
"""Export current Pre-ASR CueQC v9 candidates for audio audit/labeling."""
...
def export_candidates(*, timing_paths: list[str], output_dir: Path) -> dict[str, Any]:
            ...
            rows.append(
                {
                    **item,
                    ...
                    "cluster_method": "pre_asr_v9_feature_bucket",
    ...
    clusters_path = output_dir / "pre_asr_cueqc_v9_audit_candidates.jsonl"
    summaries_path = output_dir / "pre_asr_cueqc_v9_audit_summaries.jsonl"
    summary_path = output_dir / "summary.json"
    write_jsonl(clusters_path, rows)
    write_jsonl(summaries_path, summaries)
    summary = {
        "schema": "pre_asr_cueqc_v9_audit_candidate_export_summary",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export current Pre-ASR CueQC v9 candidates for audit.")
