...
from tools.asr.cueqc.compile_pre_asr_v5_features import FEATURE_BUNDLE_SCHEMA, project_path, repo_display_path  # noqa: E402


METRICS_SCHEMA = "cueqc_pre_asr_mamba_v5_train_metrics"


def default_checkpoint_name(asr_repo_id: str) -> str:
    return f"cueqc_pre_asr_mamba_v5_binary.{qwen_asr_repo_tag(asr_repo_id)}.pt"
...
def load_feature_bundle(path: Path) -> dict[str, Any]:
    ...
    if tuple(payload.get("feature_names") or ()) != PRE_ASR_CUEQC_FEATURE_NAMES:
        raise ValueError("feature bundle feature_names do not match Pre-ASR CueQC v5 runtime")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Pre-ASR CueQC v5 binary checkpoint.")
