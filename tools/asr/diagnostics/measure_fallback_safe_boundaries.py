        ...
        chunks.append(
            {
                ...
                "core_offset_left_s": max(0.0, core_start - start),
                "core_offset_right_s": max(0.0, end - core_end),
            }
        )
    return chunks
...
def classify_row(
    ...
    *,
    ...
    return {
        ...
        "core_offset_left_s": round(chunk["core_offset_left_s"], 6),
        "core_offset_right_s": round(chunk["core_offset_right_s"], 6),
...
def summarize(rows: list[dict[str, Any]], *, target_duration_s: float) -> dict[str, Any]:
    ...
    return {
        ...
        "fallback_chunk_duration_s": stats([row["duration_s"] for row in fallback_rows]),
...
def build_markdown(summary: dict[str, Any], *, boundary_cache: Path, diagnostics: Path, top_rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        f"- fallback source chunk duration p50/p90/max: `{summary['fallback_chunk_duration_s']['p50']:.3f}` / `{summary['fallback_chunk_duration_s']['p90']:.3f}` / `{summary['fallback_chunk_duration_s']['max']:.3f}`",
