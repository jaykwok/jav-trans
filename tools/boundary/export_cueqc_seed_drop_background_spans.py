...
def discover_latest_seed_candidates() -> Path:
    paths = sorted(
        (PROJECT_ROOT / "agents" / "temp").glob(
            "*_scorer-v7-native-realneg-candidates-from-cluster-seed/cueqc_confirmed_drop_candidates.jsonl"
        )
    )
    if not paths:
        paths = sorted(
    ...
    summary = {
        ...
        "speech_boundary_hard_negative": {
            ...
            "next_conversion": [
                ...
                "pass speech_boundary_negative_manifest.json to tools.boundary.ja.build_scorer_v7_native_dataset",
            ],
        },
    }
