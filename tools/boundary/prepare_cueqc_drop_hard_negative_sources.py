...
ENRICHED_SCHEMA = "cueqc_drop_hard_negative_source_candidate_v1"
SUMMARY_SCHEMA = "cueqc_drop_hard_negative_source_prep_summary_v1"
...
def discover_latest_candidates() -> Path:
    paths = sorted(
        (PROJECT_ROOT / "agents" / "temp").glob(
            "*_speech-boundary-hard-negative-candidates-from-cueqc/cueqc_confirmed_drop_candidates.jsonl"
        )
    )
    if not paths:
        raise FileNotFoundError(
            "no CueQC hard-negative candidate manifest found under "
            "agents/temp/*_speech-boundary-hard-negative-candidates-from-cueqc"
        )
    return paths[-1]
...
def enrich_candidate(candidate: Mapping[str, Any], audit_item: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    enriched = {
        **dict(candidate),
        ...
        "hard_negative_source_status": "source_prepared_candidate",
    }
    return enriched
...
def prepare_cueqc_drop_hard_negative_sources(
    *,
    ...
    unsupported_routes = sorted(
        {
            ...
            for row in enriched
            if str(row.get("candidate_route") or "") != FRAME_NEGATIVE_ROUTE
        }
    )
    if unsupported_routes:
        raise ValueError(
            "CueQC drop source prep only accepts speech_boundary_frame_negative_candidate. "
            f"Regenerate candidates with the current exporter. Unsupported routes: {unsupported_routes}"
        )
    frame_negative_rows = list(enriched)
    ...
    summary = {
        ...
        "speech_boundary_hard_negative": {
            ...
            "next_steps": [
                "use speech_boundary_negative_labels/manifest as a hard-negative source in a guarded SpeechBoundary-JA replay stage",
            ],
        },
    }
...
def render_markdown(summary: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# CueQC Drop Hard-Negative Source Prep",
        "",
    ...
    lines.extend(
        [
            "",
            "## SpeechBoundary Hard-Negative Boundary",
            "",
            "- This prep emits SpeechBoundary-JA negative labels only.",
            "- It does not emit Boundary Refiner training labels.",
            "- Manual CueQC `drop_ok` labels are direct discard supervision.",
            "",
        ]
    )
    return "\n".join(lines)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "SpeechBoundary-JA negative labels only."
        )
    )
    parser.add_argument(
        "--candidates",
        default="",
        help=(
            "cueqc_confirmed_drop_candidates.jsonl. Defaults to the latest "
            "agents/temp/*_speech-boundary-hard-negative-candidates-from-cueqc manifest."
        ),
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="",
        help="Defaults to agents/temp/YYYYMMDD_HHMMSS_speech-boundary-hard-negative-sources-from-cueqc.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = (
        ...
        / "agents"
        / "temp"
        / f"{local_timestamp()}_speech-boundary-hard-negative-sources-from-cueqc"
    )
    ...
    print(
        "frame_negative={materialized}/{total} training_examples={examples}".format(
