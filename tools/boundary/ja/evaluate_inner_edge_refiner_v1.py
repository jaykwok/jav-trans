...
def run(args: argparse.Namespace) -> None:
    ...
    val_rows = [row for row in _rows(manifest_path) if row.get("partition") != "train"]
    ...
    for row in val_rows:
        ...
        prediction = model.predict_subislands(
    ...
    metrics = {
        "schema": "inner_edge_refiner_v1_directional_metrics_v1",
        "checkpoint": str(Path(args.checkpoint)),
        "validation_count": len(evaluated),
        "known_clipping_count": sum(
            float(row["start_inward_s"]) > 0.0 or float(row["end_inward_s"]) > 0.0
        ...
        **{
        ...
        "within_tolerance": {
            f"{int(tolerance * 1000)}ms": {
                "start": sum(float(row["start_absolute_s"]) <= tolerance for row in evaluated)
                / len(evaluated),
                "end": sum(float(row["end_absolute_s"]) <= tolerance for row in evaluated)
                / len(evaluated),
            }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate directional Inner Edge Refiner v1 boundary errors."
    )
