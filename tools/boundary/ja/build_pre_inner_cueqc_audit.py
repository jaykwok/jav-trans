...
def validate_partition(rows: list[dict[str, Any]]) -> None:
    ...
    for sample_id, group in by_source.items():
        ...
        if float(ordered[0]["start_s"]) != 0.0:
            raise ValueError(f"{sample_id}: provisional partition must start at zero")
        for left, right in zip(ordered, ordered[1:]):
            if abs(float(left["end_s"]) - float(right["start_s"])) > 1e-6:
                raise ValueError(f"{sample_id}: provisional sub-islands must be contiguous")
