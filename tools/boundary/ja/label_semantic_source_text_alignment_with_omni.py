...
def build_prompt(sample: dict[str, Any], *, validation_feedback: str = "") -> str:
    payload = {
        ...
        "membership_contract": "derived_matched_semantic_envelope_v1",
    }
...
def validate_response(
    ...
    unsure_alignments = [row for row in alignments if row["status"] == "unsure"]
    if unsure_alignments:
        keep_span = {
            "status": "unsure",
            "start_s": None,
            "end_s": None,
            ...
            "confidence": min(float(row["confidence"]) for row in unsure_alignments),
            "reason": "存在 unsure semantic alignment，membership 整体 abstain。",
        }
    elif matched_alignments:
        keep_span = {
            "status": "matched",
            "start_s": float(matched_alignments[0]["start_s"]),
            "end_s": float(matched_alignments[-1]["end_s"]),
            "derivation": "matched_semantic_envelope_v1",
            "bridged_internal_gap_count": sum(
                float(right["start_s"]) > float(left["end_s"])
                for left, right in zip(matched_alignments, matched_alignments[1:])
            ),
            "confidence": min(float(row["confidence"]) for row in matched_alignments),
            "reason": (
                "由 matched semantic timeline 的最早 start 与最晚 end 确定性派生；"
                "只桥接内部空洞，不吸收两侧非语义声音。"
            ),
        }
    else:
        keep_span = {
            ...
            "bridged_internal_gap_count": 0,
            "confidence": 1.0,
            "reason": "没有 matched semantic span，确定性派生为 none。",
        }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Label semantic text units and audio alignment in one Omni request; derive membership from matched semantic spans."
    )
