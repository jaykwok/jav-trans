...
def build_page(
    *,
    ...
    page_title = (
        "Inner v1 held-out hard-case audit"
        if formal_inner_mode
        else (
    ...
    help_html = (
        "<h2>正式独立 Inner v1 · held-out worst-5</h2>"
        "<p>这些样本来自未参与梯度训练的 val/test partition，覆盖最大 start/end inward、最大 outward 与 model abstain。只按听感比较 raw 与 refined：完整台词不得少，能去掉的边缘喘息、亲吻、哭声、音乐或环境噪声不得明显残留。</p>"
        "<p>构造 core 区间和方向误差只用于选择 hard case，不显示给审核者，也不作为运行时阈值。</p>"
        if formal_inner_mode
        else (
    ...
    storage_key = (
        "inner-v1-heldout-hardcase-audit-v1"
        if formal_inner_mode
        else (
            "inner-noisy-edge-audit-v1"
            if noisy_edge_mode
            else "inner-subisland-edge-audit-v1"
        )
    )
