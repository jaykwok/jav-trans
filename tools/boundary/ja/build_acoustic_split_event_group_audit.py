...
</style></head><body><header><strong>Acoustic Split v3 · 相邻候选是否属于同一停顿</strong> <button id="save">保存分组</button> <span id="status"></span></header><main><section class="help"><h2>每一行只判断：两个时间点之间有没有恢复真实 speech？</h2><p><b>中间没有真实 speech：</b>两个候选仍在同一整段静音/杂音里，选择“同一 gap”，最终只形成一个 event。<b>中间有真实 speech：</b>前一个停顿后已经重新说话，随后又出现另一个停顿，选择“两个 gap”。<b>听不清：</b>选择“不确定”，该 run 暂不进入训练。</p><p>例如 5 个连续 split 候选会出现 4 行相邻关系，所以页面看起来是成对的；这不是重复审核样本。被 continue 隔开的 runs 已自动分开，singleton 自动成为 event。本页不重审候选是否 split，也不审 CueQC 或 Inner。</p></section><div id="list"></div></main><script>
