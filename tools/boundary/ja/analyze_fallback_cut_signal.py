    ...
    *,
    ...
    if any("fallback_safe" in row or "fallback_reason" in row for row in raw_rows):
        rows = [
            row
            for row in raw_rows
            if str(row.get("fallback_reason") or "").strip()
            and not bool(row.get("fallback_safe"))
        ]
    else:
        if any("fallback_type" not in row for row in raw_rows):
            raise ValueError("diagnostics input rows must include fallback_type")
        rows = [
            row
            for row in raw_rows
            if str(row.get("fallback_type") or "") not in {"", "none"}
        ]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Audit whether existing SpeechBoundary-JA speech/cut frame scores contain "
    ...
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_ROOT / "agents" / "temp" / "speech-boundary-ja" / "fallback-cut-signal-analysis"),
    )
