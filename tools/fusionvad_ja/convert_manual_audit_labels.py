...
from vad.fusionvad_ja import build_negative_record, build_supervised_record, write_jsonl
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert manual audit JSONL into FusionVAD-JA labels.")
    parser.add_argument("--input", required=True, help="manual_labels.jsonl exported from the audit HTML.")
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_ROOT / "datasets" / "val" / "fusionvad-ja" / "v1-3" / "manual-audit-galgame" / "strong-labels"),
    )
