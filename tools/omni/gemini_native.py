...
GEMINI_QUOTA_STATE_SCHEMA = "gemini_native_quota_state_v2"
...
class GeminiNativeAudioClient:
    """Stateful native client with sticky keys and 429-only rotation."""
    ...
    def __init__(
        ...
        *,
        ...
        self._last_started = [0.0 for _ in keys]
        self._blocked_until = [0.0 for _ in keys]
        self._token_windows: list[deque[tuple[float, int]]] = [
            deque() for _ in keys
        ]
        for index in range(len(keys)):
            if self._daily_available(index):
    ...
    def _empty_daily_state(self) -> dict[str, Any]:
        return {
            ...
            "pacific_date": pacific_quota_date(self._now_utc()),
            "daily_request_limit": self.daily_request_limit,
            "keys": {
                fingerprint: {"requests_started": 0, "exhausted_by_429": False}
    ...
    def _load_daily_state(self) -> dict[str, Any]:
        ...
        if (
            ...
            or saved.get("schema") != GEMINI_QUOTA_STATE_SCHEMA
            or saved.get("pacific_date") != fresh["pacific_date"]
        ):
            return fresh
        keys = saved.get("keys")
        ...
        for fingerprint in self._key_fingerprints:
            ...
            if not isinstance(item, Mapping):
                continue
            fresh["keys"][fingerprint] = {
                "requests_started": max(0, int(item.get("requests_started") or 0)),
    ...
    def _save_daily_state(self) -> None:
        if self.quota_state_path is None:
            return
        self._daily_state["updated_at_utc"] = self._now_utc().astimezone(
            timezone.utc
        ).isoformat()
    ...
    def _refresh_daily_state(self) -> None:
        ...
        if self._daily_state.get("pacific_date") == current_date:
            return
        self._daily_state = self._empty_daily_state()
    ...
    def _record_daily_request_start(self, key_index: int) -> int:
        ...
        self._save_daily_state()
        return int(item["requests_started"])

    def _mark_daily_exhausted(self, key_index: int) -> None:
        self._daily_item(key_index)["exhausted_by_429"] = True
        self._save_daily_state()

    def _wait_for_key_quota(self, key_index: int) -> None:
        now = time.monotonic()
        token_window = self._token_windows[key_index]
        while token_window and now - token_window[0][0] >= 60.0:
            token_window.popleft()
        wait_s = max(
            0.0,
            self._last_started[key_index] + self.min_request_interval_s - now,
            self._blocked_until[key_index] - now,
        )
        if sum(tokens for _stamp, tokens in token_window) >= GEMINI_NATIVE_TPM_PER_KEY:
            wait_s = max(wait_s, 60.0 - (now - token_window[0][0]))
        if wait_s > 0:
            self.log(
    ...
    def _record_usage(self, key_index: int, payload: Mapping[str, Any]) -> None:
        ...
        if total_tokens > 0:
            self._token_windows[key_index].append((time.monotonic(), total_tokens))
    ...
    def call_json(
        ...
        *,
        ...
        while len(attempted_slots) < len(self.api_keys):
            ...
            if key_index in attempted_slots or not self._daily_available(key_index):
                candidates = [
                    index
                    for index in range(len(self.api_keys))
                    if index not in attempted_slots and self._daily_available(index)
                ]
                if not candidates:
                    raise GeminiNativeError(
                        "all native Gemini key slots reached the "
                        f"{self.daily_request_limit} RPD budget for the current "
                        "Pacific quota day",
                        status_code=429,
                    )
            ...
            self._last_started[key_index] = time.monotonic()
            daily_requests = self._record_daily_request_start(key_index)
            try:
                with httpx.Client(
            ...
            if response.status_code == 429:
                ...
                self._blocked_until[key_index] = time.monotonic() + retry_s
                if _is_daily_quota_error(response):
                    self._mark_daily_exhausted(key_index)
                if len(attempted_slots) < len(self.api_keys):
                    candidates = [
                        index
                        for index in range(len(self.api_keys))
                        if index not in attempted_slots and self._daily_available(index)
                    ...
                    if not candidates:
                        raise GeminiNativeError(
                            "all native Gemini API key slots returned HTTP 429 "
                            "or reached their daily budget",
                            status_code=429,
                        )
                    next_index = candidates[0]
                    self.log(
                        f"gemini_key_rotate reason=http_429 "
                        f"from_slot={key_index + 1}/{len(self.api_keys)} "
                        f"to_slot={next_index + 1}/{len(self.api_keys)}"
                    )
            ...
            try:
                ...
                raise GeminiNativeError("native Gemini returned invalid JSON") from error
            if not isinstance(payload, Mapping):
                raise GeminiNativeError("native Gemini response must be an object")
            ...
            raw = {
                ...
                "pacific_quota_date": self._daily_state["pacific_date"],
                "status": status,
                "usage": dict(usage),
