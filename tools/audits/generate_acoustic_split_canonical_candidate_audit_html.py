...
<title>Acoustic Split Canonical Candidate Audit</title>
<style>
body{{margin:0;font-family:Segoe UI,Arial,sans-serif;background:#f5f6f8;color:#20242a}}header{{position:sticky;top:0;z-index:2;background:#fff;border-bottom:1px solid #d9dde3;padding:12px 20px}}h1{{font-size:20px;margin:0 0 8px}}.toolbar{{display:flex;gap:8px;align-items:center;flex-wrap:wrap}}button,select,input{{font:inherit}}button{{border:1px solid #b9c0c9;background:#fff;padding:7px 11px;border-radius:4px;cursor:pointer}}button.active{{background:#1769aa;color:#fff;border-color:#1769aa}}button.cut.active{{background:#b42318}}button.continue.active{{background:#197642}}button.unsure.active{{background:#8a5a00}}main{{max-width:1080px;margin:18px auto;padding:0 16px}}article{{background:#fff;border:1px solid #d9dde3;border-radius:6px;padding:14px;margin-bottom:12px}}article.done{{border-left:5px solid #197642}}.meta{{display:flex;gap:10px;flex-wrap:wrap;font-size:13px;color:#59616c}}.labels,.listen-controls{{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}}.pill{{background:#eef1f4;padding:3px 7px;border-radius:3px}}audio{{width:100%;margin:8px 0}}.candidate-ruler{{height:18px;position:relative;background:linear-gradient(90deg,#dce8f2 0 50%,#f2e2dc 50% 100%);border:1px solid #b9c0c9;border-radius:3px;overflow:hidden}}.candidate-marker{{position:absolute;top:0;bottom:0;width:3px;background:#b42318;transform:translateX(-1px)}}.candidate-label{{font-size:12px;color:#59616c;margin:3px 0 8px}}.verdict{{display:flex;gap:8px;align-items:center;flex-wrap:wrap}}.note{{flex:1;min-width:240px;padding:7px;border:1px solid #b9c0c9;border-radius:4px}}pre{{white-space:pre-wrap;background:#f6f7f9;padding:8px;border-radius:4px;font-size:12px}}#status{{font-size:13px;color:#59616c}}
</style></head><body><header><h1>Acoustic Split Canonical Candidate Audit</h1><div class="toolbar"><button id="save">保存裁决</button><select id="filter"><option value="all">全部</option><option value="todo">未审</option><option value="disagree">模型改判</option></select><span id="status"></span></div></header><main id="list"></main>
<script>
const rows={payload}; const key='acoustic-split-canonical-candidate-audit-v1'; const ann=JSON.parse(localStorage.getItem(key)||'{{}}'); let activeAudio=null;
function esc(s){{return String(s??'').replace(/[&<>"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[c]));}}
function playRange(audio,start,end){{if(activeAudio&&activeAudio!==audio)activeAudio.pause();activeAudio=audio;audio.currentTime=Math.max(0,start);const stop=()=>{{if(audio.currentTime>=end){{audio.pause();audio.removeEventListener('timeupdate',stop);}}}};audio.addEventListener('timeupdate',stop);audio.play();}}
...
const duration=Math.max(0.001,Number(row.request_clip_end_s)-Number(row.request_clip_start_s));const point=Number(row.candidate_offset_s);const marker=Math.max(0,Math.min(100,point/duration*100));const right=Math.max(0,duration-point);
const article=document.createElement('article');article.dataset.id=row.candidate_id;if(a.verdict)article.classList.add('done');article.innerHTML=`<strong>${{esc(row.candidate_id)}}</strong><div class="meta"><span>${{esc(row.partition)}}</span><span>原窗 t=${{Number(row.time_s).toFixed(3)}}s</span><span>左 ${{point.toFixed(2)}}s / 右 ${{right.toFixed(2)}}s</span><span>confidence=${{Number(row.confidence).toFixed(2)}}</span><span>${{esc((row.hard_case_categories||[]).join(', '))}}</span></div><div class="labels"><span class="pill">current: ${{esc(row.current_label)}}</span><span class="pill">legacy: ${{esc(row.legacy_label||'-')}}</span><span class="pill">Plus: ${{esc(row.label)}}</span><span class="pill">raw: ${{esc(row.omni_label)}}</span></div><audio controls preload="none" src="${{esc(row.audio)}}"></audio><div class="candidate-ruler"><span class="candidate-marker" style="left:${{marker}}%"></span></div><div class="candidate-label">红线 = 唯一候选切点（音频内 ${{point.toFixed(2)}}s）</div><div class="listen-controls"><button class="play-left">只听切点前</button><button class="play-cross">跨点听 ±1s</button><button class="play-right">只听切点后</button></div><div class="verdict"><button class="cut" data-v="cut">cut</button><button class="continue" data-v="continue">continue</button><button class="unsure" data-v="unsure">unsure</button><input class="note" placeholder="备注" value="${{esc(a.note)}}"></div><pre>${{esc(JSON.stringify({{left_complete:row.left_complete,right_complete:row.right_complete,merged_better:row.merged_better,flags:row.flags,expected_gate_label:row.expected_gate_label,reason:row.reason}},null,2))}}</pre>`;
const audio=article.querySelector('audio');audio.onplay=()=>{{if(activeAudio&&activeAudio!==audio)activeAudio.pause();activeAudio=audio;}};article.querySelector('.play-left').onclick=()=>playRange(audio,0,point);article.querySelector('.play-cross').onclick=()=>playRange(audio,Math.max(0,point-1),Math.min(duration,point+1));article.querySelector('.play-right').onclick=()=>playRange(audio,point,duration);
