...
def build_audit(
    *,
    ...
    request_audio_dir: Path | None = None,
) -> dict[str, Any]:
    selection = {str(row["island_id"]): row for row in _read_jsonl(selected)}
    label_rows = _read_jsonl(labels)
    baseline_rows = _read_jsonl(baseline_labels) if baseline_labels else []
    baseline = {str(row["island_id"]): row for row in baseline_rows}

    def display_name(rows: list[dict[str, Any]], fallback: str) -> str:
        version = str(rows[0].get("prompt_version") or "") if rows else ""
        if version.endswith("complete_v3"):
            return "Omni v3 completeness"
        if version.endswith("cut_eager_v2"):
            return "Omni v2 cut-eager"
        if version.endswith("speech_island_v1"):
    ...
    baseline_name = display_name(baseline_rows, "Omni baseline")
    rows = []
    output_dir.mkdir(parents=True, exist_ok=True)
    for label in label_rows:
        island = selection[str(label["island_id"])]
        source = Path(label["island_id"].replace("#", "__") + ".wav")
        request_audio = (request_audio_dir or labels.parent / "request_audio") / source
        ...
        segmented_audio = output_dir / "audio" / f"{source.stem}__omni-current-segmented.wav"
...
<body><header><strong>Split v3 Speech Island Smoke · 5 samples</strong> <span>裁决对象：{current_name} 红线；其他线仅供对照</span> <button id="save">保存裁决</button> <span id="status"></span></header><main id="list"></main>
<script>const rows={payload};const currentName={json.dumps(current_name, ensure_ascii=False)};const baselineName={json.dumps(baseline_name, ensure_ascii=False)};const key='split-v3-island-smoke-v1';const ann=JSON.parse(localStorage.getItem(key)||'{{}}');let active=null;
...
def main() -> None:
    ...
    parser.add_argument("--request-audio-dir", default="")
    parser.add_argument("--output-dir", required=True)
    args = parser.parse_args()
    print(json.dumps(build_audit(selected=Path(args.selected), labels=Path(args.labels), output_dir=Path(args.output_dir), baseline_labels=Path(args.baseline_labels) if args.baseline_labels else None, request_audio_dir=Path(args.request_audio_dir) if args.request_audio_dir else None), ensure_ascii=False))
