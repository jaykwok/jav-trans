...
def membership_review_required(row: dict) -> bool:
    matched = [item for item in row["semantic_alignments"] if item["status"] == "matched"]
    keep = row["keep_span"]
    if not matched and keep["status"] == "none":
        return False
    if keep["status"] != "matched" or not matched:
        return True
    same_outer_edges = (
        float(keep["start_s"]) == float(matched[0]["start_s"])
        and float(keep["end_s"]) == float(matched[-1]["end_s"])
    )
    return not (same_outer_edges and int(keep.get("bridged_internal_gap_count") or 0) == 0)
        ...
        payload_rows.append(
            {
                **row,
                ...
                "membership_review_required": membership_review_required(row),
            }
        )
...
<div class="step"><h3>3 · membership envelope</h3><p>蓝色 membership span <b>不是 Omni 的额外判断</b>，只允许桥接语义区间之间的内部空洞，绝不向两侧吸收亲吻声、喘息、呻吟、尾音、BGM 或背景噪声。当它与 semantic timeline 完全相同或同为 none 时自动随第 2 项通过，不重复裁决；只有确实桥接多个不连续 semantic spans 时才显示独立 membership 审计。</p></div>
</div><p><b>请求契约：</b>每个样本只上传一次完整音频和一次完整参考文本；Omni 在同一个响应里先拆文本、再对齐音频。此页不显示旧切点或 learned candidates。</p></section><div id="list"></div></main>
<script>
const rows={payload};
const key='semantic-source-text-alignment-audit-v2:'+location.pathname;
const ann=JSON.parse(localStorage.getItem(key)||'{{}}');
let activeAudio=null;
let activeStop=null;
function esc(s){{return String(s??'').replace(/[&<>"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[c]));}}
function ensure(row){{ann[row.sample_id]??={{text_verdict:'',timeline_verdict:'',membership_verdict:'',note:''}};return ann[row.sample_id];}}
function approved(row,a){{return a.text_verdict==='approve'&&a.timeline_verdict==='approve'&&(!row.membership_review_required||a.membership_verdict==='approve');}}
...
function timeline(row){{const keep=row.keep_span;let bars='';if(keep.status==='matched')bars+=`<div class="bar keep" style="left:${{pct(keep.start_s,row.duration_s)}}%;width:${{pct(keep.end_s-keep.start_s,row.duration_s)}}%" title="membership ${{spanLabel(keep.start_s,keep.end_s)}}">membership</div>`;for(const item of row.semantic_alignments){{if(item.status==='matched')bars+=`<div class="bar meaning" style="left:${{pct(item.start_s,row.duration_s)}}%;width:${{pct(item.end_s-item.start_s,row.duration_s)}}%" title="${{esc(item.unit_id)}} ${{spanLabel(item.start_s,item.end_s)}}">${{esc(item.unit_id)}}</div>`;}}for(const item of row.unsure_audio_spans||[])bars+=`<div class="bar uncertain" style="left:${{pct(item.start_s,row.duration_s)}}%;width:${{pct(item.end_s-item.start_s,row.duration_s)}}%" title="unsure ${{spanLabel(item.start_s,item.end_s)}}">?</div>`;return `<div class="track">${{bars}}</div><small>0s　　　　　　　　　　　　　　　　　　　　　　　　　${{Number(row.duration_s).toFixed(3)}}s　（蓝=membership envelope，绿=semantic，黄=unsure）</small>`;}}
function render(){{const root=document.getElementById('list');root.innerHTML='';for(const row of rows){{const a=ensure(row);const card=document.createElement('article');if(approved(row,a))card.classList.add('done');const audioId='audio-'+row.sample_id;const units=row.text_units.map(u=>`<span class="unit ${{esc(u.kind)}}" title="${{esc(u.reason)}}"><b>${{esc(u.unit_id)}}</b> ${{esc(u.text)}} <small>${{esc(u.kind)}} ${{Number(u.confidence).toFixed(2)}}</small></span>`).join('');const alignments=row.semantic_alignments.length?row.semantic_alignments.map(x=>`<tr><td>${{esc(x.unit_id)}}</td><td>${{esc(x.status)}}</td><td>${{x.status==='matched'?spanLabel(x.start_s,x.end_s):'—'}}</td><td>${{x.status==='matched'?`<button data-play-start="${{x.start_s}}" data-play-end="${{x.end_s}}">播放该语义 span</button>`:''}}</td><td>${{Number(x.confidence).toFixed(2)}} · ${{esc(x.reason)}}</td></tr>`).join(''):'<tr><td colspan="5">没有 semantic 文本单元；应重点核对这是否确实是纯非语义样本。</td></tr>';const keep=row.keep_span;const membershipPanel=row.membership_review_required?`<section class="panel"><h4>第 3 项 · membership envelope</h4><p><b>区间：</b>${{spanLabel(keep.start_s,keep.end_s)}} <button data-play-start="${{keep.start_s}}" data-play-end="${{keep.end_s}}">播放 membership envelope</button></p><p><b>桥接内部空洞：</b>${{Number(keep.bridged_internal_gap_count||0)}} 个</p><div class="verdict">${{verdict('membership 通过','membership_verdict','approve',a)}}${{verdict('membership 不通过','membership_verdict','reject',a)}}</div></section>`:`<section class="panel"><h4>第 3 项 · membership 自动去重</h4><p>membership 与 semantic timeline 完全相同（或同为 none），自动随第 2 项，不再重复播放和裁决。</p></section>`;card.innerHTML=`<h2>${{esc(row.sample_id)}}</h2><p><b>参考文本：</b>${{esc(row.reference_text)}}</p><p><b>审计重点：</b>${{esc(row.audit_focus)}}</p><small>来源：${{esc(row.source)}}；时长 ${{Number(row.duration_s).toFixed(3)}}s；一次完整音频 + 完整文本请求</small><p><b>完整原音频（独立播放）</b></p><audio id="${{esc(audioId)}}" controls preload="metadata" src="${{esc(row.audio)}}"></audio>${{timeline(row)}}<section class="panel"><h4>第 1 项 · 文本语义拆分</h4><div class="units">${{units}}</div><div class="verdict">${{verdict('文本拆分通过','text_verdict','approve',a)}}${{verdict('文本拆分不通过','text_verdict','reject',a)}}</div></section><section class="panel"><h4>第 2 项 · semantic timeline</h4><table><thead><tr><th>unit</th><th>状态</th><th>精确区间</th><th>试听</th><th>证据</th></tr></thead><tbody>${{alignments}}</tbody></table><div class="verdict">${{verdict('时间轴通过','timeline_verdict','approve',a)}}${{verdict('时间轴不通过','timeline_verdict','reject',a)}}</div></section>${{membershipPanel}}<label><b>备注</b><textarea placeholder="指出具体文本 unit、时间或 membership envelope 问题">${{esc(a.note)}}</textarea></label>`;card.querySelectorAll('[data-field]').forEach(button=>button.onclick=()=>{{a[button.dataset.field]=button.dataset.value;a.updated_at=new Date().toISOString();persist();render();}});card.querySelectorAll('[data-play-start]').forEach(button=>button.onclick=()=>playSpan(audioId,button.dataset.playStart,button.dataset.playEnd));card.querySelector('textarea').onchange=e=>{{a.note=e.target.value;a.updated_at=new Date().toISOString();persist();}};const audio=card.querySelector('audio');audio.onplay=e=>{{if(activeAudio&&activeAudio!==e.target){{clearStop();activeAudio.pause();}}activeAudio=e.target;}};audio.onpause=()=>clearStop();root.appendChild(card);}}status();}}
document.getElementById('save').onclick=async()=>{{const content=rows.map(row=>{{const a=ensure(row);const membership=row.membership_review_required?(a.membership_verdict||'unreviewed'):'derived_from_timeline';const rejected=a.text_verdict==='reject'||a.timeline_verdict==='reject'||(row.membership_review_required&&a.membership_verdict==='reject');return JSON.stringify({{schema:'semantic_source_text_alignment_manual_verdict_v2',sample_id:row.sample_id,reference_text:row.reference_text,text_units:row.text_units,semantic_alignments:row.semantic_alignments,keep_span:row.keep_span,unsure_audio_spans:row.unsure_audio_spans,membership_review_required:row.membership_review_required,text_verdict:a.text_verdict||'unreviewed',timeline_verdict:a.timeline_verdict||'unreviewed',membership_verdict:membership,verdict:approved(row,a)?'approve':(rejected?'reject':'unreviewed'),note:a.note||'',updated_at:a.updated_at||new Date().toISOString()}});}}).join('\\n')+'\\n';const res=await fetch('/__audit_api__/save-labels',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{href:location.pathname,filename:'manual_verdicts.jsonl',content}})}});const out=await res.json();document.getElementById('status').textContent=out.ok?'已保存到 '+out.path:'保存失败: '+out.error;}};
render();
</script></body></html>"""
