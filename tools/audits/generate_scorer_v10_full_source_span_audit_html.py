...
def _json_for_script(payload: Any) -> str:
    """Encode JSON without characters that can terminate or alter a script tag."""
...
def _render_page(rows: list[dict[str, Any]]) -> str:
    encoded = _json_for_script(rows)
    schema = _json_for_script(MANUAL_VERDICT_SCHEMA)
    return (
        """<!doctype html>
<html lang="zh-CN">
<head>
...
audio{width:100%;margin:7px 0}.track{position:relative;height:48px;margin:9px 0;background:#e1e5e9;overflow:hidden}.span{position:absolute;top:0;bottom:0;min-width:3px;border:0;border-right:1px solid rgba(0,0,0,.28);border-radius:0;overflow:hidden;white-space:nowrap;padding:6px 4px;font-size:11px;text-align:left;cursor:pointer}.speech{background:var(--speech);color:#fff}.background{background:var(--background)}.unsure{background:var(--unsure);color:#fff}
.editor{display:grid;grid-template-columns:repeat(2,minmax(150px,1fr));gap:8px;align-items:end;margin:10px 0}.editor label{display:grid;gap:4px}.editor input{width:100%;padding:7px}.controls,.source-state{display:flex;gap:5px;flex-wrap:wrap;margin:7px 0}.range-row{display:grid;grid-template-columns:minmax(230px,1fr) minmax(110px,.35fr) minmax(110px,.35fr) repeat(4,auto);gap:8px;align-items:end;border-top:1px solid #d7dde3;padding:7px 0}.range-row label{display:grid;gap:3px;font-size:12px}.range-row input{width:100%;padding:7px}
...
<section>
  ...
  <div><b>时间条播放：</b>每个绿色、黄色或紫色时间条都可点击，只播放该条自身区间并在终点停止。</div>
...
function timeline(row,state){const spans=(state.reviewed_full_source||row.materialize_unreviewed_ranges)?materialize(row,state):normalizeRanges(row,state);return spans.map(span=>`<button type="button" class="span ${esc(span.label)} timeline-span" data-start-frame="${span.start_frame}" data-end-frame="${span.end_frame}" style="left:${100*span.start_frame/row.frame_count}%;width:${Math.max(.3,100*(span.end_frame-span.start_frame)/row.frame_count)}%" title="点击播放 ${esc(span.label)} ${seconds(span.start_frame)}–${seconds(span.end_frame)}s">${esc(span.label)} ${seconds(span.start_frame)}–${seconds(span.end_frame)}s</button>`).join('');}
...
function updateRange(row,state,id,startInput,endInput){const before=state.ranges.map(range=>({...range})),index=state.ranges.findIndex(range=>String(range.id)===id);if(index<0){setMessage('待修改区间不存在',true);return;}const start=clampFrame(startInput.value,row),end=clampFrame(endInput.value,row);if(end<=start){setMessage('修改区间必须满足 end > start',true);return;}state.ranges[index]={...state.ranges[index],start_frame:start,end_frame:end};const error=validateExplicit(row,state);if(error){state.ranges=before;setMessage(error,true);return;}state.reviewed_full_source=false;state.updated_at=new Date().toISOString();persist();render();}
...
function convertRange(row,state,id,label){if(label==='background'){removeRange(row,state,id);return;}const before=state.ranges.map(range=>({...range})),index=state.ranges.findIndex(range=>String(range.id)===id);if(index<0){setMessage('待转换区间不存在',true);return;}state.ranges[index]={...state.ranges[index],label};const error=validateExplicit(row,state);if(error){state.ranges=before;setMessage(error,true);return;}state.reviewed_full_source=false;state.updated_at=new Date().toISOString();persist();render();}
function render(){stopPlayback();const root=document.getElementById('list');root.innerHTML='';for(const row of rows){const state=ensure(row),card=document.createElement('article');if(state.reviewed_full_source)card.classList.add('done');card.dataset.sourceId=row.source_id;const currentVerdict=verdict(row,state);card.innerHTML=`<h2>${esc(row.source_id)}</h2><small>${esc(row.partition)} / ${row.frame_count} frames / ${Number(row.duration_s).toFixed(2)}s / 当前=${esc(currentVerdict)}</small><audio controls preload="none" src="${esc(row.audio)}"></audio><div class="track">${timeline(row,state)}</div><div class="editor"><label>新增区间开始（秒）<input class="start" type="number" min="0" max="${row.duration_s}" step="0.02" value="0.00"></label><label>新增区间结束（秒）<input class="end" type="number" min="0" max="${row.duration_s}" step="0.02" value="${Number(row.duration_s).toFixed(2)}"></label></div><div class="controls"><button class="capture-start" type="button">开始 = 当前播放位置</button><button class="capture-end" type="button">结束 = 当前播放位置</button><button class="preview" type="button">试听新增区间</button><button class="add-speech" type="button">添加 speech</button><button class="add-unsure risk" type="button">添加 unsure</button></div><div class="ranges"></div><div class="source-state"><button class="confirm ${state.reviewed_full_source?'active':''}" type="button">已从头听到尾，以上区间完整</button><button class="reopen" type="button">撤销完整确认</button></div><div class="complete-note">确认后，未标出的完整差集才会写为 background；没有显式区间即表示人工复核为全段 background。</div>`;const audio=card.querySelector('audio'),startInput=card.querySelector('.start'),endInput=card.querySelector('.end');audio.addEventListener('play',()=>{if(activeAudio&&activeAudio!==audio)stopPlayback();activeAudio=audio;});for(const button of card.querySelectorAll('.timeline-span')){const startFrame=Number(button.dataset.startFrame),endFrame=Number(button.dataset.endFrame);button.onclick=()=>playExact(audio,button,startFrame*frameHop,endFrame*frameHop);}card.querySelector('.capture-start').onclick=()=>{startInput.value=seconds(clampFrame(audio.currentTime,row));};card.querySelector('.capture-end').onclick=()=>{endInput.value=seconds(clampFrame(audio.currentTime,row));};const preview=card.querySelector('.preview');preview.onclick=()=>playExact(audio,preview,clampFrame(startInput.value,row)*frameHop,clampFrame(endInput.value,row)*frameHop);card.querySelector('.add-speech').onclick=()=>addRange(row,state,'speech',startInput,endInput);card.querySelector('.add-unsure').onclick=()=>addRange(row,state,'unsure',startInput,endInput);const rangeRoot=card.querySelector('.ranges');for(const range of normalizeRanges(row,state)){const alternate=range.label==='speech'?'unsure':'speech',line=document.createElement('div');line.className='range-row';line.innerHTML=`<button type="button" class="${esc(range.label)}">播放 ${esc(range.label)} ${seconds(range.start_frame)}–${seconds(range.end_frame)}s</button><label>开始（秒）<input class="range-start" type="number" min="0" max="${row.duration_s}" step="0.02" value="${seconds(range.start_frame)}"></label><label>结束（秒）<input class="range-end" type="number" min="0" max="${row.duration_s}" step="0.02" value="${seconds(range.end_frame)}"></label><button type="button" class="update">更新区间</button><button type="button" class="convert-alternate">转 ${esc(alternate)}</button><button type="button" class="convert-background risk">转 background</button><button type="button" class="remove risk">删除此区间</button>`;const play=line.querySelector(`.${range.label}`),rangeStart=line.querySelector('.range-start'),rangeEnd=line.querySelector('.range-end');play.onclick=()=>playExact(audio,play,range.start_frame*frameHop,range.end_frame*frameHop);line.querySelector('.update').onclick=()=>updateRange(row,state,range.id,rangeStart,rangeEnd);line.querySelector('.convert-alternate').onclick=()=>convertRange(row,state,range.id,alternate);line.querySelector('.convert-background').onclick=()=>convertRange(row,state,range.id,'background');line.querySelector('.remove').onclick=()=>removeRange(row,state,range.id);rangeRoot.appendChild(line);}card.querySelector('.confirm').onclick=()=>{const error=validateExplicit(row,state);if(error){setMessage(error,true);return;}state.reviewed_full_source=true;state.updated_at=new Date().toISOString();persist();render();};card.querySelector('.reopen').onclick=()=>{state.reviewed_full_source=false;state.updated_at=new Date().toISOString();persist();render();};root.appendChild(card);}updateStatus();}
