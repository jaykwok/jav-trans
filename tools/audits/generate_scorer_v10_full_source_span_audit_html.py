...
button,input{font:inherit}button{padding:7px 10px;border:1px solid #69737e;border-radius:5px;background:#fff;cursor:pointer}button.active{background:#1769aa;color:#fff}button.risk{border-color:var(--risk)}button.playing{outline:3px solid #111;outline-offset:-3px}.muted,small{color:var(--muted)}h2{font-size:18px;overflow-wrap:anywhere}.error{color:var(--risk);font-weight:600}.complete-note{padding:8px;background:#eef5ef;border-left:4px solid var(--ok)}
...
<section>
  ...
  <div><b>标注方式：</b>从头到尾听完整条 source，添加所有目标语音区间；听不清或标签不确定的区间添加为 unsure。时间自动对齐到 Scorer 的 20ms frame。</div>
...
function updateStatus(){const done=rows.filter(row=>ensure(row).reviewed_full_source).length;setMessage(`完整 source 已确认 ${done}/${rows.length}`);}
...
document.getElementById('save').onclick=async()=>{let content='';try{content=rows.map(row=>{const state=ensure(row),reviewed=Boolean(state.reviewed_full_source);return JSON.stringify({schema:verdictSchema,boundary_serialization_contract_id:row.boundary_serialization_contract_id,source_id:row.source_id,partition:row.partition,frame_count:row.frame_count,frame_hop_s:frameHop,reviewed_full_source:reviewed,verdict:verdict(row,state),spans:reviewed?materialize(row,state):[],updated_at:state.updated_at||new Date().toISOString()});}).join('\\n')+'\\n';}catch(error){setMessage('保存前校验失败: '+error.message,true);return;}const response=await fetch('/__audit_api__/save-labels',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({href:location.pathname,filename:'manual_verdicts.jsonl',content})});const result=await response.json();setMessage(result.ok?'已保存到 '+result.path:'保存失败: '+result.error,!result.ok);};render();
</script>
</body>
</html>"""
