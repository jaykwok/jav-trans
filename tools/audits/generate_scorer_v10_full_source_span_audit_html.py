...
function timeline(row,state){const spans=(state.reviewed_full_source||row.materialize_unreviewed_ranges)?materialize(row,state):normalizeRanges(row,state);return spans.map(span=>`<div class="span ${esc(span.label)}" style="left:${100*span.start_frame/row.frame_count}%;width:${Math.max(.3,100*(span.end_frame-span.start_frame)/row.frame_count)}%" title="${esc(span.label)} ${seconds(span.start_frame)}–${seconds(span.end_frame)}s">${esc(span.label)} ${seconds(span.start_frame)}–${seconds(span.end_frame)}s</div>`).join('');}
