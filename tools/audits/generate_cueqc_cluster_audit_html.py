...
function setMetrics(row) {{
  ...
  const rows = [
    ...
    ["pre_asr_v7", JSON.stringify({{
      features: row.features || {{}},
      ptm_pooling_available: row.ptm_pooling_available,
      ptm_pooling_schema: row.ptm_pooling_schema,
      ptm_pooling_dim: row.ptm_pooling_dim
    }})],
    ["micro", JSON.stringify({{
      below_min: row.below_subtitle_min_duration,
      subtitle_min_s: row.subtitle_min_duration_s,
      candidate: row.micro_chunk_candidate,
      action: row.micro_resolve_action,
      reason: row.micro_resolve_reason
    }})],
    ["weak cuts", JSON.stringify(row.weak_cut_candidates || [])],
