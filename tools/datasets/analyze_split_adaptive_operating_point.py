#!/usr/bin/env python3
"""Read-only analysis for adaptive Semantic Split operating points."""
...
SUMMARY_SCHEMA = "semantic_split_adaptive_operating_point_summary_v1"
ROW_SCHEMA = "semantic_split_adaptive_operating_point_row_v1"
...
def _core_key(row: Mapping[str, Any]) -> tuple[float, float]:
    return (
        round(safe_float(row.get("core_start")), 6),
        round(safe_float(row.get("core_end")), 6),
    )
...
def _core_stats(rows: list[Mapping[str, Any]]) -> dict[tuple[float, float], dict[str, Any]]:
    grouped: dict[tuple[float, float], list[float]] = {}
    for row in rows:
        grouped.setdefault(_core_key(row), []).append(safe_float(row.get("p_cut")))
    stats: dict[tuple[float, float], dict[str, Any]] = {}
    ...
    *,
    ...
    for row in split_rows:
        ...
        if edge_gap < min_chunk_after_split_s:
            continue
        core = stats.get(_core_key(row), {})
        sorted_values = list(core.get("sorted_values") or [])
        percentile = _rank_percentile(p_cut, sorted_values)
        stdev = safe_float(core.get("stdev"))
        z_score = 0.0 if stdev <= 1e-9 else (p_cut - safe_float(core.get("mean"))) / stdev
        if percentile < percentile_floor and z_score < z_floor:
            continue
        eligible.append(
            {
                "time_s": time_s,
                ...
                "core_end": core_end,
                "core_candidate_count": int(core.get("count") or 0),
                "local_percentile": percentile,
                "local_z": z_score,
                "edge_gap_s": edge_gap,
            }
        )
            ...
            examples.append(
                {
                    ...
                    "best_internal_percentile": round(safe_float(best.get("local_percentile")), 6),
                    "best_internal_z": round(safe_float(best.get("local_z")), 6),
    ...
    *,
    ...
    for policy in policies:
        ...
        for feature_dir in _feature_dirs(reexport_dir):
            ...
            cuts = _selected_adaptive_cuts(
                split_rows,
                abs_floor=policy["abs_floor"],
                percentile_floor=policy["percentile_floor"],
                z_floor=policy["z_floor"],
    ...
    summary = {
        ...
        "rows": rows,
        "route_note": (
            "This is read-only adaptive operating-point analysis. Applying any policy changes chunk boundaries "
...
def export_adaptive_analysis(
    *,
    ...
    rows_path = output_dir / "adaptive_operating_point.jsonl"
...
def _parse_policy(value: str) -> dict[str, float]:
    parts = [part.strip() for part in value.split(",") if part.strip()]
    parsed: dict[str, float] = {}
    for part in parts:
        key, sep, raw = part.partition("=")
        if not sep:
            raise argparse.ArgumentTypeError(f"policy item must be key=value: {part!r}")
        parsed[key.strip()] = float(raw)
    required = {"abs_floor", "percentile_floor", "z_floor"}
    missing = required - set(parsed)
    if missing:
        raise argparse.ArgumentTypeError(f"policy missing keys: {sorted(missing)}")
    return {key: float(parsed[key]) for key in sorted(required)}
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--policy", type=_parse_policy, action="append", dest="policies")
    ...
    policies = args.policies or [
        {"abs_floor": 0.50, "percentile_floor": 0.90, "z_floor": 1.00},
        {"abs_floor": 0.55, "percentile_floor": 0.90, "z_floor": 1.00},
        {"abs_floor": 0.60, "percentile_floor": 0.90, "z_floor": 1.00},
        {"abs_floor": 0.50, "percentile_floor": 0.95, "z_floor": 1.25},
        {"abs_floor": 0.55, "percentile_floor": 0.95, "z_floor": 1.25},
    ]
    ...
    for policy in policies:
        if not 0.0 <= policy["abs_floor"] <= 1.0:
            parser.error("policy abs_floor must be in [0, 1]")
        if not 0.0 <= policy["percentile_floor"] <= 1.0:
            parser.error("policy percentile_floor must be in [0, 1]")
