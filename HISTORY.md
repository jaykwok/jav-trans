...
- CueQC 阶段 3 Boundary 反哺：候选 manifest、SpeechBoundary-JA negative source 和 mixed hard-negative + anchor replay 训练前包已生成，但正式训练尚未启动。后续可把已确认 `display=drop`、过碎短切片和无效切分模式沉淀成 Boundary preference / hard-case 数据，用于识别短噪声、无效 speech island 或应合并的边界；这一步是离线偏好训练，不把 CueQC 结果耦合进 Boundary runtime。转换规则：纯非语音/短噪声优先作为 SpeechBoundary-JA frame-level hard negative；过碎切分、孤立短 speech island、应并入相邻字幕的 chunk 进入 Boundary preference/hard-case 池；混合内容或原因不明的 drop 只进审计池，暂不训练。当前 522 条 frame-level hard negative 已混入 1500 条 positive anchor replay，negative share `0.25816`，训练前 gate 已过；下一步可以先跑 feature cache / plumbing smoke，但不能在 full workflow smoke + 人工审计前替换 runtime scorer。
