...
- CueQC 阶段 2 自训练：第一轮 Stage 2b 已完成并替换默认 checkpoint，当前采用 base `drop_threshold=0.85` + `short_text=0.87` 的保守自适应阈值。匿名样片 A Web 全链路 smoke 已完成；剩余验证是更多全片人工抽检、以及后续按相同 false-drop gate 继续扩充训练集。核心安全指标仍是 keep recall / false drop rate，宁可欠删，也不能误删有效字幕。
- CueQC 阶段 3 Boundary 反哺：候选 manifest、SpeechBoundary-JA negative source 和 mixed hard-negative + anchor replay 训练前包已生成，并已训练一个 opt-in feature scorer 候选。后续可把已确认 `display=drop`、过碎短切片和无效切分模式沉淀成 Boundary preference / hard-case 数据，用于识别短噪声、无效 speech island 或应合并的边界；这一步是离线偏好训练，不把 CueQC 结果耦合进 Boundary runtime。转换规则：纯非语音/短噪声优先作为 SpeechBoundary-JA frame-level hard negative；过碎切分、孤立短 speech island、应并入相邻字幕的 chunk 进入 Boundary preference/hard-case 池；混合内容或原因不明的 drop 只进审计池，暂不训练。当前 522 条 frame-level hard negative 已混入 1500 条 positive anchor replay，negative share `0.25816`，feature scorer checkpoint 只通过 `SPEECH_BOUNDARY_JA_SCORER_CHECKPOINT` 显式启用；完整 full workflow smoke + 人工审计通过前不替换默认 scorer。
...
- 2026-06-17 CueQC -> Boundary hard-case 候选池导出完成：新增 `tools.boundary.export_cueqc_drop_hardcases`，默认读取 `agents/audits/*/cueqc_false_drop_audit_labels.jsonl`，按 `sample_id/audit_id` 去重，把 `drop_ok` 导出为 `boundary_hardcase_candidate_from_cueqc_v1`，把 `false_drop_keep`、`uncertain` 或跨轮冲突样本导出为 safety holdout。当前导出产物为 `agents/temp/20260617_214103_boundary-hardcase-candidates-from-cueqc/`：原始标签 `600` 行、unique item `578`、重复 extra rows `22`；confirmed drop candidates `551`，其中 `speech_boundary_frame_negative_candidate=522`、`boundary_preference_candidate=29`；safety holdout `27`。该导出不生成 `boundary_refiner_frame_sequence_dataset_v5`，也不允许直接启动 v5.1 训练；下一步需要把 frame-negative 候选 materialize 成 SpeechBoundary-JA frame 级训练样本，或把 preference 候选补邻接上下文并审成 A/B / delta target。
- 2026-06-17 CueQC -> Boundary v5.1 source preparation 完成：新增 `tools.boundary.prepare_cueqc_drop_v51_sources`，读取上述 hard-case candidates 后，根据三份 false-drop 审计页的 `cueqc_prediction_audit_items.jsonl` 补回 `media.audio_path`、上下文字幕和 aligned segments，并把 frame-negative 候选实际切成短 WAV。当前准备包为 `agents/temp/20260617_220854_boundary-v5.1-sources-from-cueqc/`：`551` 个 confirmed drop candidates 全部 enrichment 成功，`missing_audit_item_samples=0`；`522/522` 个 SpeechBoundary frame-negative candidate 已 materialize，`frame_negative_skipped=0`，生成 `speech_boundary_negative_labels.jsonl`、`speech_boundary_negative_manifest.json`、`speech_boundary_training_manifest.jsonl`，训练可读 examples `522`；另有 `29` 个 `boundary_preference_seed_candidates.jsonl` 保留为后续邻接上下文 A/B 或 delta target 种子。该步骤仍不生成 `boundary_refiner_frame_sequence_dataset_v5`，不能直接替换 v5.1 preference 训练数据。
- 2026-06-18 SpeechBoundary-JA hard-negative mixed feature cache 与 tiny plumbing 训练已完成：基于 `agents/temp/20260617_230948_speech-boundary-hard-negative-finetune-prep/` 的 2022 条 mixed labels（522 negative + 1500 anchor），执行 `build_mixed_feature_cache.ps1` 成功生成 Qwen/MFCC feature cache，summary 为 `feature-cache-mixed-hard-negative-anchor/feature_summary.json`，结果 `cached=2022`、`skipped=0`、`errors=0`，label qualities 为 `negative=522/supervised=1500`。随后执行 `tiny_mixed_plumbing_train.ps1`，输出 `tiny-mixed-hard-negative-anchor/speech_boundary_ja_tiny.pt` 与 `train_metrics.json`，`steps=200`、`loss=0.607988`、`frame_accuracy=0.73`、`positive_ratio=0.73`。该 checkpoint 是 research/plumbing artifact，不接入 runtime，也不替换 `qwen-feature-energy-bootstrap-v1`；下一步如果要做可用 scorer，需要先补正式训练入口/评估协议，再跑 full workflow smoke 与人工审计 gate。
- 2026-06-18 SpeechBoundary-JA runtime-loadable feature scorer path 已落地并训练候选：新增 `FeatureFrameScorer` checkpoint schema `speech_boundary_ja_feature_scorer_v1`、`tools.boundary.ja.train_feature_scorer`、`tools.boundary.ja.evaluate_feature_scorer_thresholds` 和 `SPEECH_BOUNDARY_JA_SCORER_CHECKPOINT` opt-in runtime loading。默认 `SpeechBoundaryJaConfig()` 仍不设置 scorer checkpoint，`qwen-feature-energy-bootstrap-v1` 未替换；启用 scorer 时 boundary signature 和 boundary-cache env signature 会包含 checkpoint path / device，checkpoint 不兼容会显式失败。基于 `agents/temp/20260617_230948_speech-boundary-hard-negative-finetune-prep/` 的 2022 条 mixed labels 与已缓存 features 训练候选到 `agents/temp/20260618_093327_speech-boundary-feature-scorer-hard-negative-anchor/speech_boundary_ja_feature_scorer.pt`，sha256 `d99c065bd6164471f54072126caf53d09dbda85db11177ada5de7f302ffbb5d5`，`steps=200`，held-out frame metrics 为 `frame_accuracy=0.992343`、`precision=0.996499`、`recall=0.995528`、`f1=0.996013`。CUDA opt-in frame-score smoke 已跑通，输出 `opt_in_smoke_frame_scores.json`；短 negative 样本 `probability_mean=0.039604`、`max=0.262385`，在当前默认 threshold/dilation 下仍会残留短 segment。随后离线 threshold sweep 输出 `agents/temp/20260618_094005_speech-boundary-feature-scorer-threshold-eval/`，在 policy `recall>=0.995` 且 `FPR<=0.02` 下无 threshold 通过；最佳 F1 threshold `0.5` 为 `recall=0.998066/FPR=0.086026/F1=0.997301`，threshold `0.8` 虽满足 `FPR=0.019377` 但 recall 仅 `0.989536`。结论：当前候选能保正样本但 hard negative 抑制不足，不能 promoted；下一步应先调采样/损失/阈值策略或补更真实 negative，再考虑全片 no-translate smoke。
- 2026-06-17 SpeechBoundary-JA hard-negative finetune readiness gate 已落地：新增 `tools.boundary.prepare_speech_boundary_hard_negative_finetune`，默认读取最新 `speech_boundary_negative_labels.jsonl` / manifest，校验 hard negative source。negative-only 准备包 `agents/temp/20260617_223612_speech-boundary-hard-negative-finetune-prep/` 显示 `negative_examples=522`、`negative_examples_all_resolved=true`、`negative_examples_ge_min=true`，但缺少 positive/synthetic anchor，因此 `formal_training_ready=false`。随后新增 `tools.boundary.ja.build_positive_anchor_replay`，按 `anime_nsfw=55 / anime_sfw=20 / galgame=25` 抽样生成 `agents/temp/20260617_230636_speech-boundary-positive-anchor-replay/`，共 1500 条 positive anchors，实际组成为 `anime_nsfw=825`、`anime_sfw=300`、`galgame=375`，`training_skipped=0`。混合准备包 `agents/temp/20260617_230948_speech-boundary-hard-negative-finetune-prep/` 已生成 `speech_boundary_mixed_hard_negative_anchor_labels.jsonl` / manifest，`records=2022`、`trainable_examples=2022`、`negative_share=0.25816`，五项 gate 全部通过并写出 `build_mixed_feature_cache.ps1` 与 `tiny_mixed_plumbing_train.ps1`。当前仍未启动 feature cache 或训练；SpeechBoundary-JA runtime 仍是 `qwen-feature-energy-bootstrap-v1`，`tools.boundary.ja.train_tiny` 只作为 plumbing smoke，不是可直接 promoted 的运行模型。
- 2026-06-17 Boundary v5.1 训练准备结论：v5.1 不能只吃 A/B，也可以把 CueQC 已确认 `display=drop` 作为 hard negative 来源，但不能直接把 chunk-level `drop_ok` 当作 `boundary_refiner_frame_sequence_dataset_v5` 标签。当前 v5 训练器只接受 `sequence_boundary_delta_targets/weights`，因此 `display=drop` 必须先路由成两类离线数据：纯非语音/短噪声/无效 speech island -> SpeechBoundary-JA frame-level negative；过碎切分、应缩短或应并入相邻 chunk -> Boundary preference/hard-case，生成可监督的 start/end delta 或成对偏好。新增 `tools.boundary.prepare_v51_training`，用于校验 v5 preference 数据和当前默认 checkpoint 架构，并生成 dry-run / head-finetune 脚本；当前准备包为 `agents/temp/20260617_204422_boundary-v5.1-training-prep/`，摘要 `training_prep_summary.json` 显示 77 rows、`left.end=37/right.start=40`、`baseline=40/challenger=37`、feature hash `cb44d7804ad4eaec9e5e4db80123f817a99650fa`，formal gate `300` 条未达成，因此只建议 dry-run 或 pilot，不建议替换默认 checkpoint。1-step CPU dry-run 已在 `agents/temp/20260617_204116_boundary-v5.1-training-prep/dry-run-1step-matched-config/` 跑通并生成 tensor cache/checkpoint/metrics；正式或 pilot 训练必须带 `--hidden-size 128 --num-layers 2 --state-size 32 --num-heads 4 --n-groups 2 --chunk-size 8 --init-checkpoint src\boundary\checkpoints\boundary_refiner.pt --preserve-init-normalization --freeze-backbone`，否则默认小模型参数会与当前 checkpoint shape mismatch。正式训练前仍建议先把有效偏好扩到至少 300 条并加入 synthetic anchor replay。
