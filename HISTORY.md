...
-> Pre-ASR CueQC v10 keep_for_asr/drop_before_asr（chunk 内 PTM bins + planned-island chunk context + Refiner v8 metadata；已重导候选，等待 current 人工标签后训练，默认关闭且默认 checkpoint registry 置空）
...
- Pre-ASR CueQC v10：已断兼容到 `cueqc_pre_asr_mamba_v10_binary` / `pre_asr_cueqc_features_v6` / `pre_asr_planned_island_sequence_v2`，输入包含 chunk-level PTM bins、planned-island chunk context、Refiner v8 confidence/safety/raw-acoustic timing metadata；当前 latest cold-start 数据来自 fixed-time two-film rerun，导出 `7541` 条 v10 candidates，并用 `pre_asr_coldstart` feature space + compact PTM torque-cut refine 生成 `39` 簇；人工入口收敛为 focused `200` 条审计页，覆盖 top 12 大簇、全局高风险、tail smoke 和 holdout。训练等待人工簇/样本标签，不复用 v6/v7/v8/v9 checkpoint 或旧标签；默认 checkpoint registry 置空，避免未训练模型被自动加载。
...
- 2026-06-30 Grok MCP 修复：用户级 `C:\Users\Jay\.codex\config.toml` 原有 `[mcp_servers.grok-search]` 配置存在，但当前会话未加载为可用 MCP；同时 shell PATH 中没有 `grok-search-mcp` 命令。修复为使用 Node 绝对路径 `C:\Program Files\nodejs\node.exe` 启动 `grok-search-mcp/dist/server.js`，保留 `grok-search` 并新增同配置 alias `grok-search-mcp`，两者都设置 `startup_timeout_sec=120`，并在 env 中显式使用 `grok-4.20-fast` 作为 default/fallback model。同步修正 `C:\Users\Jay\.config\grok-search\config.json` 的 `model/fallback_model` 为 `grok-4.20-fast`；原文件备份为 `config.toml.bak-grok-mcp*` 和 `config.json.bak-grok-mcp-model`。本地 MCP probe `agents/temp/20260630_224700_grok_mcp_probe.mjs` 通过：列出 tools `get_config_info/switch_model/web_fetch/web_fetch_batch/web_search`，配置有效，`/models` 测试成功，返回 model_count `6`。
