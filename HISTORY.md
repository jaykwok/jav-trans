...
- 2026-06-30 Qwen ASR 参数收敛：移除项目自造的 `ASR_HEAD_CONTEXT` / `ASR_HEAD_CONTEXT_MAX_START_S`，避免“视频开头专用提示词”与 Qwen ASR 官方 wrapper 参数混在一起。`grok-search` MCP 已按配置调通并搜索/抓取 Qwen3-ASR 官方仓库、HF model card 与 hotword/context 相关 issue；默认 `grok-4.20-multi-agent-xhigh` 当前仍返回 `model_not_found / 无可用渠道`，但 fallback `grok-4.20-fast` 成功。结论是官方/工具包口径把 `context` 作为 hotword-like / context bias 入口，issue 中有用户尝试 `hotwords=[...]` 但未见官方确认；当前本地 `.venv` 的 `qwen_asr` wrapper 签名也未暴露独立 `hotword/hotwords` 入参。当前固定短语只走 `ASR_CONTEXT`，并作为 Qwen `Qwen3ASRModel.transcribe(context=...)` 传入；语言提示继续由 `ASR_LANGUAGE` 映射到 `language`，生成长度由 `ASR_MAX_NEW_TOKENS` / `TRANSCRIPTION_MAX_NEW_TOKENS` 映射到 Qwen 初始化/转录的 `max_new_tokens`，时间戳请求固定为 `return_time_stamps=False`。不新增伪 hotword API。验证：`py_compile src/asr/transcribe.py src/asr/checkpoint.py src/main.py src/core/config.py src/asr/local_backend.py`，`pytest tests/test_asr_context_runtime.py tests/test_asr_checkpoint_backend_key.py tests/test_asr_stage_env_scope.py tests/web/test_jobs_api.py tests/test_config.py -q`，39 passed。
