...
-> drop_gap mask
-> split_boundary peak picking / NMS / snap-to-valley
...
- `boundary candidates + 字幕级 planner duration split` -> `scorer split peak + edge trim`：切分由 scorer v4 的 `split_boundary_prob` 负责；Boundary Refiner v6 不做中间切点、不做 merge，也不保留 `target_chunk_s/max_core_chunk_s/min_chunk_s/max_splits_per_segment`。
