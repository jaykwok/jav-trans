...
当前主线是 **1.7B Scorer v5 + Pre-ASR CueQC 断兼容重构**：
...
-> Pre-ASR CueQC v5 keep_for_asr/drop_before_asr
...
目标已经从“传统 VAD 高 recall”修正为“字幕边界可用”：scorer v5 只负责 speech / split 两个 frame 任务，Boundary Refiner v6 只修 island 两端，Pre-ASR CueQC v5 在 ASR 前做高精 keep/drop 路由以减少无效 ASR，CueQC v4 仅保留为 ASR 后显式 opt-in 的 shadow/对照和 hard-negative mining。三个 active 模型不互相兼容旧 checkpoint，不保留 alias；schema、repo id、feature hash 或文件存在性不匹配都必须 fail-fast。

当前运行时只规划 speech core。Scorer v5 用 `speech_prob` 找 speech frames，用 `split_boundary_prob` 切开 island；删除路由从 scorer 中移出。Boundary Refiner v6 checkpoint schema 是 `boundary_edge_refiner_v6`、runtime adapter 是 `edge_sequence_v1`，head 只输出 `start_delta_s/end_delta_s`，不跨 island merge，不新增 chunk，不学习 ASR padding/context budget。Pre-ASR CueQC v5 checkpoint schema 是 `cueqc_pre_asr_mamba_v5_binary`，输入只允许 PTM/scorer/refiner/timing 数值特征，禁止 ASR text、token trace、decoder stats、ASR confidence 和 subtitle timing。

## 路线修正

- 2026-06-24 提交前全量代码审计：对 scorer v5 / Boundary Refiner v6 / Pre-ASR CueQC v5 断兼容改动做残留扫描，确认 active `src/` / `tools/` 不再输出旧 `fallback_type/fallback_subtype/fallback_window`、旧 scorer drop-gap、旧 split target/absolute threshold、ASR chunk min-duration 或 ASR-after active drop 语义。审计中修正了 Web smoke 与 workflow summary 仍把 ASR-after CueQC shadow 写成 `cueqc_enabled` / `stage_cueqc_drop_count` 的旧 active 口径，改为 `cueqc_shadow_enabled` / `stage_cueqc_shadow_drop_count`，避免后续脚本误判为 runtime drop。验证：`git diff --check`、核心 Python `py_compile`、重点回归 150 passed。
...
- `ASR-after active CueQC` -> `Pre-ASR CueQC v5 active + CueQC v4 shadow`：旧规则 QC 不再运行；前置模型负责节省 ASR，后置 v4 只用于对照、审计和 hard-negative mining。
...
- learned padding/context budget、ASR padded fallback 时间轴、DP/Viterbi planner、per-gap binary merge classifier：均已被当前 scorer v5 + Boundary Refiner v6 + Pre-ASR CueQC v5 路线取代。
...
- 1.7B Scorer v5：v5-native 4096 smoke 和 32768 full 已完成训练；当前等待 two-film duration audit 的人工观感 gate，再决定是否作为 1.7B 默认 scorer 进入后续 Refiner / Pre-ASR CueQC 数据重导。
- Boundary Refiner v6：等待 scorer v5 通过 workflow gate 后，重新用 scorer v5 predicted island edge 导出 edge-only dataset，再训练 repo-id tagged v6 checkpoint。
- Pre-ASR CueQC v5：等待 scorer/refiner 新链路稳定后，从 refined chunk 与人工 keep/drop 标签编译无 ASR 文本特征，训练 `cueqc_pre_asr_mamba_v5_binary`。
- ASR-after CueQC v4：仅保留 shadow/mining；当 Pre-ASR CueQC v5 false-drop gate 稳定后，删除后置 runtime 与训练入口。
...
- 2026-06-24 Scorer v5 split decoder 取消 `split_target_s` 数量预算：用户确认不再按“每约 N 秒最多几个 split”限制，而是保留 smoothing、adaptive score/prominence quantile、NMS、snap-to-valley 与 `min_split_segment_s` 后的有效模型峰。代码层已移除 `split_target_s` / `SPEECH_BOUNDARY_JA_SPLIT_TARGET_S` active runtime、workflow CLI/env 和诊断工具预算逻辑，残留检查只留在测试。用 full scorer v5 two-film frame scores 做无预算诊断：sample-e redecoded segments `2390 -> 7695`、max `12.26s -> 3.18s`；sample-d redecoded segments `1102 -> 3436`、max `12.26s -> 4.56s`。这说明长段问题被切开，但候选密度升到约 `62-70` 段/分钟；selected split score 中位约 `0.10-0.14`，同时 segment mean speech 低于 `0.5` 的候选约一半。当前结论是不急于把 split quantile 收紧成新的隐式时长/数量 gate，优先用 Pre-ASR CueQC v5 学 keep/drop 验证这些候选密度是否合理；如后续 ASR 前 drop 仍不能稳定压低无效候选，再回到 split effective-peak 定义做数据或校准修正。
- 2026-06-24 Pre-ASR CueQC v5 标签编译链路补齐：ASR pipeline 现在会在 wav chunk 导出前把 `details.pre_asr_candidates` 持久化，每条候选带稳定 `preasr-<video>-chunkNNNNN` 的 `sample_id/candidate_id`、repo 匹配所需的纯数值 feature schema，以及启用 Pre-ASR CueQC 时的前置 keep/drop decision。`tools/asr/cueqc/compile_pre_asr_v5_features.py` 支持把簇级 `cueqc_cluster_labels.jsonl` 通过 `examples`、`sample_ids`、`video_id#chunk_index` 或候选 `cluster_id` 广播到具体 sample id；`training_label_included=false` 仍为 abstain。验证：`py_compile` 通过，`tests/test_pre_asr_cueqc.py tests/test_pipeline_chunk_packing.py tests/test_run_full_workflow_env.py tests/test_boundary_ja_current.py tests/test_split_peak_diagnostics.py tests/test_asr_stage_env_scope.py` 共 `55 passed`，`git diff --check` 通过（仅 Windows LF/CRLF warning）。
...
- 2026-06-23 Scorer v5 + Pre-ASR CueQC 断兼容重构：active scorer schema 切到 `speech_boundary_ja_mamba2_frame_boundary_scorer_v5`，只输出 `speech_prob/split_boundary_prob`；删除路由从 scorer 移到 Pre-ASR CueQC v5。Boundary Refiner 保持 v6 edge-only；Pre-ASR CueQC v5 插入在 Refiner/pack 后、wav chunk 导出前，特征禁止 ASR text、token trace、decoder stats、ASR confidence 和 subtitle timing。
