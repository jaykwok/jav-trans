...
- 2026-06-24 Codex `logs_2.sqlite` TRACE 写盘止血：检查 `C:\Users\Jay\.codex\logs_2.sqlite` 时发现库体积已到 `312500224` bytes，`logs` 表共 `50617` 行，但累计 `estimated_bytes` 中 `TRACE` 占约 `151386738` bytes；其中 `codex_client::transport` 单一 target 约 `126553306` bytes，确认历史上存在 TRACE 高频写盘放大量。按“先止血再观察”处理：先用 SQLite backup 备份到 `agents/temp/20260624_091541_codex-logs2-trace-mitigation/logs_2.sqlite.backup`，随后在原库加 `logs_block_trace_insert` trigger，规则为 `BEFORE INSERT ON logs WHEN NEW.level='TRACE' SELECT RAISE(IGNORE)`，再执行 `PRAGMA wal_checkpoint(TRUNCATE)`，返回 `[0,0,0]`，`logs_2.sqlite-wal` 从 `12392` bytes 归零。处置前后采样记录在 `agents/temp/20260624_091541_codex-logs2-trace-mitigation/mitigation_report.json`：`MAX(id)` 保持 `6617245`，5 秒复采样 `MAX(id)` 和 WAL 均无增长；最近窗口仍可见 `INFO` 心跳 `codex_app_server_transport::transport::remote_control::websocket`，但未再观察到新 TRACE 追加。
