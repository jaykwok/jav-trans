...
-> drop_gap mask
-> split_boundary adaptive top-k peak picking / NMS / snap-to-valley
...
目标已经从“传统 VAD 高 recall”修正为“字幕边界可用”：scorer v4 负责 speech / split / drop gap 三个 dense frame 任务，Boundary Refiner v6 只修 island 两端，CueQC v4 只做 ASR 后 keep/drop。三个模型不互相兼容旧 checkpoint，不保留 alias；schema、repo id、feature hash 或文件存在性不匹配都必须 fail-fast。

当前运行时只规划 speech core。Scorer v4 用 `speech_prob` 找 speech frames，用 `drop_gap_prob` 删除明确非语音 gap，用 `split_boundary_prob` 做 adaptive top-k peak picking 切开 island；split 校验只做去毛刺，不再有绝对 split threshold 或 0.4s 字幕级最小切分。Boundary Refiner v6 checkpoint schema 是 `boundary_edge_refiner_v6`、runtime adapter 是 `edge_sequence_v1`，head 只输出 `start_delta_s/end_delta_s`，不跨 island merge，不新增 chunk，不学习 ASR padding/context budget。CueQC v4 checkpoint schema 是 `cueqc_mamba_v4_binary`，只输出 `keep/drop`，runtime 使用全局 `p_drop >= threshold`。

## 路线修正

- `FusionVAD-JA / VAD` -> `SpeechBoundary-JA / Boundary`：项目已经不是严格 speech/non-speech VAD，而是面向字幕切分的边界系统。
- `speech + cut 双头` -> `speech/split/drop_gap 三头`：旧 `cut_prob -> clear speech_prob` gate 语义混杂，已被 scorer v4 的 split boundary peak 与 drop gap mask 拆开。
- `boundary candidates + 字幕级 planner duration split` -> `scorer adaptive split peak + edge trim`：切分由 scorer v4 的 `split_boundary_prob` 负责；Boundary Refiner v6 不做中间切点、不做 merge，也不保留 `target_chunk_s/max_core_chunk_s/min_chunk_s/max_splits_per_segment`。
- `Boundary Refiner v5 frame_sequence_v1` -> `Boundary Refiner v6 edge_sequence_v1`：v5 checkpoint 或 metadata 不匹配 v6 edge-only contract 时直接报错。
- `CueQC v3-Fusion + threshold profile` -> `CueQC v4 binary + global threshold`：CueQC 只做 `keep/drop`，不再有 `text_bucket/drop_threshold_profile/density_level/vocalization_repetition`。
- `旧规则 ASR QC` -> `CueQC v4 binary 二元路由`：旧规则 QC 不再运行、不再观测、不再作为候选排序或质量报告信号；最终是否丢弃由 CueQC 模型决策。
...
- learned padding/context budget、ASR padded fallback 时间轴、DP/Viterbi planner、per-gap binary merge classifier：均已被当前 scorer v4 + Boundary Refiner v6 edge-only 路线取代。
...
- 1.7B Scorer v4-native：4096 smoke plumbing 已打通，但 cluster-seed real-negative smoke 仍未通过真实 60s long-island gate；下一步先扩大/净化真实非语音 hard negative 和长背景负样本，再重跑 4096 smoke。通过后再放大到 `32768` full，并重跑 10-film no-CueQC baseline 人工 gate。
- 1.7B CueQC v4 binary：等待 v4-native scorer 通过真实 workflow gate 后，重新用新 scorer/refiner 链路导出 candidates，再编译 v4 binary seed feature bundle 并训练首版 checkpoint。后续每轮按 false-drop/uncertainty/风险桶/随机混合抽约 200 条人工审计，再训练。匿名样片 B 保持最终测试片。
- 0.6B 三模型：1.7B scorer/refiner/cueqc 完整通过后，再按相同 schema 独立重训 0.6B；不复用 1.7B checkpoint，除非以后专门设计固定维度 adapter。
- CueQC 正向补样：如果需要混合正向样本，从 anime 数据集抽取，NSFW:SFW 按 `1:1`，记录来源、计数和 seed。
- 直接字幕边界 / timeline model：当前 active runtime 已采用 Boundary chunk 作为字幕时间轴；长期研究重点是让 Boundary first scorer / refiner 更稳定地产生一句台词一行的 speech-core chunk，而不是恢复外部 forced aligner。
- Unified Joint Model / 小模型蒸馏：等 Boundary/CueQC 审计闭环稳定后，再评估 Qwen ASR boundary-token SFT / DPO / RL、长上下文 joint segmentation-transcription，以及 SpeechBoundary-JA / Boundary Refiner 的小模型蒸馏。

## 近期记录

- 2026-06-23 ASR 后硬拆断兼容清理：两片 scorer v4-native32768 no-CueQC 审计发现 `匿名样片 D` 第一条 `33.885688s` 长 chunk 在 ASR 输入阶段已经是一整块，后续 `aligned_segments` 的多段只是 `_group_words_to_segments/_postprocess_segments` 里残留的字幕级分段兜底。active runtime 已删除 `ASR_SEGMENT_HARD_MAX_DURATION`、`ASR_POSTPROCESS_MAX_*` 和 `_split_long_postprocessed_*` 路径；`aligned_segments` 只按 `source_chunk_index` 边界聚合，不再按句末、时长、字符数或文本比例在同一 ASR chunk 内硬拆。这样长 chunk 会直接暴露为 scorer/top-k 前置切分问题，而不是被 ASR 后处理掩盖。Scorer runtime 默认 `speech threshold/drop_gap threshold` 同步到训练评估口径 `0.5/0.5`，`.env.example` 仅保留同值的实验覆盖注释；未引用的 `audio.speech_boundary_refine` ASR 后二次 boundary 切片入口已移到 `agents/rm/20260623_135138_asr-post-boundary-refine-legacy/`，active tests 不再保留它。全项目固定阈值审计确认 `ASR_CHUNK_MIN_DURATION_S=0.25` 仍会在 scorer/refiner 之后、ASR 之前吞掉短有效 island，已降为 `0.08s` 去毛刺口径并加回归测试；字幕 writer 的 `MAX_SUBTITLE_DURATION/MIN_SUBTITLE_DURATION` 只作用于最终 cue plan 展示，不参与 ASR chunk 生成。
- 2026-06-23 Scorer v4-native real background span smoke：新增 `tools.boundary.export_cueqc_seed_drop_background_spans`，用 cluster seed-drop candidates 作为锚点，在 fresh 10-film `cueqc_candidates.jsonl` 时间轴上合并连续空文本候选；仅当 span 含已审计 seed-drop anchor、无 text-present overlap，并对相邻 text-present chunk 留 `guard_s=0.12` 时导出，schema 仍为 `speech_boundary_hard_negative_candidate_from_cueqc_v1`，可直接接 `tools.boundary.prepare_cueqc_drop_hard_negative_sources`。基于 `819` 个 seed-drop sample id 导出 `109` 个 background span candidates，总时长 `547.517s`，p50/p90/p99/max `3.477/12.814/19.938/21.803s`，覆盖 10 部片；物化 `109/109` 条 negative wav/manifest。用短 seed-drop manifest + background span manifest 重建 `agents/temp/20260623_091209_scorer-v4-native-4096-realneg-bgspan-smoke/`：4096 records、negative_rows `928`、feature cache 1.7B CUDA/bf16/no-compress `4096/4096`、errors `0`、约 `136s`。smoke 训练 `1200` steps 输出 `train-bg-s1200/scorer_v4.native4096-bgspan.qwen17b.pt`，指标：speech precision/recall/F1 `0.9961/0.9697/0.9827`，drop_gap `0.8943/0.9952/0.9421`，split `0.2906/0.7411/0.4175`。同一 匿名样片 C 60s long-island probe 从上一版 realneg 的 segments/max/p90 `31/7.14s/3.98s` 改为 `110/1.88s/0.82s`，speech p90 `0.665 -> 0.460`，drop_gap p90 `0.737 -> 0.803`，基本通过长假语音 smoke gate。cache 分布显示 positive/split-stress speech 区域 speech p10 约 `0.844/0.817`、drop p99 约 `0.443/0.503`，短有效台词风险可接受；mixed_contrast speech 区域 drop p90/p99 约 `0.662/0.872`，说明带背景人声仍是主要风险，下一步可进入 `32768` full 训练，但 10-film no-CueQC workflow gate 必须重点审计“背景人声被误 drop”。
- 2026-06-23 Scorer v4-native cluster-seed real-negative smoke：新增 `tools.boundary.export_cueqc_cluster_seed_hardcases`，把 fresh 10-film cluster 审计中 `seed_action=use_seed`、`display_decision=drop`、`training_label_included` 未禁用且默认无 `text_present` 的簇导出为 SpeechBoundary-JA hard-negative candidates；`mixed_skip/skip`、keep、disabled、text-present 簇默认 abstain，避免把 `cluster_03` 这类混簇或带文本簇当成 scorer 非语音。当前审计 `agents/audits/20260622_214207_cueqc17b-v4-fresh-10film-coldstart-audit/cueqc_cluster_labels.jsonl` + layer1 clusters 导出 `35` 个 seed-drop 空文本簇、`819` 条 candidates，排除 `mixed_skip=1` 和 text-present cluster `1`；`tools.boundary.prepare_cueqc_drop_hard_negative_sources` 物化 `819/819` 条 negative wav/manifest，负样本总时长 `847.666s`，p50/p90/max `0.824/1.86/5.34s`。用该 manifest 重建 `agents/temp/20260623_031000_scorer-v4-native-4096-realneg-smoke/`：4096 records、negative_rows `819`、feature cache 1.7B CUDA/bf16/no-compress `4096/4096`、errors `0`、约 `144s`。smoke 训练 `1200` steps 输出 `speech_boundary_ja_frame_boundary_scorer_v4.native4096-realneg.jaykwok-Qwen3-ASR-1.7B-JA-Anime-Galgame.pt`，指标：speech precision/recall/F1 `0.9919/0.9805/0.9861`，drop_gap `0.8974/0.9887/0.9408`，split `0.3194/0.7833/0.4538`。同一 匿名样片 C 60s long-island probe 对比合成负样本 smoke 有改善但未过 gate：segments `16 -> 31`，max segment `12.96s -> 7.14s`，p90 duration `7.74s -> 3.98s`，speech p90 `0.951 -> 0.665`，drop_gap p90 `0.573 -> 0.737`；longest segment 仍 `7.14s`，不应晋升 registry 或放大到 32768。离线 drop threshold sweep 显示单条 probe 上 `drop_gap_threshold=0.70` 可到 p90 `2.68s`、max `4.72s`、covered `48.7s`，`0.60` 可到 p90 `0.76s`、max `3.16s` 但 covered 仅 `29.26s`，风险偏大。cache 分布检查显示 positive/split-stress speech 区域 drop p99 约 `0.403/0.462`，但 mixed-contrast speech 区域 drop p90/p99 约 `0.627/0.818`，说明只下调 runtime drop threshold 可能吞带背景人声；下一步先扩大真实非语音/长背景 hard negative，再重跑 4096 smoke，必要时再用多片 workflow gate 校准 drop threshold。
- 2026-06-23 Scorer v4-native 数据集实现：新增 `tools.boundary.ja.build_scorer_v4_native_dataset`，schema 为 `speech_boundary_ja_scorer_v4_native_dataset`，默认生成 `4096` smoke。数据 mix 固定为 positive speech timeline `40%`、pure hard negative `25%`、mixed contrast `20%`、split stress `15%`；正向默认 anime NSFW:SFW=`1:1` 加 galgame，hard negative 可显式传 `--negative-manifest`，不传时仅用于 smoke/单测的合成 silence/noise/hum fallback。record metadata 写入 dataset schema、ASR repo id、feature hash、source mix、negative source、speech dilation、split radius、drop policy、seed、created_at、source audio ids 和按单个 source id 哈希得到的 `train/val/test` split；mixed/split-stress 组合样本会强制所有 speech/negative source id 落在同一 partition，发现跨 partition 直接报错，避免 source audio id 泄漏。输出 labels、audio manifest、training manifest、details、splits、summary。训练端 `_feature_training_arrays()` 新增 `boundary_metadata.head_frame_weights` 支持，语义为基础 `frame_weights` 乘 per-head override，旧 labels 不受影响。单测覆盖 pure hard negative `speech=0/drop_gap>=0.95`、mixed contrast 的 speech 区域不标 drop、split stress 有 split label 且不自动产生 drop、source id 不跨 split，以及 per-head weight 长度校验。后续 4096 smoke/cache/train/probe 已由 cluster-seed real-negative 记录覆盖；CueQC/Refiner 新训练继续等待新 scorer gate。
- 2026-06-23 60s long-island probe 人工听感结论：`adaptive_topk_peak` 把旧 decoder 的 60s probe 从约 `2` 段、最长约 `39s` 改成 `13` 段、最长 `9.58s`，说明相对峰切分方向有效；但人工听感确认多数切出片段仍是非语音，且 9s 片段仍明显过长。逐段统计显示所有输出段 `speech_mean` 约 `0.94-0.99`，而 `drop_gap_p90` 仅约 `0.09-0.39`，低于 runtime `0.75` 阈值，证明问题不在继续调 split 阈值，而在 scorer 训练数据/标签：speech 头把背景/非语音高置信激活，drop_gap 头没有学会压制真实背景。下一步暂停 CueQC v4 与 Boundary Refiner v6 新训练，先重建 v4-native scorer 数据：显式加入真实背景/环境声/hard negative 为 `speech=0`，把非语音 gap/drop 与 utterance split 分开监督，保留 Galgame 随机拼接只表达 split，不表达 speaker id；scorer 通过真实 workflow audit 后，再用新 scorer 重新导出 Refiner edge dataset 和 CueQC candidates。
- 2026-06-22 SpeechBoundary-JA scorer v4 split decoder 断兼容修正：fresh 10-film audit 发现 1.7B v4 链路仍有明显 10s+ 甚至更长 speech island，根因不是 speech/drop_gap 指标，而是 `split_boundary_prob` 绝对阈值过高；长窗 probe 显示 split peak 相对存在但绝对值普遍低于旧默认阈值。active runtime 已移除 `SPEECH_BOUNDARY_JA_SPLIT_THRESHOLD` / `SPEECH_BOUNDARY_JA_SPLIT_PROMINENCE` 配置入口，改为单一路径 `adaptive_topk_peak`：按 segment duration 和 `SPEECH_BOUNDARY_JA_SPLIT_TARGET_S` 预算切点数量，候选用局部极大值、score/prominence quantile、NMS 和 snap-to-valley 选择，默认不再有强峰/绝对阈值分支。Boundary cache bump 到 v7，`.env.example` 只保留 `SPEECH_BOUNDARY_JA_SPLIT_TARGET_S`、`SPEECH_BOUNDARY_JA_SPLIT_SCORE_QUANTILE`、`SPEECH_BOUNDARY_JA_SPLIT_PROMINENCE_QUANTILE` 等自适应参数。长期训练策略：scorer checkpoint 先不盲目重训，优先用新 decoder 重跑 fresh no-CueQC baseline 审计；由于 chunk/edge 分布改变，Boundary Refiner v6 数据应重新由新 decoder 导出并重训，CueQC v4 seed/feature/prediction 也必须基于新 scorer/refiner workflow 全量重跑，旧 fresh audit 只保留为失效对照。若新 decoder 仍明显粘句，再重建 v4-native scorer 数据并重训 scorer。
- 2026-06-22 1.7B CueQC v4 fresh baseline 与 cold-start 审计入口：确认旧 CueQC v3/Stage2a/cluster 产物来自旧 scorer/refiner/runtime，不再作为最终 v4 binary 训练数据，只保留历史参考。已用当前链路（SpeechBoundary scorer v4、Boundary Refiner v6、1.7B ASR，CueQC shadow/apply 均关闭）重跑 10-film no-translate baseline，输出 `agents/temp/speech-boundary-ja/20260622_192613_cueqc17b-v4-latest-no-cueqc-baseline-bs8-10film/`；10/10 完成，`cueqc_enabled=False`，总 pipeline 约 `7123.4s`，ASR batch `8`，timing 峰值 reserved 约 `5.3GB`，匿名样片 B 继续排除为最终测试。基于 archived aligned outputs 重新导出 fresh CueQC candidates 到 `agents/temp/20260622_213913_cueqc17b-v4-fresh-10film-candidates/cueqc_candidates.jsonl`，共 `12111` 条，schema 全部为 `cueqc_candidate_v4`，旧字段 `density_level/drop_threshold_profile/text_bucket/vocalization_repetition/threshold_profile` 命中 `0`。为避免 12k 全量 TORC 的 O(n²) 聚类成本，先生成分层 cold-start audit sample `agents/temp/20260622_214207_cueqc17b-v4-fresh-audit-sample/cueqc_candidates.audit_sample.jsonl`，`1599/12111` 条，覆盖每部片并提高短文本、重复、低置信、极端时长等易错样本占比；该 sampling policy 只用于审计入口，不是 runtime 规则或训练标签。Torque structured 默认 gap-cut 仅 `2` 簇，改用 `--merge-layer 1` 得到 `37` 个审计簇；正式审计页生成到 `agents/audits/20260622_214207_cueqc17b-v4-fresh-10film-coldstart-audit/index.html`，`review_item_count=1599`，显式传入 fresh baseline archived/jobs media roots，校验 10/10 音频和字幕存在，`missing_media_videos=[]`、`missing_subtitle_videos=[]`、单播放器懒加载。下一步等人工导出 cluster labels 后，只广播 `seed_action=use_seed` 且 `display_decision in {keep,drop}` 的簇进入 v4 seed 训练。
- 2026-06-22 1.7B Boundary Refiner v6 scorer-driven 训练完成并晋升 registry：确认旧 `tools.boundary.build_refiner_frame_sequence_dataset` 仍从 true/teacher segments 直接构造样本，会让 v6 学到“已正确边界的零 delta”，不符合 edge-only contract；已断兼容重写为必须加载 scorer v4 checkpoint，并用 runtime 共享的 `decode_frame_boundary_segments()` 从 `speech/split/drop_gap` frame heads 生成 predicted islands，再把 predicted island left/right edge 对齐到 true speech segments 生成 `start_delta_s/end_delta_s`。dataset row 和训练 checkpoint metadata 都记录 `dataset_source=scorer_v4_predicted_island_edges`、scorer schema/repo/sha，trainer 现在拒绝缺 `metadata.scorer_checkpoint` 或非 scorer-driven source 的 v6 row。4096 条 scorer-driven hardmix 数据输出到 `agents/temp/20260622_172846_refiner17b-v6-scorer-driven-4096/frame_sequence_dataset_v6.qwen17b.scorer-driven4096.jsonl`，`sequences=4096`、`sequence_items=17748`、feature hash `5ef1426b2f181a8e6d5d3c3a3b54862e54497feb`、scorer sha256 `9073e4b6def1248ecd53dc643b677542dae5d538efa8d95f6095bf9468db5a31`，start/end supervised `15212/17335`，target clip `742/803`。CUDA 训练 `600` steps、batch `512`、lr `5e-4`、weight decay `0.01`、hidden/layers/state `128/2/32`，输出 `agents/temp/20260622_172846_refiner17b-v6-scorer-driven-4096/train-mamba2-v6-qwen17b-scorer-driven4096/boundary_edge_refiner_v6.jaykwok-Qwen3-ASR-1.7B-JA-Anime-Galgame.pt`；val start/end MAE `0.0842/0.0912s`，loader smoke 通过，expected 0.6B repo mismatch fail-fast。checkpoint 已复制到 `src/boundary/checkpoints/boundary_edge_refiner_v6.jaykwok-Qwen3-ASR-1.7B-JA-Anime-Galgame.pt`，sha256 `98d3465de5b8131de03382d013076b35845bd310b4676cc3a3e328d00871a839`，旧 v5 `boundary_refiner.*.pt` 已移到本地 `agents/rm/20260622_183257_legacy-boundary-refiner-v5-checkpoints/` 并从 Git 删除。4096 版是先打通 scorer-driven v6 工作流的正式 1.7B checkpoint；全量 32768 scorer-driven 导出预计小时级，后续如果真实 smoke/人工 gate 需要再后台扩大。
- 2026-06-22 1.7B SpeechBoundary-JA scorer v4 训练完成并晋升 registry：复用 `agents/temp/20260619_130400_boundary-v5-qwen17b-delta-only-32768-hardmix/synthetic/labels.jsonl` 与 `feature-cache-qwen17b/feature_manifest.jsonl` 训练三头 scorer v4，训练目录 `agents/temp/20260622_140504_scorer17b-v4-32768-hardmix/`，输出 checkpoint `ckpt/speech_boundary_ja_frame_boundary_scorer_v4.jaykwok-Qwen3-ASR-1.7B-JA-Anime-Galgame.pt`，sha256 `9073e4b6def1248ecd53dc643b677542dae5d538efa8d95f6095bf9468db5a31`。训练指标：speech precision/recall/F1 `0.99663/0.99254/0.99458`，drop_gap `0.95602/0.99395/0.97461`，split_boundary `0.39335/0.45267/0.42093`；split 是稀疏 frame label，后续以 runtime peak picking / workflow 审计为主 gate。checkpoint 已复制到 `src/boundary/ja/checkpoints/speech_boundary_ja_frame_boundary_scorer_v4.jaykwok-Qwen3-ASR-1.7B-JA-Anime-Galgame.pt`，loader smoke 确认 repo id `jaykwok/Qwen3-ASR-1.7B-JA-Anime-Galgame`、schema `speech_boundary_ja_mamba2_frame_boundary_scorer_v4`、output heads `speech_prob/split_boundary_prob/drop_gap_prob` 和 decoder `peak_split_v1` 匹配。旧 v3 `speech_boundary_ja_feature_scorer.*.pt` 已归档到本地 `agents/rm/20260622_164812_legacy-scorer-checkpoints/` 并从 Git 删除；0.6B v4 checkpoint 尚未训练，因此 0.6B 默认会 fail-fast，这是本轮“先完整 1.7B，后续同 schema 重训 0.6B”的预期状态。训练代码补充 normalization/train progress 日志，便于长跑时判断是否进入共享显存或卡在预处理。
- 2026-06-22 1.7B 三模型断兼容重构落地：active code 已切到 scorer v4 / Boundary Refiner v6 / CueQC v4 binary。旧 `cut_prob` gate、boundary candidates、planner duration split、CueQC threshold profile、subtitle display/soft split 规则和旧评估/准备脚本已移到 `agents/rm/20260622_103438_breaking-v4-legacy-runtime/`；active runtime 不保留 alias。默认 registry 现在指向 v4/v6/v4 目标文件名，旧 v3/v5/v3-Fusion checkpoint 会 fail-fast；1.7B 新 checkpoint 训练落盘前默认运行会因文件不存在或 schema 不匹配报错，这是刻意的断兼容状态。
- 2026-06-22 repo-id checkpoint 绑定决策更新：三个 Mamba checkpoint 都自动跟随当前 ASR repo id 解析，`.env` 不再持久化默认 checkpoint map。训练入口和 loader 均要求 metadata repo id 匹配：SpeechBoundary scorer feature manifest 必须有唯一 `ptm`，Boundary Refiner v6 rows 必须有唯一 `metadata.ptm_repo_id`，CueQC v4 feature bundle 必须有 `feature_config.asr_model_id`。文件名只作为人工可读 tag，不参与归属判断。
- 2026-06-22 CueQC 启发式分桶与字幕规则位置清理：CueQC candidate 不再生成 `text_density.level`、`short_vocalization_candidate`、`repeated_vocalization_candidate`、`vocalization_repetition` 等手工语义分桶；聚类 structured vector / 审计页摘要改为使用原始数值文本观测。ASR 后 SRT writer 只保留排序、no-overlap、2-frame gap 和必要 timing polish，不再运行 subtitle-level soft split / display policy。
- 2026-06-22 训练前全量代码审计：active tree 再次扫描旧 scorer/refiner/CueQC schema、旧 checkpoint 文件名、`cut_prob` / planner duration split / CueQC profile / display policy 残留。确认 CueQC runtime 只保留 v4 binary keep/drop，旧词只允许出现在 fail-fast 测试或 README/HISTORY 的废弃说明中。已把未引用的 `src/boundary/ja/postprocess.py` 移到 `agents/rm/20260622_125336_audit-dead-code/`，修正 CueQC v4 Stage2a/source metadata、feature shard help、run_full_workflow help、packaging README 和 repo-id registry 测试里的旧 v3/v5 文件名。
- 2026-06-19 0.6B SpeechBoundary-JA scorer 候选固定：正式作为 0.6B scorer 候选的是 `agents/temp/20260618_221851_speech-boundary-frame-boundary-scorer-v3-negative-rich8192-dil06-prep/frame-boundary-scorer-v3/speech_boundary_ja_feature_scorer.pt`，因为它是当前 v3 schema `speech_boundary_ja_mamba2_frame_boundary_scorer_v3`、model type `mamba2_frame_boundary_scorer`、feature manifest `ptm=jaykwok/Qwen3-ASR-0.6B-JA-Anime-Galgame` 的最新 speech+cut 双头训练产物；两个 v2 scorer 已是废弃 schema，不作为绑定候选。该文件已复制为 `src/boundary/ja/checkpoints/speech_boundary_ja_feature_scorer.jaykwok-Qwen3-ASR-0.6B-JA-Anime-Galgame.pt`，补写 `metadata.ptm_repo_id=jaykwok/Qwen3-ASR-0.6B-JA-Anime-Galgame`，sha256 `79cdab90a980e9e77fe68a5e0b58e0f0ec555f12cc247ad4dcf62f5f6afc2a1e`，大小 `2477377` bytes。经 匿名样片 A A/B 审计确认 0.6B scorer 观感优于 bootstrap 后，该 checkpoint 晋升为 0.6B 默认 scorer；1.7B scorer 后续也已在 2026-06-20 进入默认 registry，此条只保留 0.6B provenance。
...
- 2026-06-21 CueQC 1.7B Stage2a 主动学习进展：用户完成 `agents/audits/20260620_232325_cueqc-qwen17b-seed-t040-false-drop-audit/cueqc_false_drop_audit_labels.jsonl`，200/200 已标，结果 `drop_ok=145 / false_drop_keep=55 / uncertain=0`，裸 false-drop rate `27.5%`；这确认 seed t0.40 只适合主动学习筛选，未审 drop pseudo 不能进训练。按既有 Stage2a 安全策略编译 `agents/temp/20260621_204726_cueqc-qwen17b-stage2a-from-seed-t040-audit/cueqc_stage2a_features_v3_fusion.pt`：只纳入 cold-start seed、200 条人工审计和高置信 keep pseudo，跳过未审 drop pseudo `11159`，输出 `8387` 条（`drop=3388 / keep=4999`），其中 high-conf keep pseudo `4906`、人工 `drop_ok=145`、人工纠正 keep `55`。首次训练在第 25 步验证 OOM，原因是 `train_mamba_v3_fusion.evaluate()` 对 val/test 一次性 pad 成整批；已新增 `--eval-batch-size` 并改为分批 eval。Stage2a candidate 训练到 early stop step `45`，恢复 best step `5`，checkpoint `stage2a_model/cueqc_mamba_v3_fusion.stage2a-seed-t040-audit.jaykwok-Qwen3-ASR-1.7B-JA-Anime-Galgame.pt`，默认 `drop_threshold=0.85`；匿名样片 K holdout `n=861`，`keep_recall=0.9979 / false_drop_rate=0.0021 / drop_precision=0.9877 / drop_recall=0.2030`，val `keep_recall=0.9985 / false_drop_rate=0.0015 / drop_recall=0.9705`。该候选非常保守且 holdout drop recall 偏低，不能替换默认；已对 10-film full features 重新预测，records `19552`、`drop=5206 / keep=14346`、pseudo `drop=5206 / keep=8493`，drop prob 分布 `0.850073..0.986627`、median `0.945983`。下一轮 false-drop 审计页已生成 `agents/audits/20260621_235915_cueqc-qwen17b-stage2a-default-false-drop-audit/index.html`，200 条 mixed drop，采样 `min=0.85/high=0.95/near_margin=0.03`；等待人工导出标签后再决定 Stage2b 或阈值 profile，不碰 匿名样片 B。
- 2026-06-18 force aligner 断兼容结论：匿名样片 A forced vs Boundary A/B 人工审计确认 Boundary Refiner speech-core chunk 时间轴更自然；force aligner 成功样本也系统性向内裁剪 chunk，排除首条异常后的 `905` 个 forced-success chunk 中，`delta_start = forced_start - chunk_start` mean/p50/p90/p95 为 `+0.440/+0.160/+1.280/+1.600s`，`delta_end = forced_end - chunk_end` 为 `-0.871/-0.640/-0.068/-0.020s`，duration delta p50 `-1.140s`。因此 active runtime、默认配置、Web 模型需求、Windows packaging 和主动诊断工具全部移除 forced aligner；ASR 只做 text-only，字幕时间轴由 Boundary chunk 等比分配生成。旧 forced 代码、silver mining、fallback/sentinel 诊断、forced 对比审计生成器和相关测试已归档到 `agents/rm/20260618_191036_forced-aligner-retired-code/`；历史 ASR diagnostics 工具已归档到 `agents/rm/20260618_193224_retired-asr-diagnostics-tools/`；为 forced 匹配保留的 `align_text` 双文本策略、旧日志解析和 batch/env 参数也不再保留。
- 历史记录 2026-06-10 v5 断兼容审计结论：当时 active runtime 升级为 **Boundary Refiner v5 core-only**，链路为 `SpeechBoundary-JA frame scores -> boundary candidates -> frame-sequence Mamba2 refiner -> candidate_sequence_core_planner_v5 -> ASR speech-core chunks`。这条路线已在 2026-06-22 被 scorer v4 + Boundary Refiner v6 edge-only 重构 supersede。
- 历史记录 2026-06-10/11 Boundary Refiner v5 registry 快照：当时 v5 checkpoint 由后端 registry 按当前 ASR repo id 自动选择，`BOUNDARY_REFINER_MODEL_PATH_BY_REPO` 仅作为实验覆盖项，checkpoint schema 要求 `boundary_refiner_v5`、`metadata.runtime_adapter=frame_sequence_v1` 和 repo id 匹配。该快照已被 2026-06-22 scorer v4 + Boundary Refiner v6 edge-only 断兼容重构 supersede，active registry 现在使用 `boundary_edge_refiner_v6.<repo-tag>.pt`，不再加载 v5 checkpoint。
