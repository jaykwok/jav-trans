...
- 2026-07-04 Windows run-log 深路径修复：真实验收首次因任务名重复进入目录和文件名、超过 legacy MAX_PATH 而 `FileNotFoundError`。文件名不再重复 job id，并按完整绝对路径动态压缩 job/backend 组件；长目录回归测试通过，最终 sample-c 长任务名正常写日志。
