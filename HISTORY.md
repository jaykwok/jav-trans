...
- 10k+ sequence dataset 不能把 JSONL 先全量解析成 Python `list[dict]` 再转 tensor；32768 hard-negative 的 `3.5G` JSONL 曾直接吃满 8GB RAM + swap 并卡崩。训练入口必须使用流式扫描 / 紧凑 tensor / batched evaluation。
