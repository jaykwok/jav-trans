...
- 2026-06-10 断兼容审计结论：active runtime 已回到 **Boundary Refiner v4 core-only**。当前链路是 `SpeechBoundary-JA frame scores -> boundary candidates -> frame-sequence Mamba2 refiner -> candidate_sequence_core_planner_v4 -> ASR speech-core chunks`。运行时不再使用 DP/Viterbi planner、不再使用 learned padding/context budget，也不再保留 gap-only/per-gap refiner loader 或 heuristic fallback。
- Boundary Refiner v4 checkpoint 固定为 `src/boundary/checkpoints/boundary_refiner.pt`，schema 必须是 `boundary_refiner_v4`、`output_dim=3`、`metadata.runtime_adapter=frame_sequence_v1`。`BOUNDARY_REFINER_MODEL_PATH` 为空时等价于源码默认 checkpoint；禁用 refiner 必须显式 `BOUNDARY_REFINER_ENABLED=0`，不再把空路径解释为旧 heuristic fallback。
- `merge_score` / `boundary_merge_prob` 只保留为相邻 island 的模型诊断、训练标签和审计依据；runtime 不再跨 speech island 合并 ASR chunk。`pack_speech_segments()` 只 materialize planner 输出的单个 refined speech core，并应用 `start_delta/end_delta`。
- active tree 审计结果：`BOUNDARY_DP_*`、`BOUNDARY_PLANNER_START_WEIGHT`、`BOUNDARY_CONTEXT_*`、`BOUNDARY_PLANNER_MAX_PADDED_CHUNK_S`、`speech_left_padding_s/right_padding_s`、`sequence_context_targets`、`build_refiner_gap_dataset.py`、`load_boundary_refiner()`、`RefinerInput`、`HeuristicBoundaryRefiner` 均已从当前代码/测试/配置入口移除。保留的 `start_weighted_*` 是评测指标，保留的 `merge_positive` 是训练标签，不是 runtime merge 策略。
...
- v4 core-only 匿名样片 A GPU downstream 已完成，证明当前短 speech-core 路线可全片跑通且没有回到 20-30s 粗 fallback；主要残留瓶颈转为字幕密度、nonlexical/短促人声、repeat loop QC、forced-aligner sentinel/proportional fallback 和少量极短重叠。下一步优先做审计/诊断与 silver-label mining，再决定是否训练 display-boundary correction 或引入 ASR/QC dense reward。Unified Joint Model 仍在 backlog。
