...
- 2026-06-30 Qwen ASR 参数收敛：移除项目自造的 `ASR_HEAD_CONTEXT` / `ASR_HEAD_CONTEXT_MAX_START_S`，避免“视频开头专用提示词”与 Qwen ASR 官方 wrapper 参数混在一起。当前固定短语只走 `ASR_CONTEXT`，并作为 Qwen `Qwen3ASRModel.transcribe(context=...)` 传入；语言提示继续由 `ASR_LANGUAGE` 映射到 `language`，生成长度由 `ASR_MAX_NEW_TOKENS` / `TRANSCRIPTION_MAX_NEW_TOKENS` 映射到 Qwen 初始化/转录的 `max_new_tokens`，时间戳请求固定为 `return_time_stamps=False`。本地 `.venv` 中 `qwen_asr` 源码未发现 `hotword/hotwords` 入参，当前不新增伪 hotword API。验证：`py_compile src/asr/transcribe.py src/asr/checkpoint.py src/main.py src/core/config.py src/asr/local_backend.py`，`pytest tests/test_asr_context_runtime.py tests/test_asr_checkpoint_backend_key.py tests/test_asr_stage_env_scope.py tests/web/test_jobs_api.py tests/test_config.py -q`，39 passed。
