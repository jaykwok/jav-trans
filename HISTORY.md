...
- 1.7B CueQC v4 binary：fresh scorer v4 + Boundary Refiner v6 + 1.7B ASR no-CueQC baseline 和 cold-start 审计页已生成；等待人工导出 `cueqc_cluster_labels.jsonl` 后，编译 v4 binary seed feature bundle 并训练首版 checkpoint。后续每轮按 false-drop/uncertainty/风险桶/随机混合抽约 200 条人工审计，再训练。REAL-988 保持最终测试片。
