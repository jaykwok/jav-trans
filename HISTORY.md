...
-> ASR QC / forced alignment / subtitle timing polish
```
...
- ASR 非词声音显示策略：Galgame SFT 已明显改善正常对白，当前问题优先按“识别结果可用但不适合全部显示”处理。下一步从三部现有全片抽约 300 条呻吟/喘息、亲吻拟声、重复 vocalization、短正常对白和相邻混合语音，盲审 `SFT better / original better / both usable / both bad`，并另标 `应该显示 / 应该省略 / 应该压缩`。第一轮先在字幕 cue 生成前增加可读性 display policy，保留 `raw_text`、原始 `display_text` 和 alignment 诊断，不改 ASR 转写本身；决策输出为 `display_decision=keep/drop/compact/review`。特征只使用时长、字符数、假名占比、字符多样性、重复 profile、QC、nonlexical、前后 cue 距离、连续同类 cue 数和是否含稳定词汇，不引入具体词黑名单。孤立且承担情绪/剧情作用的短促人声默认保留；连续重复且没有新增信息的 cue 才压缩或省略。若审计确认是 SFT 过度转写，再构建 hard-negative 空文本/统一非词标签数据，采用 LoRA 或局部低学习率修正，并混入 Galgame 对白 replay，避免 full SFT 灾难性遗忘。
- CueQC cluster-first 路线：Boundary Mamba 继续只做切分；新增 CueQC Mamba 作为独立 ASR/QC/内容分类/显示建议模型。工作流固定为 `Boundary切分 -> ASR文本 -> CueQC/聚类特征导出 -> CueQC Mamba -> 可选Aligner -> 字幕显示策略`。第一阶段不把 `content_type` 写死为最终 taxonomy，而是先导出 `cueqc_candidates.jsonl` 并聚类约 300 条样本，人工审计簇代表样本后再把稳定簇合并成少量标签。标签分两层：内容 `dialogue/non_dialogue/mixed/uncertain` 与决策 `keep/drop/compact/review` 独立，不能由内容标签硬推导。CueQC 输出 schema 固定为 `qc_decision`、`content_type`、`cluster_id`、`alignment_policy`、`display_hint`、`confidence/reasons/model_version`；第一版 shadow-only，只写审计字段，不改变 aligned segments/SRT。pre-align skip 只在人工 gate 后启用，高置信低信息才允许 `skip_align_fallback`；模型不可用或 uncertain 时全部保守进 aligner。首版聚类直接启用离线高维 text/audio embeddings：默认 `BAAI/bge-m3` 生成文本 embedding、`rinna/japanese-hubert-base` 生成音频 embedding，再与结构化 CueQC 特征融合后走 UMAP + HDBSCAN；FINCH-style first-neighbor hierarchy 保留为无依赖 fallback 和对照审计后端。embedding/UMAP/HDBSCAN 只进入离线候选构建与聚类审计，不进入实时 ASR 主链路；聚类只负责发现分布和提高审计效率，不直接生成 hard reject 训练目标。
- ASR/QC dense feedback：当前 Qwen 后端没有稳定 token logprob / entropy 输出，也没有参考文本可计算真正 local CER；第一阶段只使用现有可观测信号。等 preference-supervised v5.1 稳定后，再考虑 token confidence、reference-free ASR quality、dense reward / DPO / RL。
...
- 2026-06-15 CueQC embedding-first 聚类升级：首版离线聚类不再等待后续增强，直接加入高维文本和音频 embedding。新增 `tools/asr/cueqc/enrich_embeddings.py`，默认文本模型 `BAAI/bge-m3`、音频模型 `rinna/japanese-hubert-base`，输出写入候选行的 `embeddings.text` / `embeddings.audio`，并保留 `embedding_meta` 便于审计模型版本和维度。`cluster_candidates.py` 默认 `auto` 现在解析为 fused structured+dense feature space + UMAP reducer + scikit-learn HDBSCAN，并自动扫描 `min_cluster_size` / selection method，在无依赖或无稳定簇时仅 `auto` 回退到 FINCH；显式 `--method umap_hdbscan` 会失败报错，避免静默降级。新增依赖 `scikit-learn>=1.9`、`umap-learn`、`sentence-transformers`；这些模型和依赖只用于离线候选 enrichment / clustering，不改变 realtime ASR、aligned segments 或 SRT。验证：`uv run python -m py_compile tools/asr/cueqc/cluster_candidates.py tools/asr/cueqc/enrich_embeddings.py` 通过；`uv run python -m pytest tests/test_cueqc_cluster_first.py --basetemp=agents/temp/pytest_cueqc_embeddings` 为 `6 passed`。
