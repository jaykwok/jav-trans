...
-> forced alignment / subtitle timing polish
```
...
当前运行时只规划 speech core。默认 checkpoint 为 `src/boundary/checkpoints/boundary_refiner.pt`，schema `boundary_refiner_v5`，head 只输出 `start_delta/end_delta`。运行时不跨 island merge，不学习或应用 ASR padding/context budget，不暴露 refiner disable/backbone/threshold 兼容开关。
...
- `forced-aligner 指标硬 gate` -> `观察 + silver source`：Qwen3-ForcedAligner 未做目标域 finetune，fallback/sentinel 不再作为硬 gate；forced 成功样本可挖 silver labels，但必须经过人工审计 gate。
- `自动指标替换默认` -> `人工观感 gate`：silver-ft01 自动指标更好但人工确认观感和准确度变差，因此不采用为默认。
- `旧规则 ASR QC` -> `CueQC v3-Fusion 二元路由`：旧规则 QC 不再运行、不再观测、不再作为候选排序或质量报告信号；最终是否丢弃由 CueQC 模型的 `keep/drop` 决策负责，模型缺失或异常时保守 keep。
...
- true v5 绝对质量 gate：当前默认 true v5 是 synthetic-only 32768 hardmix，从零训练 delta-only head，尚未使用 real-domain silver mining 做 finetune。后续不再恢复或对照 v4，先审当前 v5 本身的字幕边界质量。
- v5 boundary preference pilot：第一阶段 120 条盲测已完成并通过 gate。试点包含 108 条唯一样本和 12 条隐藏重复，覆盖短/中/长三部片、start/end 两轴、方向和偏移档位；hidden duplicate consistency `83.3%`、usable `119`、decisive ratio `72.3%`、challenger wins `37`、扰动类别覆盖 `8/8`。
- v5.1 preference finetune：第一阶段 gate 已通过，77 条 unique decisive winner 已编译为现有 `boundary_refiner_frame_sequence_dataset_v5` 的绝对 delta target，只监督被比较轴，另一轴 weight 为 `0`。下一步先扩充到至少 300 条有效偏好，再从当前 v5 初始化、冻结 Mamba2 backbone、低学习率训练 delta head，并混入 synthetic anchor replay；不能直接用当前 77 条替换默认 checkpoint，完整全片人工 gate 通过前也不替换默认 checkpoint。
- real-domain forced-aligner silver 暂缓：旧 silver-ft01 已证明 forced-aligner 指标改善可能伴随人工观感和准确度下降。第一轮 v5.1 不混入 forced-aligner silver；forced alignment 仅作为候选排序、诊断和审计信号。偏好路线验证后，才重新评估经过人工过滤的 silver 是否有附加价值。
- CueQC v3-Fusion 二元路由：CueQC 是旧规则 QC 的替代，而不是并行 shadow。当前目标是 `Boundary切分 -> ASR文本 -> CueQC v3-Fusion -> keep/drop -> aligner/字幕`，只保留 `display_hint=keep/drop` 这个训练和运行目标；不再打 `content_type`、`qc_decision`、`alignment_policy`，也不再输出 compact/review 作为模型监督。CueQC 输入采用 ASR encoder features、ASR token trace、decoder aggregate stats 与 structured metadata；不引入 BGE、sentence-transformers、HuBERT、UMAP/HDBSCAN/FINCH 或额外音频 embedding 模型作为默认依赖。
...
- CueQC 阶段 2 自训练：CueQC v3-Fusion 先用 300 条 cold-start 种子保守启动，之后对 10 部全片候选输出高置信 `drop/keep`，人工抽检 false drop 风险，扩充训练集并重训 v3-Fusion。核心安全指标是 keep recall / false drop rate；宁可欠删，也不能误删有效字幕。
- CueQC 阶段 3 Boundary 反哺：后续可把 `display=drop`、过碎短切片和无效切分模式沉淀成 Boundary preference / hard-case 数据，用于识别短噪声、无效 speech island 或应合并的边界；这一步是离线偏好训练，不把 CueQC 结果耦合进 Boundary runtime。
...
- Boundary Refiner v5 checkpoint 固定为 `src/boundary/checkpoints/boundary_refiner.pt`，schema 必须是 `boundary_refiner_v5`、`output_dim=2`、`metadata.runtime_adapter=frame_sequence_v1`。v5 head 只输出 `start_delta/end_delta`；`BOUNDARY_REFINER_THRESHOLD`、`BOUNDARY_REFINER_ENABLED`、`BOUNDARY_REFINER_BACKBONE`、`sequence_labels`、`merge_positive`、`split_negative`、`boundary_merge_prob`、`boundary_split_prob` 和 `boundary_decision_merge` 均已从 active code / config 移除。pipeline 必须加载 canonical `transformers.Mamba2Model` v5 checkpoint；`SPEECH_BOUNDARY_JA_EXPORT_SEQUENCE_FEATURES` 仅是 pipeline/exporter 内部特征导出通道，不进入 boundary-cache signature。
...
- 2026-06-17 CueQC v3-Fusion false-drop 审计完成并生成 Stage 2a 训练包：用户导出 `agents/audits/20260617_130642_cueqc-v3-false-drop-audit/cueqc_false_drop_audit_labels.jsonl`，共 `200` 条，人工决策 `drop_ok=178 / false_drop_keep=21 / uncertain=1`，裸 false-drop rate `10.5%`；按全量 `drop>=0.85` population 分层加权估计 false-drop 约 `4.2%`，仍然不满足“直接接收 14588 条高置信 drop pseudo labels”的安全条件。新增 `tools/asr/cueqc/compile_stage2a_features_v3_fusion.py`，规则固定为：人工审计标签覆盖 cold-start 冲突，`drop_ok -> drop=0`，`false_drop_keep -> keep=1`，`uncertain` 跳过，高置信 keep pseudo labels 纳入，未审高置信 drop pseudo labels 全部跳过。已生成 `agents/temp/20260617_143121_cueqc-v3-stage2a-training-features/cueqc_stage2a_train_features_v3_fusion.pt`：`538` records，labels `drop=344/keep=194`，来源为 `298` cold-start seed、`178` 人工确认 drop、`21` 人工纠正 keep、`41` 高置信 keep pseudo，`14588` 未审 drop pseudo 已跳过。下一步先用该 Stage 2a 包重训 CueQC v3-Fusion，再重新跑 10-film prediction 和 false-drop/keep-recall gate；不要把未审 drop pseudo labels 直接喂回训练。`drop_ok` 的 `178` 条可以作为后续 Boundary hard-negative / preference finetune 候选，但当前不编译进 Boundary 训练，等 CueQC Stage 2a 闭环评估后单独做 Stage 3。
- 2026-06-17 CueQC v3-Fusion Stage 2a 训练完成但仍需人工 gate：使用 `agents/temp/20260617_143121_cueqc-v3-stage2a-training-features/cueqc_stage2a_train_features_v3_fusion.pt` 训练，首次参数因 `hidden_size * 2 != num_heads * head_dim` 失败，无产物；修正为 `hidden_size=64/head_dim=32/num_layers=1/dropout=0.3/lr=5e-4/weight_decay=1e-2/label_smoothing=0.15/keep_weight=1.5` 后输出 `agents/temp/20260617_143911_cueqc-v3-stage2a-train/cueqc_mamba_v3_fusion.pt`。固定 holdout `匿名样片 D` 指标：`accuracy=0.84`、`keep_recall=0.9375`、`false_drop_rate=0.0625`、`drop_precision=0.9643`、`drop_recall=0.7941`，按默认 `drop_threshold=0.85` 仍不达替换默认 checkpoint 的安全条件。阈值扫描显示 `drop_threshold=0.88` 可使 holdout `keep_recall=1.0/false_drop_rate=0.0`，代价是 `drop_recall=0.618`，符合“宁可欠删”的方向；用 t=0.88 重跑 10-film 预测输出 `agents/temp/20260617_144200_cueqc-v3-stage2a-10film-predictions-t088/`，records `45643`，预测 `keep=35936/drop=9707`，高置信 pseudo `11214`（`drop=9707/keep=1507`）。已生成新的人工抽检页 `agents/audits/20260617_144327_cueqc-v3-stage2a-t088-false-drop-audit/index.html`，抽样 `200` 条，音频和字幕无缺失；在用户完成该轮 t=0.88 false-drop 审计前，不替换 `src/asr/checkpoints/cueqc_mamba_v3_fusion.pt`。
- 2026-06-17 CueQC v3-Fusion Stage 2b 继续自训练但仍等待第三轮人工 gate：t=0.88 审计结果 `200` 条中 `drop_ok=197/false_drop_keep=3`，裸 false-drop rate `1.5%`，比 Stage 2a 前的 `10.5%` 明显下降；3 个误删样本为 `匿名样片 G ...はい`、`匿名样片 B あ...`、`匿名样片 K あっ...`。阈值扫描显示 t=0.92 可避开这 3 条但全量仅剩 `337` 个 drop，收益过低，因此不直接调阈值替换默认。`tools/asr/cueqc/compile_stage2a_features_v3_fusion.py` 已扩展为可重复传 `--false-drop-audit-labels`，用于合并多轮人工审计；用前两轮审计 + t=0.88 的高置信 keep pseudo 编译 `agents/temp/20260617_161704_cueqc-v3-stage2b-training-features/cueqc_stage2b_train_features_v3_fusion.pt`，共 `2177` 条，labels `drop=532/keep=1645`，其中 `375` 人工 drop_ok、`24` 人工 false_drop_keep、`1504` 高置信 keep pseudo，`9707` 未审 drop 仍跳过。Stage 2b 训练输出 `agents/temp/20260617_161806_cueqc-v3-stage2b-train/cueqc_mamba_v3_fusion.pt`，固定 holdout `匿名样片 D` 指标 `accuracy=0.9118/keep_recall=1.0/false_drop_rate=0.0/drop_precision=1.0/drop_recall=0.8475`，val 同样 `keep_recall=1.0/false_drop_rate=0.0`。回放两轮已审计样本时，24 个 `false_drop_keep` 全部不再被 drop；但 10-film 全量预测 `agents/temp/20260617_162231_cueqc-v3-stage2b-10film-predictions/` 在默认 t=0.85 下变为 `drop=19456/keep=26187`，比 Stage 2a 更激进，必须再抽检新分布。已生成 `agents/audits/20260617_162350_cueqc-v3-stage2b-false-drop-audit/index.html`，抽样 `200` 条且音频/字幕无缺失；在用户完成该轮审计前，不替换默认 `src/asr/checkpoints/cueqc_mamba_v3_fusion.pt`。
- 2026-06-17 CueQC Stage 2b false-drop 审计页标签补充：用户指出“呼吸声”不属于现有 `语气词/呻吟`、`环境/噪声` 等分类，已在 `tools/audits/generate_cueqc_prediction_audit_html.py` 的 reason tag 选项中新增 `breath / 呼吸声`，并写入审计 `summary.json` 的 `reason_tag_options`。当前待审计页面 `agents/audits/20260617_162350_cueqc-v3-stage2b-false-drop-audit/index.html` 已用同一 predictions、同一 seed、同一抽样参数原地重生成；当时目录内尚无 `cueqc_false_drop_audit_labels.jsonl`，因此未覆盖人工标签。
- 2026-06-17 CueQC v3-Fusion Stage 2b gate 通过并替换默认 checkpoint：第三轮 Stage 2b false-drop 审计 `agents/audits/20260617_162350_cueqc-v3-stage2b-false-drop-audit/cueqc_false_drop_audit_labels.jsonl` 共 `200` 条，`drop_ok=198/false_drop_keep=1/uncertain=1`，裸 false-drop rate `0.5%`；唯一误删为 `匿名样片 J chunk02280 "...私"`，`p_drop=0.864778`，属于 `short_text` 最低置信桶。三轮合计 `600` 条人工审计为 `drop_ok=573/false_drop_keep=25/uncertain=2`。基于回放，新增 CueQC 自适应阈值校准层 `src/asr/cueqc_thresholds.py`：默认基础阈值仍 `0.85`，但 `text_bucket=short_text` 阈值提升到 `0.87`；profile 只允许把阈值抬高，不允许低于 base threshold，避免变成更激进的删除规则。该 profile 在三轮已审计样本上使 `25` 个 `false_drop_keep` 全部保留，同时保留 `573` 个 `drop_ok` 中 `540` 个 drop；10-film 全量预测 `agents/temp/20260617_174344_cueqc-v3-stage2b-adaptive-10film-predictions/` 为 records `45643`、`drop=19380/keep=26263`、高置信 pseudo `drop=19380/keep=1372`。默认 checkpoint 已替换为 Stage 2b + adaptive profile：`src/asr/checkpoints/cueqc_mamba_v3_fusion.pt` sha1 `98f9631a63dc19736b50619100fb4be4d08075e8`，旧默认备份到 `agents/temp/20260617_174154_cueqc-default-checkpoint-backup/cueqc_mamba_v3_fusion.before-stage2b-adaptive.pt`。runtime 与 `predict_v3_fusion.py` 共用同一 `resolve_drop_threshold()`，每条模型决策会记录实际 `drop_threshold` 和 `threshold_profile`，方便审计回放。
- 2026-06-17 CueQC v3-Fusion Stage 2 全量预测完成，进入 false-drop 人工抽检 gate：正式目录 `agents/temp/20260617_113159_cueqc-v3-10film-sharded-features/`，按 1000 条/片提取 46 个 shard（最后一片 643 条），`RUN_DONE.txt` 时间 `2026-06-17 13:00:31+08:00`；完整性校验 `shards=46`、`total=45643`、`labels_unlabeled=45643`、无 schema/长度错配。合并输出 `cueqc_full_features_v3_fusion.pt`，大小约 `3.02GB`。预测输出 `agents/temp/20260617_130642_cueqc-v3-10film-predictions/`：records `45643`，预测 `keep=31055/drop=14588`，高置信伪标签 `14629`（`drop=14588/keep=41`）。因为高置信 drop 占比约 `32%`，不符合 cold-start 直接自训练的安全假设，当前禁止直接把这些 pseudo labels 喂回训练；必须先人工抽检 false-drop 风险。已新增 `tools/audits/generate_cueqc_prediction_audit_html.py`，生成 false-drop 审计页 `agents/audits/20260617_130642_cueqc-v3-false-drop-audit/index.html` 并刷新导航；抽样 200 条高置信 drop，按 `video_id + drop_confidence_bin + text_bucket` 分层，覆盖 10 部片，音频和字幕无缺失。人工标签 schema 为 `cueqc_false_drop_audit_label_v1`，主决策 `drop_ok / false_drop_keep / uncertain`；用户审完导出 `cueqc_false_drop_audit_labels.jsonl` 后，再决定是否调阈值、只接收部分 pseudo labels，或回到 cold-start 标签/模型结构修正。
- 2026-06-17 CueQC v3-Fusion 全量抽取前性能修复：正式 Stage 2 全量池仍是 10-film `45643` 条候选，且仍按 `--start-index/--max-samples` 分片提取；但 smoke 发现原离线提取每个 chunk 都会重新从磁盘读取同一部长 `.wav`，20 条就耗时约 `23s`，放大全量会造成不必要 I/O。`extract_features_v3_fusion.py` 现在按视频缓存已解码 wav 数组（`--audio-cache-size`，默认 `1`），同一分片内只读一次当前视频音频，再传 numpy array 给 `AsrInternalsCapturer.extract()` 按窗口切片；同时关键进度日志改为 `flush=True`，避免长跑日志不落盘。补充测试覆盖同一视频两条候选只加载一次音频，`py_compile`、`tests/test_cueqc_v3_stage2.py` 和 `git diff --check` 通过（仅 CRLF warning）。误手写的非真实时间前缀 smoke 目录 `agents/temp/20260617_120000_cueqc-v3-10film-sharded-features/` 已按删除约定移动到 `agents/rm/20260617_112851_invalid-temp-prefix/`；后续正式产物继续用 `Get-Date -Format yyyyMMdd_HHmmss` 生成前缀。
- 2026-06-17 CueQC v3-Fusion 全量特征提取改为分片执行：提交保存点 `b081717 Prepare CueQC v3 fusion self-training` 后，先用 `extract_features_v3_fusion.py --input --max-samples 10` 对 10-film 全量候选池做 smoke，10/10 成功、labels_unlabeled=10；随后尝试直接提取 45643 条到单个 `.pt`，但该模式没有中间保存点，进程被工具超时遗留且 stdout 缓冲不刷新，长任务失败成本过高，因此停止单进程方案。新增 `--start-index` 支持按行分片提取，新增 `tools/asr/cueqc/merge_features_v3_fusion.py` 合并分片并校验 feature_config；docs v3 的 Stage 2 命令同步改为 shard -> merge -> predict。后续全量 Stage 2 一律按分片跑，避免 40k+ 样本提取中途失败后重头开始。
- 2026-06-17 CueQC v3-Fusion Stage 2 训练准备完成：对照 `docs/cueqc_mamba_v3_plan_extracted.md`，下一轮不能直接重跑 300 条 cold-start 训练，而应先对 10-film 全量候选池输出高置信伪标签并人工抽检 false drop。为此新增 `tools/asr/cueqc/predict_v3_fusion.py`：读取 `cueqc_mamba_v3_fusion_features` bundle + checkpoint，导出 `cueqc_predictions.jsonl`、`cueqc_pseudo_labels.high_conf.jsonl` 和 `summary.json`；`extract_features_v3_fusion.py` 增加 `--input` 支持无标签 `cueqc_candidate_v1` 全量候选，未标注样本编码为 `label=-1`；`train_mamba_v3_fusion.py` 现在拒绝含无标签样本的特征包，防止误把预测池直接训练。docs v3 已补 Stage 2 可执行命令。准备 smoke 使用现有 300 条特征 + 当前 checkpoint 在 CPU 跑通，输出 `agents/temp/20260617_111200_cueqc-v3-predict-smoke/`：records `300`，预测 keep/drop `218/82`，高置信伪标签 `131`（drop `82`、keep `49`）。下一步顺序固定为：先提交当前准备保存点，再提取 10-film 全量 45643 条 v3 特征，跑 `predict_v3_fusion.py`，人工抽检高置信 drop 的 false-drop 风险，通过后再合并 pseudo labels 并重训 CueQC v3-Fusion。
- 2026-06-17 workflow runner 目录整理：`run_full_workflow.py` 已从 `tools/boundary/ja/` 移到 `tools/workflows/run_full_workflow.py`，因为它是端到端闭环 runner，不属于 Boundary-JA 训练/研究工具；测试 import 同步改为 `from tools.workflows import run_full_workflow`，旧路径不保留兼容 shim。`make_paths()` 现在会给未带时间前缀的 `--task-name` 自动补本地时间 `YYYYMMDD_HHMMSS_`，已带前缀的任务名保持原样，避免新运行继续生成无法判断时间的 `agents/temp/speech-boundary-ja/<task>` 目录。本轮工作目录整理只移动可再生生成物：`tmp/` 中除 `.gitkeep` 外的旧 cache/chunks/web/checkpoint/pytest 临时内容，以及 `src/`、`tools/`、`tests/` 下的 `__pycache__`，归档到 `agents/rm/20260617_104211_workspace-tidy/` 和 `agents/rm/20260617_104439_workspace-tidy-remaining/`；源码、模型、视频、dist、docs 不做扩大清理。
- 2026-06-17 旧规则 ASR QC 断兼容退役：用户确认“旧 QC 决策直接删除，不观测不运行”。active tree 已移除 `src/asr/qc.py`、`src/asr/qc_stage.py` 和旧 ASR QC 专属测试；pipeline 不再调用 `_run_TRANSCRIPTION_qc` / `collect_adaptive_precision_review`，quality report、alignment quality、boundary preference pilot、silver mining 和审计页不再读取或展示 `asr_qc_*` / `asr_review_uncertain`。CueQC v3-Fusion 成为唯一 ASR 后保留/丢弃路由，输出只保留 `display_hint=keep/drop`；模型不可用或异常时 fallback keep。`tools/asr/cueqc/compile_training_set.py` 只接受簇级 `cueqc_cluster_labels.jsonl`，per-sample `cueqc_manual_labels.jsonl` 和 `content_type/qc_decision/alignment_policy` 旧标签头不再兼容。旧 `generate_asr_attribution_audit_html.py` 因按旧 QC 分桶已移到 `agents/rm/20260617_101851_retired-asr-attribution-qc-audit/`。后续路线固定为：Torque 聚类只用于第一次 300 样本粗标签 bootstrap；阶段 2 用 CueQC 高置信 drop/keep 自训练 + 人工抽检 + 扩充训练集 + 重训；阶段 3 可把 `display=drop` 与过碎/无效切分模式离线反哺 Boundary preference。
- 2026-06-14 ASR 非词声音可读性与 forced-aligner 去留判断：人工审计观察到 forced aligner 时间轴与 SpeechBoundary-JA / Boundary Refiner 产出的 speech core 窗口已经接近，说明当前 v5 core window 基本找准了，低信息/重复人声问题更像显示策略问题，而不是继续放大窗口或重新依赖 forced aligner 的问题。当前代码已有 `ALIGNMENT_TIMESTAMP_MODE=forced/native/hybrid`，并且 Web model requirements 在 `native` 模式下不会要求 `forced_aligner`；但默认配置仍是 `forced`，README 与 Windows 打包仍把 `Qwen/Qwen3-ForcedAligner-0.6B` 作为分发模型。因此本轮不直接删除 forced aligner：它继续保留为 word-timing polish、候选排序、诊断审计和未来 teacher/silver source；若要节约空间，应先做同片 `forced` vs `native`/`hybrid` A/B，确认字幕时间轴、fallback、QC、人工观感没有明显退化后，再新增 lite/no-aligner 打包档或把默认切到 native。删除默认 bundled aligner 前必须同步 `DEFAULT_SETTINGS["ALIGNMENT_TIMESTAMP_MODE"]`、Web model requirements 测试、README/packaging 文档和 `packaging/prepare_default_model.py` / `packaging/javtrans-web.spec`，不能只删模型目录。
- 2026-06-14 CueQC cluster-first 第一阶段落地：新增 `src/asr/cueqc.py`、`tools/asr/cueqc/export_candidates.py`、`cluster_candidates.py`、`compile_training_set.py` 和 `train_mamba.py`。runtime 在 ASR text/QC 后、forced aligner 前运行 CueQC shadow，输出 `asr_details.cueqc_shadow` 并把 chunk/segment shadow 摘要透传到 `transcript_chunks` / aligned segments；默认 `CUEQC_SHADOW_ENABLED=1`、`CUEQC_PREALIGN_SKIP_ENABLED=0`，因此当前不跳过 aligner、不改变字幕结果。完整候选特征默认不嵌入 aligned cache，需通过 `CUEQC_EXPORT_CANDIDATES_PATH` 或离线 `export_candidates.py` 生成 `cueqc_candidates.jsonl`，避免全片 cache 膨胀；若确需嵌入可设 `CUEQC_SHADOW_EMBED_CANDIDATES=1`。aligned cache signature 升到 `version=7`，ASR runtime signature 升到 `version=3` 并包含 CueQC policy、taxonomy version、model version、checkpoint hash、prealign policy 与 embed-candidates policy。聚类工具输出 `cueqc_clusters.jsonl`、`cueqc_cluster_representatives.jsonl`、`cueqc_cluster_summaries.jsonl` 和 `cluster_audit.html`；人工标签编译器强制低一致性簇降级到 `uncertain/review/align`，不会生成 hard reject 训练目标。`train_mamba.py` 只是多头 Mamba2 训练脚手架，torch/transformers/boundary backbone 延迟导入；正式训练和 pre-align skip 启用仍等待人工 gate。
- 2026-06-15 CueQC embedding 增强暂缓：曾尝试把 `tools/asr/cueqc/enrich_embeddings.py` 作为离线增强入口，默认文本模型 `BAAI/bge-m3`、音频模型 `rinna/japanese-hubert-base`，但实测暴露三类成本/稳定性问题：`sentence-transformers` 在当前 Windows 环境触发 `torchcodec` DLL 兼容问题，`BAAI/bge-m3` 需要额外下载和缓存，`rinna/japanese-hubert-base` 当前 Hugging Face 返回 `401/Repository Not Found`。因此 embedding 工具保留为显式可选增强，不再作为第一阶段默认路线；`requirements.txt` 默认移除 `sentence-transformers` 和 `umap-learn`，只保留 `scikit-learn>=1.9` 支撑本地 HDBSCAN/PCA/无降维聚类。`cluster_candidates.py --method auto` 改为低依赖 structured-first 的 sklearn HDBSCAN（默认 `--reducer none`）并保留 FINCH fallback；显式 `--method umap_hdbscan` 仍可在用户自行安装 `umap-learn` 后作为质量对照。验证：`uv run python -m py_compile tools/asr/cueqc/cluster_candidates.py tools/asr/cueqc/enrich_embeddings.py` 和 `uv run python -m pytest tests/test_cueqc_cluster_first.py --basetemp=agents/temp/20260615_152934_pytest_cueqc_lowdep`。
- 2026-06-15 FINCH 对照复盘：`agents/temp/20260615_152934_cueqc-10film-clusters-finch/` 产物是 `17` 个簇、`300/300` 全覆盖、`84` 个代表样本、无 noise；它适合做无依赖对照和 fallback，但因为会把所有点强行归类，低置信样本的人工审计负担更高，所以不把它提成默认审计入口。为避免 Python API 与 CLI 默认不一致，`cluster_rows()` 的默认 `reducer` 也同步改成 `none`，和 `--reducer none` 的低依赖路线一致。
- 2026-06-15 CueQC 第一轮 10-film baseline 候选池与 sample-a holdout 修正：最初误把 sample-a smoke 合入 11-film CueQC 审计池；复核后确认 匿名样片 A 应作为 smoke/holdout 测试锚点，10-film baseline run 才是第一轮候选池。已重新从 CueQC 合入前 baseline commit `5afe535` 的 10 部匿名全片 aligned artifacts（均 `skip_translation=True` 且保留 ASR chunks）导出 CueQC 候选池，产物 `agents/temp/20260615_152934_cueqc-10film-candidates/`。全量 `cueqc_candidates.full.jsonl` 共 `45643` 条，按视频/QC/文本密度/重复 run 分层抽样 `cueqc_candidates.sampled.jsonl` 共 `300` 条，10 部各 `30` 条；全量分布包括 `density_empty_or_punctuation=12037`、`density_short_vocalization_candidate=9759`、`density_short_kana_dialogue_candidate=7137`、`qc_warn=5839`、`normal_dialogue_candidate=7596`。structured-only UMAP 对照产物 `agents/temp/20260615_152934_cueqc-10film-clusters-structured/` 得到 `16` 个稳定簇、`1` 个 noise 组，`cueqc_clusters.jsonl=300` 行、`cueqc_cluster_representatives.jsonl=82` 行，mean cluster probability `0.937111`、noise `18/300=6%`；低依赖 no-reducer HDBSCAN 产物 `agents/temp/20260615_152934_cueqc-10film-clusters-structured-none/` 得到 `16` 个稳定簇、noise `80/300=26.7%`，可作为默认无 UMAP 审计入口但质量略低；PCA 版 noise `117/300=39%`，不优先。下一步人工标注应以 10-film structured audit 为训练/聚类池，匿名样片 A 只用于后续 holdout smoke/test；在人工标签导出前不运行 `compile_training_set.py` 或 `train_mamba.py`，也不启用 pre-align skip。
- 2026-06-15 CueQC 审计入口与清理：`agents/audits/index.html` 继续只作为导航页；已把当前第一轮低依赖审计入口发布到 `agents/audits/20260615_160304_cueqc-10film-structured-hdbscan-audit/index.html` 并重建 `latest-audit.html`，导航按时间倒序只显示该 CueQC 审计。旧审计目录 `true-v5-boundary-preference-pilot/`、`v5-subtitle-audio-audit/` 已移动到 `agents/rm/audit-deletions/20260615_160304_previous-audits/`；误合入 sample-a 的旧 11-film CueQC 候选池/聚类和 pytest 临时目录已移动到 `agents/rm/20260615_160304_cueqc-superseded-temp/`。保留当前有效 10-film 候选池、低依赖 HDBSCAN 审计源、UMAP/FINCH 对照源、baseline worktree 和 ASR chunks，避免丢失后续标注、对照和 holdout 验证所需数据。
- 2026-06-15 CueQC 审计页音频字幕增强：新增 `tools/audits/generate_cueqc_cluster_audit_html.py`，从低依赖 HDBSCAN `cueqc_clusters.jsonl` 重建当前 300 条审计页，并从 baseline root `agents/temp/20260615_094437_b5/agents/temp/speech-boundary-ja/20260615_094437_o10/` 自动映射 10 部视频的全片 `.wav`、`.ja.srt` 与 `aligned_segments.json`。当前审计页 `agents/audits/20260615_160304_cueqc-10film-structured-hdbscan-audit/index.html` 已生成 `subtitles/*.ja.vtt` 共 10 个，支持按当前候选条目切换对应音频、播放 chunk / 上下文、同步字幕覆盖层、chunk/fallback/context 字幕列表、aligned segments 列表，并保留原 CueQC 内容/显示/align/QC 标签和 `cueqc_manual_labels.jsonl` 导出 schema。校验：`uv run python -m py_compile tools/audits/generate_cueqc_cluster_audit_html.py`，HTML 内嵌 JSON 解析确认 `300` rows / `10` videos 且音频与字幕无缺失，内联 JS `new Function` 语法检查通过；`localhost:8765` 浏览器 smoke 确认页面初始化出 300 条列表、当前音频 source、VTT track 和字幕覆盖层。
- 2026-06-15 CueQC 审计标签交互优化：同一审计页的人工标签按钮改为中文显示，英文标签值保留在按钮 title、localStorage 和导出 JSONL 的主字段中，并额外导出 `*_zh` 与 `label_display` 方便人工复盘。预设中文标签为内容类型 `对话/非对话声/混合/不确定`、显示决策 `保留/丢弃/压缩显示/复核`、对齐策略 `正常对齐/跳过对齐 fallback/对齐复核`、质量判定 `可用/复核/拒绝`。自定义标签改为单个全局入口：先选择标签组，再输入中文名和英文标签值，点击“添加并选中”后立即写入当前样本并持久化为该组常用按钮，减少逐条审计操作步骤。校验：页面 HTML 数据检查、内联 JS 语法检查和 `localhost:8765` 浏览器只读 DOM smoke 均通过，确认 300 条列表、音频/VTT、中文按钮和自定义入口正常初始化。
- 2026-06-15 CueQC 审计标签语义收窄：本轮人工审计不再要求判断 forced aligner 取舍；`对齐策略`按钮和自定义组选项已从审计页移除，导出时默认写 `alignment_policy=align` / `alignment_policy_zh=默认保留 aligner`，表示现阶段继续保留 forced aligner，后续等 speech core 切分足够精准后再单独做 aligner 去留 A/B。`显示决策=丢弃` 改为终止型一键标签：点击后自动写 `display_decision=drop`、`content_label=non_dialogue`、`qc_decision=reject`、`terminal_decision=true`、`terminal_reason=display_drop` 并跳到下一条，避免丢弃项还要继续选择后续标签。校验：生成器 `py_compile`、HTML 数据检查、内联 JS 语法检查和浏览器只读 DOM smoke 均通过，确认页面只剩 `内容类型/显示决策/质量判定` 三组标签，音频和 VTT 仍正常初始化。
- 2026-06-15 CueQC 审计标签进一步收口：用户确认本轮只需要两个必选项，`声音类别` 与 `策略`；`质量判定` 只在策略不是 `丢弃` 时才需要。页面因此把 `内容类型` 改名为 `声音类别`，`显示决策` 改名为 `策略`，`质量判定` 在 `策略=丢弃` 时自动弱化并导出为 ignored，不再要求填写；`丢弃` 只需要先有声音类别，或者先点丢弃后再补声音类别，就会自动写终止标记并跳下一条。导出保持 `content_label`、`display_decision`、`alignment_policy=align` 默认保留 aligner，丢弃项附带 `qc_ignored=true`、`terminal_decision=true`、`terminal_reason=display_drop`，避免把呻吟/喘息这类明确丢弃项误记成质量失败。验证：重新生成审计页、`py_compile`、HTML 数据校验、内联 JS 语法检查和浏览器只读 DOM smoke 均通过。
- 2026-06-15 CueQC 审计声音类别固定：当前审计页切换到 `fixed_sound_categories_v1`，并用新的 localStorage key `fixed-sound-categories-v1` 重置旧已选人工标签；旧 `cueqc-cluster-audit:<pathname>`、自定义选项和自定义分组 key 会在页面加载时移除。声音类别固定为 `对话/dialogue`、`语气词/呻吟声/vocalization`、`环境声/environment`、`混合/mixed`、`不确定/uncertain`，不再提供自定义标签入口。`策略=丢弃` 仍只要求先完成声音类别，导出 `qc_ignored=true`、`needs_quality_label=false`、`qc_decision=""` 和 `alignment_policy=align`；非丢弃策略仍要求质量判定。`compile_training_set.py` 与 `train_mamba.py` 同步支持 `cluster_first_manual_v2` 的新内容标签和 `qc_decision=ignored`，低一致性簇仍会把 `drop/ignored` 降级为 `review`，避免不稳定簇生成硬丢弃训练目标。验证：`uv run python -m py_compile tools/audits/generate_cueqc_cluster_audit_html.py tools/asr/cueqc/compile_training_set.py tools/asr/cueqc/train_mamba.py`、`uv run python -m pytest tests/test_cueqc_cluster_first.py --basetemp=agents/temp/20260615_175520_pytest_cueqc_labels`、审计页 HTML/JS 数据检查和 `localhost:8765` 浏览器只读 DOM smoke 均通过；浏览器确认 300 条样本、5 个固定声音类别、自定义入口为 0、当前音频 source 与 VTT track 均存在。
- 2026-06-15 CueQC 短切片标签与 Boundary 反馈路线：人工审计发现 `cluster_00` 代表“切得很短、无法判断”的样本，已新增声音类别 `短切片/无法判断/short_unclear`，审计页 summary 升到 `fixed_sound_categories_v2`，训练编译和 Mamba 训练头升到 `cluster_first_manual_v3`。定位 `cluster_00` 共 `39` 条，10 部视频均有，duration `0.358-0.919s`、median `0.520s`，文本 `...` 为 `37` 条、`...!` 为 `2` 条；锚点 `匿名样片 D chunk875` 的原始 transcript chunk 为 `1510.926-1511.356s`、`0.430s`、`raw_text=…`，aligned cache 标记 `alignment_quality=nonlexical` / `fallback_subtype=nonlexical_text`。根因不是审计页或 forced aligner 二次切割，而是 boundary cache 中该位置本身是单独 `speech_island`：speech segment `1510.94-1511.36s`、score `0.542`，planner `boundary_planner_min_chunk_s=0.4` 允许它作为独立 ASR core chunk，前后 gap 分别约 `0.479s` 和 `0.004s`。后续可把人工确认的 `short_unclear` / `drop` / `review` 聚类样本连同 boundary cache `processing_spans`、相邻 chunk、speech segment score 和上下文音频收集为 Boundary hard cases，但不能直接把 CueQC 类别当边界监督；应先编成“保留 / 合并到左邻或右邻 / 抑制短 island / 仍需复核”的边界偏好或 hard-case 审计集，通过人工 gate 后再反哺 Boundary Refiner 或 planner 阈值策略。验证：`uv run python -m py_compile tools/audits/generate_cueqc_cluster_audit_html.py tools/asr/cueqc/compile_training_set.py tools/asr/cueqc/train_mamba.py`、`uv run python -m pytest tests/test_cueqc_cluster_first.py --basetemp=agents/temp/20260615_182832_pytest_cueqc_short_unclear`、审计页 HTML/JS 检查和 `localhost:8765` 浏览器 DOM smoke 均通过。
- 2026-06-15 CueQC 审计转为簇级自由标注：用户决定暂不把 17 个自然簇武断合并成固定类别，而是先让每个簇展示 2-3 个代表例子，由人工为每簇填写自定义 `cluster_label` 和 `classification_reason`。当前审计页新增 `cueqc_cluster_label_v1` 导出 `cueqc_cluster_labels.jsonl`，每行包含 dataset、cluster_id、簇统计、3 个代表样本和人工标签/原因；旧逐条固定标签保留为折叠可选区，不作为当前第一步必填项。后续 Codex 先分析人工写下的簇标签和分类原因，再讨论是否合并 taxonomy、是否反馈 Boundary hard cases 或训练 CueQC；在该分析前不运行标签编译、Mamba 训练或 pre-align skip。
- 2026-06-15 CueQC 审计布局继续调整为单簇浏览：用户反馈右侧 3 条代表音频不够，页面已改为左侧 `cluster_00 -> cluster_16` 的单列簇导航，右侧上方是当前簇的标签/原因输入，下面展示当前簇的全部音频并加了滚动容器，便于人工审计长簇和 noise 簇。`cluster_00` 对应原 `cluster_00`，最后一项 `cluster_16` 对应 `noise_00`；点击左侧任一簇会只切换右侧当前簇和完整音频列表。导出 `cueqc_cluster_labels.jsonl` 仍只保留每簇 3 个代表样本，页面展示与导出分离。当前审计页 summary 记录 `cluster_review_layout=left_nav_single_cluster_audio_list_all_v2`、`cluster_review_audio_render_mode=all`。
- 2026-06-16 CueQC 簇级标签初步归纳：`cluster_07` 已明确标成 `多人对话`；导出里页面左侧 `cluster_16` 对应的实际簇是 `noise_00`，它不是纯正常语句，而是明显混桶，里面同时有正常长句、环境音+对话、纯呻吟声和多人对话，后续应优先按 `混合/不确定` 处理。当前 17 簇先作为 taxonomy 草图，下一步把 `短噪声 / 杂音 / 呻吟声 / 正常语句 / 语气句 / 噪声-呻吟混合 / 带呻吟声的正常语句 / 多人对话 / 正常短句 / 呻吟声长句 / 短促重复呻吟声 / 重复正常语句 / 短促语句 / 重复正常短句 / 短促呻吟声 / 重复语气词 / 混合-不确定` 压成少量顶层类，再决定哪些只保留为子类或原因标签；当前不启用 pre-align skip，也不直接进训练。
- 2026-06-16 CueQC 聚类算法替换为 Torque Clustering（TORC）+ 双 Mamba / 时序聚类新方向：砍掉 `cluster_candidates.py` 里全部 HDBSCAN/FINCH/UMAP/PCA 链路和 `--method/--reducer/--min-clusters/--max-clusters/--min-cluster-size/--min-samples/--pca-*/--umap-*` 等参数，新增单文件 `tools/asr/cueqc/torque.py`（Jie Yang TORC 的 Python 移植，CC BY-NC-SA 4.0 署名保留，仅依赖 numpy/scipy/networkx），`cluster_rows()` 现在只接受 `metric/feature_space/representatives_per_cluster/detect_noise/adjustment_factor/merge_layer`。TORC 是参数无关算法：每个点连 1-NN 成初始簇→迭代中每个簇只向质量≥自己的最近邻簇连接→记录每次合并的 mass×distance²=torque。移植踩坑：(1) 原 reference 的 `Nab_dec` 阈值（mean−0.5·std）在重尾 torque 分布上塌缩；(2) cut 应用原本走脆弱的累积 `link_adj`，合并阶段没正确累积成生成树，合成 3-blob 上切之前就有 33 个分量（应 1），导致一度出现 84 簇/300 簇等明显 bug 结果。修复：cut 应用改为直接从记录的点对边（cutlink_all 第 2/3 列）重建生成树再切；合成 3-blob 验证完美恢复 3 簇（30/30/30 纯度）。但 10-film structured 特征（300 样本 17 维）上 torque-gap 最终树切（factor 调参）在 2-4 簇间剧烈漂移、不稳定——根因是 structured 特征 torque 重尾，最终树切依赖 factor。
- 2026-06-16 TORC 改用 merge_layer 取层次树中间层作为种子审计粒度（最终定案）：放弃在最终树上调 factor 切边，改为 `--merge-layer N` 直接取 TORC 合并层次树的第 N 层划分。10-film 的层次结构是 `76 → 7 → 1`（layer0=1-NN 初始细簇、layer1=一次合并、layer2=全合并），`--merge-layer 1` 得到稳定的 7 簇（152/72/45/12/9/6/4，无 noise 桶），接近用户复盘旧 HDBSCAN 16 簇后归纳的 5-6 个语义大类。layer 1 是层次树的自然中间层，不依赖任何 factor 阈值，比最终树切稳定得多。用户确认 structured 聚类只是当**种子标注工具用完即弃**：在这 7 簇上快速标一轮 drop/keep/compact 决策，产出 ~300 条带决策标签的数据作为后续 CueQC Mamba 训练种子；structured 聚类本身不充当 QC 分类器（手工特征表达不了"听起来像碎噪声"的音频信号，要靠 Mamba 隐状态聚类）。产物 `agents/temp/20260616_clueqc-torque-10film/`（`--merge-layer 1 --feature-space structured`）。`requirements.txt` 删 `scikit-learn>=1.9`、加 `networkx`。测试 `tests/test_cueqc_cluster_first.py` 重写为 TORC（含 merge_layer 层次划分用例），`8 passed`，`py_compile` 通过。
- 2026-06-16 双 Mamba + 时序聚类新方向（取代 content_type 路线）：用户指出固定 `content_type` 标签不好定（16 簇合并仍有边界模糊），决定转向 **去类别、决策驱动** 架构。新工作流：Boundary Mamba(切分)→ASR→CueQC Mamba(序列建模，输出每 chunk 表示 + 决策)→在 CueQC Mamba 隐状态上做时序聚类（TORC）→簇驱动下游 drop/mark/compact/review，不命名 content_type。CueQC Mamba 训练目标定为：去掉 content_type head，保留 `display/align/qc` 三个决策头 + 表示向量输出；聚类不进训练 loss，只在推理/审计时对表示做 TORC。关键补充：用户要求 QC 结果反哺 Boundary 切分以减少碎的无意义短切片（如 cluster_00 的 0.4-0.9s 短噪声）；确定走**离线偏好蒸馏**路径（CueQC 的 display=drop + 短时长沉淀为 Boundary preference 数据，离线 fine-tune Boundary），不引入运行时耦合，保持 Boundary 仍是纯 timing 模型。数据流分工确认：10 部影片 ASR 全量是 `45643` 条候选（每部 1933-8859），`300` 条只是按 stratum（density/qc/repeat）+ 每部 30 条的分层抽样子集；300 条定位为**一次性人工标注种子**（在 TORC 7 簇上标 drop/keep/compact → 训练 CueQC Mamba），全量 45643 条由模型上线后自动处理（隐状态聚类当 QC 分类器），structured TORC 不在模型链路上。本次先收口 TORC 代码替换 + 7 簇种子审计（已完成、测试通过），用户在审计页标完决策标签后再启动 CueQC Mamba 架构改造和 Boundary 反哺。旧 `20260615_205639_cueqc-handoff.md` 的 17 簇人工标签、顶层 taxonomy 映射因换算法而失效（簇号簇数对不上），但其语义大类归纳和"不训练/不启用 pre-align skip/匿名样片 A holdout"约束仍保留参考。
- 2026-06-16 TORC layer1 审计页接入音频/字幕并发布到导航页：用 `tools/audits/generate_cueqc_cluster_audit_html.py` 从 TORC `--merge-layer 1` 的 `cueqc_clusters.jsonl` 重建带音频的审计页（生成器只读 clusters.jsonl，与聚类算法解耦），baseline root `agents/temp/20260615_094437_b5/agents/temp/speech-boundary-ja/20260615_094437_o10/` 自动映射 10 部视频的 `.wav`、`.ja.srt` 与 `aligned_segments.json`。审计入口 `agents/audits/20260616_clueqc-torque-layer1-audit/index.html`（dataset_id `cueqc-10film-torque-layer1-20260616`），已生成 `subtitles/*.ja.vtt` 共 10 个，`--refresh-nav` 后 `agents/audits/index.html` 倒序第一项（带"最新"徽章），`latest-audit.html` 已更新，旧 HDBSCAN 审计降到第二。HTML 数据校验：`300` rows / `7` clusters（cluster_00..06）/ `10` videos 且首个视频 `audio_exists=true`，VTT 10 个无缺失。
- 2026-06-16 簇级标注改为 display 二分类种子（CueQC Mamba 路由头定型）：审计页簇级标注从"自由文字标签（cluster_label + classification_reason）"改为**纯策略二分类**——每簇选一个 `display_decision`（keep=进 aligner 正常显示 / drop=不进 aligner 丢弃+boundary 偏好种子）+ 可选 reason 备注。决策依据：(1) CueQC Mamba 定为**单路由头二分类**，最干净最易训练、reward 最清晰；(2) 去掉 compact（=keep 的显示变体，aligner 行为相同，判断标准模糊）；(3) 去掉 review（不确定标签是训练噪声，fallback 默认值不需要人工标）。改 `generate_cueqc_cluster_audit_html.py`：簇级 UI 用 `setClusterButtons("clusterDisplayButtons", CLUSTER_DISPLAY, clusterId, "display_decision")` 渲染 keep/drop 按钮（`CLUSTER_DISPLAY=[{keep,保留（进 aligner 正常显示）},{drop,丢弃（不进 aligner + boundary 偏好种子）}]`），删 `activeClusterLabel` 输入框，`isClusterComplete` 改为只要求 `display_decision∈{keep,drop}`，`exportClusterRows` 导出加 `display_decision` 字段（schema 仍 `cueqc_cluster_label_v1`，cluster_label 保留为空），clusterNav/状态文案改为显示策略，manifest 加 `cluster_display_decision_options`/`cluster_display_decision_required`。逐条标注（legacy hidden UI）不动。重新生成审计页校验 8/8 通过（clusterDisplayButtons/CLUSTER_DISPLAY/setClusterButtons/无 activeClusterLabel/export display_decision/isClusterComplete/keep/drop 文案 + manifest）。导出产物 `cueqc_cluster_labels.jsonl` 每行带 `display_decision`，是 CueQC Mamba 二分类路由器的训练种子。这是用户标 keep/drop 决策种子的入口。
- 2026-06-16 簇级标注完成 + cold-start 训练数据生成 + 三阶段收敛路径定型：用户完成 7 簇 display 二分类审计（`agents/audits/20260616_clueqc-torque-layer1-audit/cueqc_cluster_labels.jsonl`）。结果：cluster_00(152)=keep、01(72)=drop、02(45)=drop、03(12)=keep、04(9)=drop、05(6)=drop、06(4)=drop，即 keep 164 条(54.7%)/drop 136 条(45.3%)，二分类均衡。cluster_00 备注含语义语句混呻吟声、cluster_02 备注呻吟声混少量短语义句——印证 structured 聚类粒度粗的局限。
  **cluster_00 噪声量化**：152 条中 31 条(20%) qc=reject、29 条(19%) repeated_vocalization，这些大概率该 drop 但被簇级 keep 误标。
  **cold-start QC 覆盖**：`compile_training_set.py` 加 `_broadcast_cluster_labels`（簇级→逐条广播）+ `--cluster-labels` 参数 + QC=reject 覆盖逻辑（簇级 keep 但 qc.severity=reject 的样本覆盖成 drop，`label_source="cold_start_qc_override"`，可通过 `--no-qc-override` 关闭）。生成训练数据 `agents/temp/20260616_clueqc-cold-start-train/cueqc_train.jsonl`：300 records/0 skipped，最终 keep 133 / drop 167（其中 31 条是 cluster_00 的 QC 覆盖），alignment_policy=align 133 / skip_align_fallback 167。每条 record 带 `label_meta.label_source`（cluster_broadcast / cold_start_qc_override / manual）便于后续审计与退役。
  **三阶段收敛路径（定型，解决"什么时候修好 cluster_00 粗粒度"）**：(阶段0/cold-start,现在) 簇级广播 + QC 规则覆盖，仅用于 Mamba 冷启动 bootstrap，QC 覆盖退役标记 `cold_start_qc_override`；(阶段1/Mamba v1) 在 Mamba 隐状态上重新 TORC 聚类 + 用户在新簇重新标，QC 规则覆盖退役、structured 聚类退役；(阶段2/自训练) Mamba v1 对全量 45643 条出预测→高置信样本当伪标签→训 Mamba v2，簇级种子退役。structured 聚类、QC 覆盖都是 cold-start 用完即弃的权宜手段，不进自训练循环。
- 2026-06-16 CueQC Mamba v3-Fusion 断兼容重构落地（取代 v2 multimodal，权威设计见 `docs/cueqc_mamba_v3_plan_extracted.md`）：v2 用 `boundary PTM+MFCC frame + BGE-m3 text embedding + structured → display`；v3 改为 `ASR encoder features + ASR token trace + decoder aggregate stats + structured metadata → display keep/drop`，删除 BGE-m3 / sentence-transformers / boundary frame 作为主输入。核心可行性结论：ASR 后端 Qwen3-ASR 的 `transcribe()` 只返回文本，**不**返回 token logits / encoder frames；`asr_signals.avg_logprob` 现状恒 `None`。v3 用 **teacher-forced forward 重算**（零侵入 `qwen_asr` 包）拿 per-token logits（logprob/entropy/top1-top2 margin）+ 复用 `thinker.get_audio_features`（`src/boundary/ja/features.py` 已验证）拿 encoder frames。runtime 复用已加载的 `LocalAsrBackend.model`，不加载第二份 Qwen3-ASR。新增/重写文件：`src/asr/cueqc_features.py`（TOKEN/DECODER/STRUCTURED feature names + builders，13/22/14 维）、`src/asr/asr_internals.py`（`AsrInternalsCapturer`，复用 `wrapper._build_text_prompt` 单源 prompt，处理 causal shift `logits[:, prompt_len-1:prompt_len-1+L]`）、`src/asr/cueqc_model.py`（`CueQCMambaV3Fusion` 4 臂：ASR/Token 各 Bi-Mamba2 + masked mean+max pool，Decoder/Structured 各 LayerNorm→MLP，fusion MLP→[B,2]，logits[:,0]=drop/[:,1]=keep）、`src/asr/cueqc_refiner.py`（`CueQCRefinerV3Fusion` + `load_cueqc_mamba_checkpoint`，保守 `p_drop>=0.85` 才 drop，所有异常 fallback keep）、`tools/asr/cueqc/extract_features_v3_fusion.py`（teacher-forced 提取，输出 `cueqc_mamba_v3_fusion_features` schema，label **反转** drop=0/keep=1）、`tools/asr/cueqc/train_mamba_v3_fusion.py`（固定 `--test-audio-id` 影片 holdout，normalization 只在 train split 算防泄漏，CrossEntropy keep 权重 1.5 + label_smoothing，报告 test_accuracy/keep_recall/drop_precision/drop_recall/**false_drop_rate**，checkpoint schema `cueqc_mamba_checkpoint_v3_fusion` 含 normalization/decision_config/label_config）。pipeline 接入：`_run_cueqc_shadow` 加 `backend=` 参数，`CUEQC_MODEL_PATH` 存在时模型决策覆盖 heuristic（heuristic 仍作 shadow 审计），`_apply_cueqc_drop_filter` 在 alignment 前按 `chunk["index"]` 同步过滤 `chunk_infos`/`text_results` 两个并行数组（只 drop `mode=cueqc_mamba_v3_fusion` 的候选）。特征捕获必须在 `backend.unload_model()`（pipeline.py ~1069）之前完成。config：`CUEQC_MODEL_PATH=src/asr/checkpoints/cueqc_mamba_v3_fusion.pt`、`CUEQC_DROP_THRESHOLD=0.85`、`CUEQC_DROP_APPLY_ENABLED=1`、`CUEQC_DEVICE=auto`、`CUEQC_USE_BGE=0`、`CUEQC_FALLBACK_POLICY=keep`。删除 v2 `tools/asr/cueqc/extract_features.py`、`train_mamba.py`、`enrich_embeddings.py`（被 v3 取代，无 importer）。依赖库代码未改动（`asr_internals.py` 全程外部调用 `wrapper._build_text_prompt` / `thinker.get_audio_features`）。300 样本 cold-start 阶段关注 keep_recall/false_drop_rate，后续 40k+ 语料重训是既定路线。验证：全文件 `py_compile` + 端到端 smoke 通过。
- 2026-06-17 CueQC v3-Fusion 训练落地（cold-start 300 样本）：特征提取 300 条全成功（0 跳过，asr_dim=1024，labels keep=133/drop=167）。**实施期发现两个关键 bug 已修**：(1) `Qwen3ASRForConditionalGeneration.forward` 是未实现的 `_forward_unimplemented` 存根——teacher-forced 必须走 `wrapper.model.thinker.forward(...)`（返回 `Qwen3ASRThinkerCausalLMOutputWithPast.logits`），不能调顶层 `.forward()`；(2) `get_audio_features` 在 batch=1 时返回 `[T, D]` 而非 `[1, T, D]`，capturer 加 `dim()==2 → unsqueeze(0)` 规整。训练诊断：首轮（hidden=128/2层/dropout=0.2/500步无 early-stop）**严重过拟合**——logits 爆炸到 ±14、train acc 0.95、test_keep_recall 仅 0.5625、false_drop_rate 0.4375（43.75% 有效字幕被误删，不可用）。根因：cold-start 标签是**簇级广播**生成（7 簇中 6 簇 pure），模型学到簇/影片伪信号而非通用 display 质量；加上无 early-stop 跑满 500 步。修复：训练脚本加 `label_smoothing`（防 logits 爆炸）+ val `keep_recall-false_drop_rate` 上的 early-stop + 保存最优 checkpoint；重训用保守配置（hidden=64/1层、dropout=0.3、label_smoothing=0.15、lr=5e-4、weight_decay=1e-2）。**结果**：early-stop 在 step 10 达最优，test_keep_recall=**1.000**（目标≥0.95 ✅）、test_false_drop_rate=**0.000**（✅）、test_accuracy=0.833、drop_recall=0.643（欠删方向，安全）。完全符合 v3"宁可漏删无效片段、绝不误删有效字幕"的保守减法设计。drop_recall 偏低是 300 样本+簇广播标签的固有局限，40k+ 语料重训会改善。checkpoint 分发到 `src/asr/checkpoints/cueqc_mamba_v3_fusion.pt`（968KB），runtime `load_cueqc_mamba_checkpoint` 加载验证通过（schema/drop_threshold=0.85/normalization shape 正确）。test 用 `匿名样片 D` 做 9 部内部 holdout（匿名样片 A 真实 test 集需后续单独导出标注）。
...
- 2026-06-13 true v5 boundary preference 人工盲测完成并通过第一阶段 gate：用户导出的 `agents/audits/true-v5-boundary-preference-pilot/manual_boundary_preference_labels.jsonl` 有 `120` 行、schema/dataset ID 一致且 item ID 无重复；标签分布为 `A better=35`、`B better=51`、`tie=10`、`both bad=23`、missing `1`。漏标项为 display index `44`、item `bp-401896c9bc9f`、视频 `匿名样片 A`，但不影响预先定义的 gate。运行 `tools/boundary/summarize_boundary_preferences.py` 得到 hidden duplicate consistency `10/12=0.833333`、usable `119`、decisive `86`、decisive ratio `0.722689`、unique challenger wins `37`、challenger win category coverage `8/8`，五项 gate 全部通过；各类别 challenger wins 为 `left.end minus large/small=3/2`、`left.end plus large/small=6/6`、`right.start minus large/small=2/6`、`right.start plus large/small=8/4`。随后运行 `tools/boundary/compile_boundary_preferences.py`，输出 `agents/temp/speech-boundary-ja/true-v5-boundary-preference-pilot/compiled_preference_v5.jsonl` 和 `compile_summary.human.json`：编译 `77` 条 unique decisive rows，baseline/challenger winner 为 `40/37`，`right.start=40`、`left.end=37`；跳过 hidden duplicates `12`、tie `10`、both bad `20`、missing `1`。全部行保持 `boundary_refiner_frame_sequence_dataset_v5`，feature dim `630`，`right.start` weight `[1.0,0.0]`、`left.end` weight `[0.0,0.6]`，无异常行。现有训练器 `_load_dataset_tensors_from_jsonl` 成功读取为 features `(77,1,630)`、targets/weights `(77,1,2)`、mask items `77`，确认无需修改 runtime/checkpoint schema。第一阶段只完成 gate 与标签编译，没有训练 v5.1、没有替换默认 checkpoint；按路线下一步扩充到至少 300 条有效偏好后再评估冻结 backbone 的低学习率 delta-head finetune。
- 2026-06-13 true v5 boundary preference 审计播放窗口调整：原“播放 A/B”严格从候选字幕 start 播到 end，且字幕遮罩全程常驻，不利于判断字幕出现和消失是否合适。`tools/audits/generate_boundary_preference_audit_html.py` 现将 A/B 播放窗口扩为候选字幕前 `1.5s`、结束后 `1.5s`，同步扩展时间轴 context，字幕遮罩只在各自候选的实际 start/end 区间显示；“播放上下文”保持完整 context。使用原 blind manifest 和原 dataset ID `true-v5-boundary-preference-pilot-20260611` 重新生成 `agents/audits/true-v5-boundary-preference-pilot/index.html`，因此已有 localStorage 标注不失效；导航页仍将该审计放在倒序第一项并标记“最新”。聚焦测试 `tests/test_boundary_preference.py tests/test_audit_nav.py` 为 `13 passed`，语法检查通过。
- 2026-06-11 true v5 boundary preference 第一阶段已落地并停在人工标注 gate：新增 `tools/boundary/boundary_preference.py`、`build_boundary_preference_pilot.py`、`summarize_boundary_preferences.py`、`compile_boundary_preferences.py` 和 `tools/audits/generate_boundary_preference_audit_html.py`。构建命令使用项目 `.venv`、`PYTHONIOENCODING=utf-8`、`uv run`，对 `匿名样片 D`、`匿名样片 A`、`匿名样片 B` 以 CUDA 和当前默认 true v5 checkpoint `503d7e2299460aff555e02cba2b840c59195e577719bd0637a5ae98657ef919f` 重算边界特征；三片时长分别 `3328.186/5396.643/9069.621s`，speech segments `1737/2589/4125`，planned chunks `1933/3024/5138`，实际 feature hop 均为 `20ms`。候选只改 `right.start` 或 `left.end` 单轴，偏移为实际 frame 对齐的 `±80/±160ms`，每个候选仅物化并重跑 baseline/challenger 的受影响相邻 chunk；Qwen 0.6B ASR、QC 和 forced alignment 只用于候选排序与审计，不作为训练监督。完整构建 elapsed `181.2s`，ASR text/QC/alignment 分别 `42.647/0.025/16.377s`；prepool `384` 条、候选 chunk `768` 个，最终选 `108` unique 并加入 `12` hidden duplicates，生成 `120` 条盲测。unique 配额为每片 `36`、两轴各 `54`、四个偏移各 `27`，8 个轴/方向/尺度类别各 `13-14`；风险桶为 gap crossing `54`、QC/alignment change `19`、repeat/nonlexical `12`、cue density `9`、stable control `8`、text changed `6`。排序只使用 ASR text stability、ASR QC、repeat/nonlexical、forced/fallback/sentinel、cue density、gap crossing，明确未用 local CER、`avg_logprob`、`no_speech_prob` 或 token confidence。盲文件未包含 baseline/challenger 身份、候选 ID、重复关系、轴、偏移或类别；12 对重复项独立随机 A/B，其中 `7` 对左右相反。审计页位于 `agents/audits/true-v5-boundary-preference-pilot/index.html`，支持源视频 A/B/context 播放、`A better / B better / tie / both bad / uncertain`、timing 多标签、notes、localStorage 和 JSONL 导出；answer key、候选和空标签入口位于 `agents/temp/speech-boundary-ja/true-v5-boundary-preference-pilot/`。汇总 gate 已实现为 hidden duplicate consistency `>=0.80`、usable `>=90`、decisive ratio `>0.50`、challenger wins `>25`、category coverage `>3`；空标签 smoke 得到 `gate_passed=false`、各计数为 `0`。编译器在 gate 未过时退出码 `2` 且不写训练集；gate 通过后只编译 unique decisive winner，输出既有 `boundary_refiner_frame_sequence_dataset_v5`，winner 转成相对 raw boundary 的绝对 `sequence_boundary_delta_targets`，`right.start` 权重 `[1.0,0.0]`、`left.end` 权重 `[0.0,0.6]`，tie/both bad/uncertain 和 hidden duplicates 跳过。验证为 preference/training/audit nav 聚焦测试 `21 passed`、既有 Boundary Refiner 回归 `10 passed`，`py_compile` 与 `git diff --check` 通过；浏览器工具因安全策略拒绝直接打开本地 `file://` 页面，未绕过该限制，改以生成 HTML 数据级 smoke 和测试验证 localStorage、JSONL、标签完整性及 answer-key 隔离。当前人工标签文件为空，因此没有训练 v5.1、没有替换默认 checkpoint，也未使用 forced-aligner silver、pairwise runtime head、merge/padding/DP/speaker。
- 2026-06-11 true v5 后续路线决策：结合 active code、现有审计工具和外部研究重新审计 backlog。当前训练器只接受 `boundary_refiner_frame_sequence_dataset_v5` 的 `sequence_boundary_delta_targets/weights`，runtime/checkpoint 只允许 `boundary_refiner_v5`、`output_dim=2`、`start_delta/end_delta`；现有 Qwen 运行结果中的 `avg_logprob/no_speech_prob` 基本为空，因此 token confidence 不能作为首阶段依赖。用户确认不再管 v4，直接从当前 true v5 开始；当前 v5 为 synthetic-only 32768 hardmix，未使用 silver mining finetune。下一阶段采用 boundary perturbation preference 路线：对当前 v5 边界做 frame-hop 对齐的 `±80ms/±160ms` 单轴扰动，只重跑相邻 chunk，生成 120 条盲测 A/B 试点（108 unique + 12 hidden repeats），标签为 `A better / B better / tie / both bad / uncertain` 并附带 timing 原因。第一轮不使用 forced-aligner silver、不改 runtime schema、不新增 pairwise head、merge、padding、DP 或 speaker sidecar；有效偏好最终编译成现有 v5 absolute delta target，只监督被比较轴。若人工一致性和胜负信号未过 gate，则保留当前 v5 并停止 finetune；通过后扩到至少 300 条，冻结 backbone、只微调 delta head并加入 synthetic anchor replay，完整全片人工 A/B 通过前不替换默认 checkpoint。外部参考包括 Microsoft TWAD 的 lexicon-free word activity timing、EvalSubtitle/Sigma 的分段评价思想、PAIRS 的成对偏好排序和 TeLeS 的 ASR confidence estimation；TWAD/timeline teacher、Sigma、token confidence、RL/DPO 与 Unified Joint Model 均延后，原因分别是实现成本、缺少可靠参考字幕或当前后端缺少稳定置信输出。
