...
- 2026-06-30 Qwen3-ASR 非 `-hf` fine-tune 迁移验证：用户当前 `jaykwok/Qwen3-ASR-1.7B-JA-Anime-Galgame` 确认为基于 `Qwen/Qwen3-ASR-1.7B` 非 `-hf` 版 fine-tune。本地对比显示 fine-tune repo 与官方非 `-hf` repo 均为 `model_type=qwen3_asr`、`architectures=Qwen3ASRForConditionalGeneration`、`thinker_config` 结构；safetensors key 与官方非 `-hf` 基本同构，fine-tune 为 `707` 个 tensor、shape 全匹配，官方 base 多一个 `thinker.lm_head.weight`。再读取 `Qwen/Qwen3-ASR-1.7B-hf` 小文件与 safetensors header，确认 `-hf` config 将 `thinker_config.audio_config/text_config` 展平为顶层 `audio_config/text_config`，核心维度一致（audio d_model `1024` / `24` 层、text hidden `2048` / `28` 层、vocab `151936`），权重 header 为 `707` 个 `model.*` tensor。因此迁移可行，规则为 `thinker.* -> model.*` 并使用官方 `-hf` 模板文件。新增 `tools.asr.convert_qwen3_asr_to_hf`，真实转换输出 `agents/temp/20260630_120000_qwen3_asr_hf_conversion/`，`tensor_count=707`、`total_tensor_bytes=4076104960`、`shard_count=6`，校验输出全部为 `model.*` 且 index 覆盖 `707` 个权重。注意当前项目 `.venv` 的 `transformers==4.57.6` 还没有内置 `Qwen3ASR*`，原生 Transformers 推理需按官方 `-hf` README 使用支持 Qwen3-ASR 的 Transformers source/新版本；项目 runtime 暂不切换，继续默认 qwen-asr wrapper。
