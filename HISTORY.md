...
- 1.7B SpeechBoundary-JA scorer v4：按 2026-06-11/19 的 32768 hardmix 思路重建数据，监督 `speech`、`split_boundary`、`drop_gap` 三头。Galgame 随机拼接只表达“应切开”，不训练 speaker id；匿名样片 B 不进入训练或 pseudo pool。
- 1.7B Boundary Refiner v6：基于 scorer v4 输出重新导出 edge-only dataset，只训练 start/end trim，不学习中间切点。训练 summary 必须记录 ASR repo id、feature schema hash、dataset path 和 checkpoint 文件名。
