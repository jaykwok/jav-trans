...
- 2026-06-15 CueQC 第一轮 10-film baseline 候选池与 NAMH holdout 修正：最初误把 NAMH smoke 合入 11-film CueQC 审计池；复核后确认 NAMH-055 应作为 smoke/holdout 测试锚点，10-film baseline run 才是第一轮候选池。已重新从 CueQC 合入前 baseline commit `5afe535` 的 10 部匿名全片 aligned artifacts（均 `skip_translation=True` 且保留 ASR chunks）导出 CueQC 候选池，产物 `agents/temp/20260615_152934_cueqc-10film-candidates/`。全量 `cueqc_candidates.full.jsonl` 共 `45643` 条，按视频/QC/文本密度/重复 run 分层抽样 `cueqc_candidates.sampled.jsonl` 共 `300` 条，10 部各 `30` 条；全量分布包括 `density_empty_or_punctuation=12037`、`density_short_vocalization_candidate=9759`、`density_short_kana_dialogue_candidate=7137`、`qc_warn=5839`、`normal_dialogue_candidate=7596`。structured-only UMAP 对照产物 `agents/temp/20260615_152934_cueqc-10film-clusters-structured/` 得到 `16` 个稳定簇、`1` 个 noise 组，`cueqc_clusters.jsonl=300` 行、`cueqc_cluster_representatives.jsonl=82` 行，mean cluster probability `0.937111`、noise `18/300=6%`；低依赖 no-reducer HDBSCAN 产物 `agents/temp/20260615_152934_cueqc-10film-clusters-structured-none/` 得到 `16` 个稳定簇、noise `80/300=26.7%`，可作为默认无 UMAP 审计入口但质量略低；PCA 版 noise `117/300=39%`，不优先。下一步人工标注应以 10-film structured audit 为训练/聚类池，NAMH-055 只用于后续 holdout smoke/test；在人工标签导出前不运行 `compile_training_set.py` 或 `train_mamba.py`，也不启用 pre-align skip。
- 2026-06-15 CueQC 审计入口与清理：`agents/audits/index.html` 继续只作为导航页；已把当前第一轮低依赖审计入口发布到 `agents/audits/20260615_160304_cueqc-10film-structured-hdbscan-audit/index.html` 并重建 `latest-audit.html`，导航按时间倒序只显示该 CueQC 审计。旧审计目录 `true-v5-boundary-preference-pilot/`、`v5-subtitle-audio-audit/` 已移动到 `agents/rm/audit-deletions/20260615_160304_previous-audits/`；误合入 NAMH 的旧 11-film CueQC 候选池/聚类和 pytest 临时目录已移动到 `agents/rm/20260615_160304_cueqc-superseded-temp/`。保留当前有效 10-film 候选池、低依赖 HDBSCAN 审计源、UMAP/FINCH 对照源、baseline worktree 和 ASR chunks，避免丢失后续标注、对照和 holdout 验证所需数据。
